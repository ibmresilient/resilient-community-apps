...
from mock_artifacts import get_mock_config, get_emails, go_to_folder

PACKAGE_NAME = "fn_exchange"
FUNCTION_NAME = "exchange_move_emails"
...
def call_exchange_move_emails_function(circuits, function_params, timeout=10):
    # Fire a message to the function
    evt = SubmitTestFunction("exchange_move_emails", function_params)
    circuits.manager.fire(evt)
    event = circuits.watcher.wait("exchange_move_emails_result", parent=evt, timeout=timeout)
    assert event
...
class TestExchangeMoveEmails:
    """ Tests for the exchange_move_emails function"""
    ...
    @pytest.mark.parametrize(
        "exchange_delete_source_folder, exchange_force_delete_subfolders, exchange_email, exchange_folder_path, exchange_destination_folder_path, exchange_sender, exchange_start_date, exchange_end_date, expected_results", [
        (True, True, "user@exch.com", "Top of Information Store/TESTFOLDER_1", "Top of Information Store/TESTFOLDER_2", "text", 1518480000000, 1518480000000,
         {
             'email_ids': ['<ed6f175b4ef84ceaa921ba8bfdd37739@exch.exch.com>'], 
             'emails': {
    ...
    def test_success(self, mock_get_emails, mock_go_to_folder, circuits_app, exchange_delete_source_folder, exchange_force_delete_subfolders, exchange_email, exchange_folder_path, exchange_destination_folder_path, exchange_sender, exchange_start_date, exchange_end_date, expected_results):
        """ Test calling with sample values for the parameters """
        function_params = { 
            "exchange_delete_source_folder" : exchange_delete_source_folder,
            "exchange_force_delete_subfolders" : exchange_force_delete_subfolders,
        ...
        assert results
        assert "src_folder" in results
        assert "dst_folder" in results

        assert(expected_results.get("email_ids") == results.get("email_ids"))
        assert(expected_results.get("emails") == results.get("emails"))