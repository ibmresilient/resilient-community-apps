...
class TestUrlVoidFunction:
    ...
    @pytest.mark.parametrize("artifact_value, url_void_api, expected_results", [
        ("text", 'Scan', {"value": "xyz"}),
        ("text", 'Retrieve', {"value": "xyz"})
    ])
    def test_success(self, circuits_app, artifact_value, url_void_api, expected_results):
        """ Test calling with sample values for the parameters """
        function_params = { 
            "artifact_value": artifact_value,
            "url_void_api": url_void_api
        }
        results = call_url_void_function_function(circuits_app, function_params)
