...
class Resilient(object):
    ...
    def get_datatable(self, inc_id, table_name):
        ...
        """
        try:
            uri = "/incidents/{}/table_data/{}".format(inc_id, table_name)
    ...
    def create_type(self, mapped_type_name, sync_inc_id, sync_task_id, payload):
        ...
        try:
            ...
            self.log.error("Unable to create %s, Incident %s", mapped_type_name, sync_inc_id)
            self.log.error(uri)
            self.log.error(payload)
            self.log.exception(err)
            new_type_id = None
    ...
    def upload_attachment(self, src_rest_client, src_inc_id, src_org_id, type_name, payload, orig_id):
        ...
        if payload.get("task_id", None):
            ...
            sync_inc_id, sync_type_id = self.dbsync.find_sync_row(src_org_id, src_inc_id, "task", src_task_id)
            dst_task_id = sync_type_id
        else:
            src_task_id = None
            dst_task_id = None
            # get the target incident
            sync_inc_id, sync_type_id = self.dbsync.find_sync_row(src_org_id, src_inc_id, "incident", src_inc_id)
...
def get_url(inc_id, type_name, update_flag=False):
    ...
    return Resilient.DATATABLE.format(inc_id, type_name), True # if type not found, it's a datatable

def set_sync_fields(payload, orig_org_id, orig_inc_id):
    """
    keep a record of which incident created the copy in the newly created incident
    :param payload:
    :param orig_org_id:
    :param orig_inc_id:
    :return:
