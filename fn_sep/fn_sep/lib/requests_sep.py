        ...
        with ZipFile(BytesIO(content), 'r') as zfile:
            for zip_file_name in zfile.namelist():
                if len(zip_file_name) in HASH_LENGTHS or zip_file_name == "metadata.xml":
                    ...
                    raise ValueError(f'Unknown hash type or key for zipfile contents: {zip_file_name}')
...
def callback(response):
    if response.status_code == 410 and \
            findall("/policy-objects/fingerprints.*$", response.url):
        ...
        # Allow error to bubble up to the SOAR function.
    elif response.status_code == 400 and \
            findall("/groups/.*/system-lockdown/fingerprints/.*$", response.url):
