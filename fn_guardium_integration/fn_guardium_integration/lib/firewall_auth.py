...
from fn_guardium_integration.util.static_data import F_URL, F_LOGIN_URL
...
def firewall_authenticate(bso_ip, bso_user, bso_password, log, cert=False):
    ...
    try:
        ...
        if get_response.ok:
            ...
            log.debug(u"Error while connecting to firewall: {} {}".format(get_response.status_code, get_response.reason))
