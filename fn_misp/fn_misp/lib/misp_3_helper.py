...
def search_misp_attribute(misp_client, search_attribute):
    ...
    if not isinstance(search_results, list):
        raise IntegrationError(
            f"Received an unexpected response type from the MISP API. Expected a list but received: {type(search_results)}")
    return {
...
def get_attribute_uuid(misp_client, misp_attribute_value, misp_event_id):
    ...
    if not attributes:
        log.error(
            f"Could not get a uuid for event = {misp_event_id} and attribute = {misp_attribute_value}. Does it exist?")
        raise IntegrationError(
            f"Failed to find any attributes on event {misp_event_id}")
        ...
        if attribute_uuid:
            ...
            raise IntegrationError(
                f"Failed to match attribute value = {misp_attribute_value} for any attributes associated with event = {misp_event_id}")
...
def create_tag(misp_client, misp_attribute_value, misp_tag_type, misp_tag_name, misp_event_id):
    if misp_tag_type == "Event":
        ...
        object_uuid = get_attribute_uuid(
            misp_client, misp_attribute_value, misp_event_id)
