...
### Result
After installing from **RuleAndScript.res**, the Resilient platform will have a new Python script called "Generic email script v3.0.0" and a new rule called "Process email message v3.0.0". The rule runs the script when it is triggered by a new email message being received by the Resilient platform. If instead you imported **ScriptAlone.res** then you will only have the new script and not the rule.
