...
class FunctionComponent(AppFunctionComponent):
    ...
    def _app_function(self, fn_inputs):
        ...
        bigfix_action_id = fn_inputs.bigfix_action_id

        self.LOG.info(f"bigfix_action_id: {bigfix_action_id}")
        yield self.status_message(f"Running Query BigFix for BigFix action id '{bigfix_action_id}' ...")

        # Check status every 'retry_interval' secs up to 'retry_timeout' secs
        status = None
        try:
            (status, message) = poll_action_status(BigFixClient(self.opts, self.options), bigfix_action_id,
        ...
        if not status:
            ...
            yield self.status_message("Received successful status message '{}' for BigFix action {}.".format(sub('\.$', '', message), bigfix_action_id))
            results = {"status": "OK", "status_message": message}
        elif status == "Failed":
            yield self.status_message(f"Received error status {message} for BigFix action {bigfix_action_id}.")
            results = {"status": "Failed", "status_message": message}
        elif status == "Unsupported":
            yield self.status_message(f"Received unexpected status {message} while retrieving status for BigFix action {bigfix_action_id}.")
        elif status == "Timedout":
            yield self.status_message(f"Timed out getting action status for BigFix action {bigfix_action_id}")
