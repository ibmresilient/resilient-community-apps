...
class TestPostBfActionQuery:
    ...
    def test_post_bf_action_query(self, mock_get, query, computer_id, action_name, expected_results):
        """Test create_attachment using mocked data."""
        bigfix_client = BigFixClient(get_opts(), get_config())
        status = bigfix_client._post_bf_action_query(
            query, computer_id, action_name)
    ...
    class TestProcessBfComputerQueryResponseToAttachment:
        ...
        @pytest.mark.parametrize(
            ...
            [
                (mocked_requests("/response_text", 13550086),
                 "Computer ID 13550086 Properties", "Computer ID 13550086 Properties"),
                (mocked_requests("/response_text", 12315195),
                 "Computer ID 12315195 Properties", "Computer ID 12315195 Properties"),
            ])
