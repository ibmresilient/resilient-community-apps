...
# name of field in the endpoint entity (alert, case, etc) with the ID value
ENTITY_ID = "Id"
# name of field in endpoint entity to reference the close state
ENTITY_CLOSE_FIELD = "Status"
# name of custom IBM SOAR case field to retain the endpoint entity_id
SOAR_ENTITY_ID_FIELD = "salesforce_case_id"
# label the name the case, alert, event, etc. native to your endpoint solution
ENTITY_LABEL = "PagerDuty Incidents"
# header used to identify comments create by the endpoint entity
ENTITY_COMMENT_HEADER = "Created by PagerDuty"
...
class PollerComponent(AppFunctionComponent):
    ...
    def __init__(self, opts):
        ...
        super(PollerComponent, self).__init__(
            opts, PACKAGE_NAME, required_app_configs=required_fields)

        # collect settings necessary and initialize libraries used by the poller
        if not self._init_env(opts, self.options):
            LOG.info(
                "Poller interval is not configured.  Automated escalation is disabled.")
            return
    ...
    def _init_env(self, opts, options):
        ...
        if not self.polling_interval or is_this_a_selftest(self):
            LOG.debug(
                "Exiting poller because polling interval set to 0 or this run is a selftest.")
            return False

        LOG.info("Poller initiated, polling interval %s",
        ...
        self.soar_create_case_template = options.get(
            "soar_create_case_template")
        self.soar_update_case_template = options.get(
    ...
    def run(self, *args, **kwargs):
        ...
        query_results = query_entities(
            self.app_common, kwargs["last_poller_time"])
    ...
    def process_query_list(self, query_results):
        ...
        for entity in query_results:
            try:
                ...
                # create linkback url
                entity["entity_url"] = self.app_common.make_linkback_url(
                    entity_type='Case', entity_id=entity_id)

                # determine if this is an existing SOAR case
                soar_case, _error_msg = self.soar_common.get_soar_case(
                    {SOAR_ENTITY_ID_FIELD: entity_id}, open_cases=False)
                ...
                if not soar_case:
                    ...
                    soar_case_id = create_soar_case.get(
                        "id")  # get newly created case_id

                    cases_insert += 1
                    LOG.info("Created SOAR case %s from %s %s",
                             soar_case_id, ENTITY_LABEL, entity_id)
                else:
                    ...
                    if is_entity_closed(entity):
                        if soar_case.get("plan_status", "C") == "A":
                            ...
                            cases_closed += 1
                            LOG.info("Closed SOAR case %s from %s %s",
                                     soar_case_id, ENTITY_LABEL, entity_id)
                    else:
                        ...
                        cases_updated += 1
                        LOG.info("Updated SOAR case %s from %s %s",
                                 soar_case_id, ENTITY_LABEL, entity_id)
                # Add this entity to processed list.
                processed_cases.append(entity_id)
                LOG.debug(
                    "Salesforce case Id %s added to processed list", entity_id)
