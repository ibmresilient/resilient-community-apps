...
class FunctionComponent(ResilientComponent):
    ...
    def createdataobject(self,kwargs):
        ...
        cisco_dsturl, cisco_dstdomain = self._parseUrl(kwargs.get("cisco_dsturl"), kwargs.get("cisco_dstdomain"))
        ...
        self.log.info("cisco_dstdomain: %s", cisco_dstdomain)
        ...
        basicdata = {"alertTime": cisco_alerttime,
                ...
                "dstDomain": cisco_dstdomain.strip(),
                "dstUrl": cisco_dsturl.strip(),
    ...
    def _parseUrl(self, url, domain):
        if url and domain:
            d = urlparse(domain)

            return url, self._getDomain(d.hostname if d.hostname else d.path)
        elif not url and domain:
            d = urlparse(domain)
            hostname = d.hostname if d.hostname else d.path

            return 'http://'+hostname, self._getDomain(hostname)
        elif url and not domain:
            u = urlparse(url)

            return url, self._getDomain(u.hostname if u.hostname else u.path)

        raise ValueError('either cisco_dsturl or cisco_dstdomain required')


    def _getDomain(self, hostname):
        """ return a domain from a url. ex: www.abc.com returns abc.com """
        parts = hostname.split('.')
        if len(parts) > 2:
            return '.'.join((parts[len(parts)-2], parts[len(parts)-1]))
        else:
            return hostname
