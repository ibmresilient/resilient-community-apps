...
class FunctionComponent(ResilientComponent):
    ...
    def __init__(self, opts):
        ...
        self.max_rows = int(self.fn_options.get(
    ...
    def close_helix_incident(self, incident_id, task, helix_client, rp, helix_payload):
        ...
        if not rows:
            LOG.info("Task %s: %s not found in the BMC Helix datatable. Incidents in BMC Helix will be left unchanged.",
                     task["id"], task["name"])
        ...
        if len(rows) > 1:
            LOG.info("Multiple datatable rows found matching task %s: %s.\n"
                     "Any incidents matching these rows in BMC Helix will be closed.", task["id"], task["name"])
        ...
        for row in rows:
            ...
            try:
                ...
                if e.value.find("404 Client Error: Not Found") > -1:
                    ...
                    LOG.info("Unable to find Request ID %s in BMC Helix. It may have been deleted. Continuing to the next ID.",
                             request_id)
                else:
                    # Unknown error case - the helix_payload supplied might have a bad key
                    LOG.error("Received error response from BMC Helix when attempting to close Request ID %s.\n"
    ...
    def update_incident_values(self, helix_client, closed, skipped, incident, helix_payload):
        ...
        if incident["values"]["Status"] not in CLOSED_LIST:
            ...
            skipped.append(incident)
            LOG.info(
                "Request ID %s already closed, resolved, or cancelled. Skipping this incident.", request_id)
