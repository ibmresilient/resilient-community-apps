...
class LDAPUtilitiesHelper:
    ...
    def __init__(self, app_configs, ldap_domain_name):
        SUPPORTED_LDAP_AUTH_TYPE_TYPES = ["ANONYMOUS", "SIMPLE", "NTLM"]

        self.LDAP_SERVER = self.__get_config_option(app_configs=app_configs, option_name=ldap_domain_name+"-ldap_server", optional=False, placeholder="xxx.xxx.xxx.xxx")
        self.LDAP_PORT = int(self.__get_config_option(app_configs=app_configs, option_name=ldap_domain_name+"-ldap_port", optional=False))
        self.LDAP_USE_SSL = self.str_to_bool(self.__get_config_option(app_configs=app_configs, option_name=ldap_domain_name+"-ldap_use_ssl", optional=False))
        self.LDAP_AUTH_TYPE = self.__get_config_option(app_configs=app_configs, option_name=ldap_domain_name+"-ldap_auth", optional=False).upper()
        self.LDAP_PAS = self.__get_config_option(app_configs=app_configs, option_name=ldap_domain_name+"-ldap_password", optional=True)
        self.LDAP_USER_DN = self.__get_config_option(app_configs=app_configs, option_name=ldap_domain_name+"-ldap_user_dn", optional=True)
        self.LDAP_USER_NTLM = self.__get_config_option(app_configs=app_configs, option_name=ldap_domain_name+"-ldap_user_ntlm", optional=True)
        self.LDAP_IS_ACTIVE_DIRECTORY = self.str_to_bool(self.__get_config_option(app_configs=app_configs, option_name=ldap_domain_name+"-ldap_is_active_directory", optional=False))
        self.LDAP_CONNECT_TIMEOUT = int(self.__get_config_option(app_configs=app_configs, option_name=ldap_domain_name+"-ldap_connect_timeout", optional=False))
