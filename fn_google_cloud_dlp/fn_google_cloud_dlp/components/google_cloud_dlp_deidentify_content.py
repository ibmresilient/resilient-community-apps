...
class FunctionComponent(ResilientComponent):
    ...
    @function("google_cloud_dlp_deidentify_content")
    def _google_cloud_dlp_deidentify_content_function(self, event, *args, **kwargs):
        """Function: DLP Deidentify Content
        Used to take in an input of an attachment or artifact,
        parses the text and sends it to Google Cloud DLP for inspection
        Findings are de-identified by replacing all instances with a masking character."""
        try:
            ...
            try:
                response, de_identified_text = helper.deidentify_with_mask(
            ...
            # Upload our new attachment to Resilient. Only used on Attachment Workflows
            helper.upload_attachment_to_resilient(artifact_id, attachment_id, attachment_name, de_identified_text,
