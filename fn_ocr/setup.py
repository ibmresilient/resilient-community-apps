...
setup(
    ...
    entry_points={
        "resilient.circuits.components": [
            ...
            "{}FunctionComponent = fn_ocr.components.{}:FunctionComponent".format(
                snake_to_camel(get_module_name(filename)), get_module_name(filename)
            )
            for filename in glob.glob("./fn_ocr/components/[a-zA-Z]*.py")
        ],
        "resilient.circuits.configsection": [
            "gen_config = fn_ocr.util.config:config_section_data"
        ],
        "resilient.circuits.customize": [
            "customize = fn_ocr.util.customize:customization_data"
        ],
        "resilient.circuits.selftest": [
            "selftest = fn_ocr.util.selftest:selftest_function"
        ],
    },
)
