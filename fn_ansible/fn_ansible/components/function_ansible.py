    ...
    def _fn_ansible_function(self, event, *args, **kwargs):
        ...
        try:
            ...
            if ansible_parameters:
                for item in ansible_parameters.strip(',').split(","):
                    k, v = item.split("=")
                    extra_vars[k] = v

            # prepare playbook arg
            playbook_extention = playbook_name.split(".")[
                len(playbook_name.split(".")) - 1
            ]
            ...
            if playbook_name in os.listdir(playbook_dir):
                # As per ansible convention a trailing ',' given
                target_playbook = '/'.join((playbook_dir, playbook_name)), 
            else:
                raise FunctionError(
                    "Target playbook not present in following path: '%s'" % playbook_dir
                )
