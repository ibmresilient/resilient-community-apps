...
class FileFeedDestination(FeedDestinationBase):
    ...
    def send_data(self, context, payload):
        """Write a simplified version of the payload to a creatively named file in the configured 'directory'."""
        name = context.type_info.get_pretty_type_name()

        output_file = self._make_file_path(context, name, payload)

        logger.debug('writing %s to %s', name, output_file)
