...
class SqlDialect(object):
    ...
    def get_upsert(self, table_name, field_names):
        """Gets SQL that implements an 'upsert' for the specified table name with the specified field names.
    ...
    def get_delete(self, table_name):
        ...
        :returns The SQL for the delete (note that this will require a single bind parameter named "id"
        """
        raise NotImplemented
    ...
    def get_create_table_if_not_exists(self, table_name, column_spec_dict):
        ...
        :param column_spec_dict: A dict containing the column name to column spec mapping.  For example, it will
            contain something like {'id': 'integer primary key'}."""
        raise NotImplemented
    ...
    def get_add_column_to_table(self, table_name, column_name, column_spec):
        ...
        Ideally the returned DDL will not error if the column already exists.  However, if it is expected to
        generate an error, then you must also override is_column_exists_exception method to allow the exception
        to be bypassed.
    ...
    def is_column_exists_exception(self, e):
        """Determines if a given exception is due to an attempt to add a column that already exists.

        You only need to implement this if your SQL DDL returned by get_add_column_to_table can error if the
        column already exists.

        :param e: The exception that occurred while attempting to add the column.
    ...
    def get_parameters(self, parameter_names, parameters):
        """Get's the parameter values as the particular SQL access library wants them.  For example, pyodbc wants
        a list of values while SQLite wants a dict of named parameters.  This is in the dialect class because
        the dialect is the thing that returns the SQL and it will either be positional (using a '?' character) or
        reference names (e.g. ':inc_id')

        :param parameter_names: The names of the parameters.  Note that the order may be important here.  If your
        dialect is returning SQL with positional bind parameters then the order of parameter_names will dictate
        the order of the parameter values that you need to return.

        :param parameters: The actual object dict containing the parameters.  The key for the dict is the parameter
        names.

        :returns The parameter values in a format consistent with how you're handling bind parameters in this
        dialect class.
        """
        raise NotImplemented

    def configure_connection(self, connection):
        """Configures the connection.  Useful if you want to set the default charset for the connection or something
        like that.

        :param connection: The connection object."""
        pass

    def _get_column_type(self, input_type):
        """Gets the DB column type for the specified Resilient 'input type'

        :param input_type: The Resilient input type value (e.g. datepicker, boolean, number, text, text_area, etc.)
...
class ODBCDialectBase(SqlDialect):
    """Base class for all ODBC-based databases."""
    def __init__(self):
        super(ODBCDialectBase, self).__init__()
...
class SqliteDialect(SqlDialect):
    def __init__(self):
        super(SqliteDialect, self).__init__()
    ...
    def get_add_column_to_table(self, table_name, column_name, column_type):
...
class PostgreSQL96Dialect(ODBCDialectBase):
    ...
    def __init__(self):
        super(PostgreSQL96Dialect, self).__init__()

    def get_upsert(self, table_name, field_names):
        # The pyodbc API wants ? for the bind parameters, so prepend that for all of the fields.  Note that
        # self.get_parameters will eventually be called and that will ensure that binding is done in the right
        # order (that the values for a field correspond to the order in which it appears in our list of bind
        # parameters).
        #
