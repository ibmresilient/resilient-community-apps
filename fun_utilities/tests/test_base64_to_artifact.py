...
PACKAGE_NAME = "utility_functfun_utilitiesions"
FUNCTION_NAME = "base64_to_artifact"
...
def call_base64_to_artifact_function(circuits, function_params, timeout=10):
    ...
    circuits.manager.fire(SubmitTestFunction("base64_to_artifact", function_params))
    event = circuits.watcher.wait("base64_to_artifact_result", timeout=timeout)
    assert event
...
class TestBase64ToArtifact:
    ...
    @pytest.mark.parametrize("base64content, incident_id, artifact_file_type, file_name, content_type, description, expected_result", [
        ("text", 123, 'Log File', "text", "text", {"type": "text", "content": "line1\nline2"}, {"value": "xyz"}),
