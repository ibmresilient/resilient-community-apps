...
class WikiHelper():
    ...
    def get_wiki_contents(self, wiki_title_or_id):
        # grab the wiki content
        wiki_id = self.get_wiki_id_by_title(wiki_title_or_id)
        if not wiki_id:
            return None
    ...
    def update_wiki(self, wiki_id, wiki_title, content):
        payload = {
            "id": wiki_id,
            "title": wiki_title,
            "text": {
