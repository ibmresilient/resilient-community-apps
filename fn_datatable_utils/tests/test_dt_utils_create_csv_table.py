...
@pytest.mark.parametrize("inputs, mapping_table, expected_results",[
        ("'h1','h2','h3'\n'a','b','c'", { 'h3':"col0", 'h1':"col1", 'h2':"col2" }, {'col1': {'value': "a"}, 'col2': {'value': "b"}, 'col0': {'value': "c"}}),
        ("'h1','h2','h3'\n'a','b','c'", { 'h3':"col0", 'h2':"col2" }, {'col0': {'value': "c"}, 'col2': {'value': "b"}})
])
def test_build_row_has_header(inputs, mapping_table, expected_results):
    ...
    for data_row in csv_reader:
        result = build_row(data_row, mapping_table, DT_COLUMN_NAMES, DATE_FORMAT)
        for col in result:
            assert col in expected_results
            assert result[col] == expected_results[col]

@pytest.mark.parametrize("inputs, mapping_table, expected_results",[
    ("'a','b','c'", { 2: "col0", 0:"col1", 1:"col2" }, {'col1': {'value': "a"}, 'col2': {'value': "b"}, 'col0': {'value': "c"}}),
    ("'a','b','c'", { 0:"col1", 1:"col2" }, {'col1': {'value': "a"}, 'col2': {'value': "b"}})
])
def test_build_row_no_header(inputs, mapping_table, expected_results):
    s = StringIO(inputs)
    csv_reader = csv.reader(s, dialect=csv.Sniffer().sniff(inputs))
    for data_row in csv_reader:
        result = build_row(data_row, mapping_table, DT_COLUMN_NAMES, DATE_FORMAT)
        for col in result:
            assert col in expected_results
