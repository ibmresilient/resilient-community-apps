...
class FunctionComponent(ResilientComponent):
    ...
    def _browse_cve_api(self, vendor_name=None):
        ...
        if vendor_name:
            browse_api = f"{self.CVE_BASE_URL.format('browse')}/{vendor_name}"
        else:
    ...
    def _cve_db_information(self):
        ...
        """
        db_info_url = f"{self.CVE_BASE_URL.format('dbInfo')}"
    ...
    def _function_cve_browse_function(self, event, *args, **kwargs):
        ...
        try:
            ...
            return_limit = kwargs.get("return_limit", 100)  # number
            ...
            if api_call_type == 'browse':
                ...
                if len(_browse_dict_list) > return_limit:
                    _browse_dict_list = _browse_dict_list[:return_limit]
            ...
            log.debug(f"Vendor and Products Received from CVE Database: {result_data_dict}")
