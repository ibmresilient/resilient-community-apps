...
from resilient_lib import build_resilient_url, MarkdownParser, build_incident_url
...
class PostMessageClient:
    ...
    def post_message(self, options, required_parameters):
        """
        ...
        options       = options
        webhook_url   = required_parameters.get("webhook_url")
        teams_channel = required_parameters.get("teams_channel")
        teams_payload = required_parameters.get("teams_payload")
        ...
        self.build_conversation(
            options,
            card,
            task_id=required_parameters.get("incident_id"),
            incident_id=required_parameters.get("incident_id"),
            teams_payload=payload_json,
            teams_mrkdown=required_parameters.get("incident_id"))
    ...
    def build_conversation(self, options, card, **kwargs):
        ...
        incident_url = build_incident_url(
            build_resilient_url(
                ...
                options['port']),
                incident_id)
        case_type = constants.INCIDENT

        # build url back to resilient
        if task_id:
            incident_url = parse.urljoin(incident_url, constants.TASK_FRAGMENT, task_id)
            case_type = constants.TASK

        # url back to resilient
        card.addLinkButton(f"Resilient {case_type}", incident_url)
