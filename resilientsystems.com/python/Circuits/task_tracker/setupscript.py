...
"""
Example script to auto-create custom fields, rules, and message destinations.
"""
...
def create_message_destination(resilient_client, credentials):
    ...
    for user in users:
        ...
        LOG.error(u"User %s not found in org", credentials['queue_reader_email'])
...
def create_automatic_rule(resilient_client, credentials):
    """Creates an automatic rule assigned to the user in the config file"""
    # Create automatic rule that triggers whenever the task is changed from
    # active to closed or vice-versa. This automatic rule will be
    # pointed at the message destination we just created earlier.
    LOG.info("Creating new rule...")
    LOG.info("Pointing rule at message destination...")
    queue_id = get_queue_id(resilient_client, credentials["queue_name"])
    automatic_rule = {"object_type": 1,
                      "name": credentials['action_name'],
                      "message_destinations": [queue_id],
                      "conditions": [{"method": "changed",
                                      "field_name": "task.status"}]
    }
    resilient_client.post("/actions", automatic_rule)
...
def setup():
    ...
    # Create automatic rule with the specified user in the setup.config file
    create_automatic_rule(resilient_client, credentials)
