...
from fn_vmware_cbc.poller.configure_tab import init_vmware_cbc_tab
...
SOAR_ENTITY_ID_FIELD = "vmware_cbc_alert_id" # name of custom IBM SOAR case field to retain the endpoint entity_id
ENTITY_LABEL = "Carbon Black Cloud Alert" # label the name the case, alert, event, etc. native to your endpoint solution
ENTITY_COMMENT_HEADER = "Created by VMware CBC" # header used to identify comments create by the endpoint entity
...
def init_app(rc, options):
    ...
    # initialize Carbon Black Cloud tab in the SOAR UI
    init_vmware_cbc_tab()
...
def is_entity_closed(entity):
    ...
    :rtype: bool
    """
    workflow = entity.get("workflow", None)
    if not workflow:
        # Don't think we should get here.
        LOG.error("No workflow in CBC alert!")
        return False
    status = workflow.get("status", "")
    return True if status.lower() == "closed" else False
    ...
    def process_query_list(self, query_results):
        ...
        for entity in query_results:
            try:
                ...
                LOG.debug("VMware CBC case Id %s added to processed list", entity_id)
