...
from fn_rapid7_insight_idr.lib.app_common import (AppCommon, SOAR_HEADER)
from fn_rapid7_insight_idr.poller.configure_tab import (init_rapid7_insight_idr_tab)

PACKAGE_NAME = "fn_rapid7_insight_idr"
ENTITY_ID = "rrn"  # name of field in the endpoint entity (alert, case, etc) with the ID value
ENTITY_CLOSE_FIELD = "status" # name of field in endpoint entity to reference the close state
SOAR_ENTITY_ID_FIELD = "rapid7_insight_idr_rrn" # name of custom IBM SOAR case field to retain the endpoint entity_id
ENTITY_LABEL = "Rapid7 InsightIDR Investigation" # label the name the case, alert, event, etc. native to your endpoint solution
ENTITY_COMMENT_HEADER = "Created by Rapid7 InsightIDR" # header used to identify comments create by the endpoint entity
...
def query_entities(app_common, last_poller_time):
    ...
    query_results, error_msg = app_common.query_entities_since_ts(last_poller_time)

    return query_results, error_msg
...
def is_entity_closed(entity):
    ...
    :rtype: bool
    """
    return bool(entity.get(ENTITY_CLOSE_FIELD, "OPEN") == "CLOSED")
...
class PollerComponent(AppFunctionComponent):
    ...
    def _init_env(self, opts, options):
        ...
        self.polling_add_case_url_comment_in_rapid7 = str_to_bool(options.get("polling_add_case_url_comment_in_rapid7", "True"))
        LOG.info("Poller add case URL comment to Rapid7 InsightIDR investigation: %s", self.polling_add_case_url_comment_in_rapid7)
    ...
    def run(self, *args, **kwargs):
        ...
        query_results, error_msg = query_entities(self.app_common, kwargs["last_poller_time"])
    ...
    def process_query_list(self, query_results):
        ...
        for entity in query_results:
            try:
                ...
                # create linkback url
                entity["entity_url"] = self.app_common.make_linkback_url(entity_id=entity_id)
                ...
                if not soar_case:
                    ...
                    if self.polling_add_case_url_comment_in_rapid7:
                        # Build a URL to the SOAR case and update Rapid7 investigation with a comment
                ...
                LOG.debug("Rapid7 InsightIDR Id %s added to processed list", entity_id)
