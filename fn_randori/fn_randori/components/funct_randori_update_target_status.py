...
class FunctionComponent(AppFunctionComponent):
    ...
    def _app_function(self, fn_inputs):
        """
        Function: Update the specified target in Randori with the specified target status.
        Inputs:
        ...
        validate_fields(["randori_target_id", "randori_target_status", "randori_note"], fn_inputs)

        app_common = AppCommon(self.rc, self.PACKAGE_NAME, self.options)

        results = app_common.update_target_status(fn_inputs.randori_target_id, 
                                                  fn_inputs.randori_target_status)
