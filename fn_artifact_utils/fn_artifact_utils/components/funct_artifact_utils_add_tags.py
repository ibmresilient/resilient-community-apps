...
FN_NAME = "artifact_utils_add_tags"


class FunctionComponent(AppFunctionComponent):
    """Component that implements function 'artifact_utils_add_tags'"""
    ...
    def _app_function(self, fn_inputs):
        """
        Function: Function used to add one or more Tags from one or more Artifacts in an incident at the global level.
        Inputs:
        ...
        validate_fields(["incident_id", "artifact_id_list", "artifact_tag_values"], fn_inputs)
        ...
        Check Tag types.
        """
        # Grab available Tags from SOAR Instance.
        soar_tags = res_client.get('/tags/data?handle_format=names&exclude_unused=false')
        
        # Create a list of available Items and a dictionary of their IDs.
        tags_available = []
        tags_dict = {}
        for global_tag in soar_tags['entities']:
            tags_available.append(global_tag['name'])
            tags_dict[global_tag['name']] = global_tag['id']
        self.LOG.debug("Tags Available in Platform: %s", tags_dict)
        
        tag_id_list = []
        for tag in fn_inputs.artifact_tag_values.split(','):
            if tag not in tags_available:
                self.LOG.info("Tags not Available: %s", tag)
                tag_creation = {"display_name": tag}
                created_tag = res_client.post('/tags/data?handle_format=names', tag_creation)
                self.LOG.debug("Tag Creation Results: %s", created_tag)
                tag_id_list.append(created_tag['id'])
                self.LOG.info("Tag Created: %s", tag)
            else:
                tag_id_list.append(tags_dict[tag])
                self.LOG.info("Tag Found: %s", tag)
        ...
        for artifact_id in fn_inputs.artifact_id_list.split(','):
            global_artifact = res_client.get('/incidents/{}/artifacts/{}'.format(fn_inputs.incident_id, artifact_id))['global_artifact'][0]
            ...
            for tag_id in tag_id_list:
                ...
                if new_tag not in global_artifact['tags']:
                    self.LOG.info("Artifact does not contain Tag, Adding now: %s", new_tag)
                    global_artifact['tags'].append(new_tag)
            self.LOG.info("Updated Artifact: %s", global_artifact)
