...
from fn_mitre_integration.lib.mitre_attack import MitreAttack, MitreAttackTactic, MitreAttackTechnique, MitreAttackMitigation
...
class MitreQueryMocker(object):
    ...
    MITIGATIONS=[
                [{"type": "course-of-action",    "id": "course-of-action--f6b7c116-0821-4eb7-9b24-62bd09b3e575",    "created_by_ref": "identity--c78cb6e5-0c4b-4611-8297-d1b8b55e40b5",    "created": "2018-10-17T00:14:20.652Z",    "modified": "2019-07-25T11:25:50.338Z",    "name": "Port Knocking Mitigation",    "description": "Mitigation of some variants of this technique could be achieved through the use of stateful firewalls, depending upon how it is implemented.",    "external_references": [        {            "source_name": "mitre-attack",            "url": "https://attack.mitre.org/mitigations/T1205",            "external_id": "T1205"        }    ],    "object_marking_refs": [        "marking-definition--fa42a846-8d90-4e51-bc29-71d5b4802168"    ],    "x_mitre_deprecated": True,    "x_mitre_version": "1.0"}],
...
class TestMitreTechnique(object):
    ...
    def test_tech_of_tactic(self):
        ...
        assert MitreAttackTechnique.get_by_tactic(self.mitre_attack, MitreAttackTactic.get_by_name(self.mitre_attack,
                                                                                                   "Command and Control")) is not None
...
class TestMitreMitigation(object):
    ...
    def test_mitigation_of_tech(self):
        mitigations = MitreAttackMitigation.get_by_technique(self.mitre_attack, MitreAttackTechnique.get_by_name(self.mitre_attack, "Port Knocking"))
        print(mitigations)
...
class TestMitre(object):
    ...
    def test_get_all_tactics_from_all_frameworks(self):
        tactics = self.mitre_attack.get_all_tactics()
    ...
    def test_get_tactic_url(self):
        ...
        for tactic in tactics[:1]:
            url = self.mitre_attack.get_tactic_url(tactic.name)
            assert url_get(url)

        # Test error handling
        url = self.mitre_attack.get_tactic_url("Fake Tactic")
        assert (url is None)

    @pytest.mark.livetest
    def test_get_tech_mitigation(self):
        techs = self.mitre_attack.get_all_techniques()
        assert len(techs)
        try:
            #
            ...
            for tech in techs:
                id = tech.id
                assert(id)
                mitigation = self.mitre_attack.get_tech_mitigation(tech_id=id)
                assert mitigation
                count += 1
                if count > 2:
                    break
                # Test getting mitigation using name
                mitigation = self.mitre_attack.get_tech_mitigation(tech_name=tech.name)
                assert mitigation
        except:
            assert(False)
    ...
    def test_get_tech_info(self):
        data_mocker = MitreQueryMocker()
        with patch("fn_mitre_integration.lib.mitre_attack.TAXIICollectionSource.query", data_mocker.query):
            tech = self.mitre_attack.get_technique(name="Port Knocking")
            assert(tech["name"] == "Port Knocking")

            tech = self.mitre_attack.get_technique(ext_id="T1205")
            assert(tech["mitre_tech_id"] == "T1205")
            #
            #   Error handling
            #
            tech = self.mitre_attack.get_technique()
            assert tech is None
