...
class TestFnCsFalconSearch:
    ...
    @pytest.mark.parametrize("cs_filter_string, cs_query, expected_results", [
        ("text", "text", {"value": "xyz"}),
        ("text", "text", {"value": "xyz"})
    ])
    def test_success(self, circuits_app, cs_filter_string, cs_query, expected_results):
        """ Test calling with sample values for the parameters """
        function_params = { 
            "cs_filter_string": cs_filter_string,
            "cs_query": cs_query
        }
        results = call_fn_cs_falcon_search_function(circuits_app, function_params)
