...
FN_NAME = "harfanglab_telemetry_search_destination_ip"


class FunctionComponent(AppFunctionComponent):
    """Component that implements function 'harfanglab_telemetry_search_destination_ip'"""
    ...
    def _app_function(self, fn_inputs):
        ...
        try:
            ...
            results = conn.search_telemetry('searchDestinationIP', args)

            if incident_id:
                self.resilient_common = ResilientCommon(self.rest_client())
                self.resilient_common.add_csv_file_to_incident_attachments(incident_id, f'{FN_NAME}-{ip}.csv', results.get('output'))
