...
# Playbook - ExtraHop Reveal(x): Search Detections (PB)
...
| Assignee | `extrahop_detection_assignee` | text | Optional: Detection search filter assignee. Note :  To search by the  app ‘ExtraHop user’ use ‘me’ | Optional |
| Category | `extrahop_detection_category` | select | Optional:  Detection search filter category. Note: Performance categories not included in the selection list. | Optional |
| Detection ID | `extrahop_detection_id` | text | Optional: ExtraHop Detection ID. | Optional |
| Limit | `extrahop_limit` | number | (Optional) Limit the number of detections returned to the specified maximum number. | Optional |
| Mod time | `extrahop_mod_time` | datetimepicker | (Optional) Return detections that were modified on or after the specified date, expressed in milliseconds since the epoch. | Optional |
| Offset | `extrahop_offset` | number | (Optional) Skip the specified number of detections. This parameter is often combined with the limit parameter to paginate result sets. | Optional |
| Resolution | `extrahop_detection_resolution` | multiselect | Optional: Detection search filter resolution. | Optional |
| Risk Score minimum | `extrahop_detection_risk_score_min` | number | Optional: Detection search filter riskscore minimum. | Optional |
| Status | `extrahop_detection_status` | multiselect | Optional: Detection search filter status. | Optional |
| Ticket ID | `extrahop_detection_ticket_id` | text | Optional: Detection search filter SOAR case  ID. | Optional |
| Types | `extrahop_detection_types` | multiselect | (Optional): Detection search filter types. | Optional |
| Update time | `extrahop_update_time` | datetimepicker | (Optional) Returns detections related to events that occurred after the specified date, expressed in milliseconds since the epoch. Note that ExtraHop Machine Learning Services analyze historical data to generate detections, and so there is a time delay between when the events that cause those detections occur and when the detections are generated. If you search for detections in the same update_time window multiple times, the later search might return detections that were not returned by the earlier search. | Optional |
...
## Local script - ExtraHop: Write Detection to Detections Data Table

### Description
Write the detection information from get detection to the ExtraHop Detections data table.
...
---
## Local script - ExtraHop: Write Search Detections results to Detections data table

### Description
Write Search Detections results to Detections data table
