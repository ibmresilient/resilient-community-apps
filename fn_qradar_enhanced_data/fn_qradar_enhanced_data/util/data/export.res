{
  "action_order": [],
  "actions": [
    {
      "automations": [
        {
          "scripts_to_run": "Create Artifact from Assets info",
          "type": "run_script",
          "value": null
        }
      ],
      "conditions": [],
      "enabled": true,
      "export_key": "Create Artifact from Assets info",
      "id": 135,
      "logic_type": "all",
      "message_destinations": [],
      "name": "Create Artifact from Assets info",
      "object_type": "qr_assets",
      "tags": [],
      "timeout_seconds": 86400,
      "type": 1,
      "uuid": "c76c8216-cfd8-4338-8f43-a28efb3c9802",
      "view_items": [
        {
          "content": "09cb191e-152a-409c-80e2-dfdfb1490234",
          "element": "field_uuid",
          "field_type": "actioninvocation",
          "show_if": null,
          "show_link_header": false,
          "step_label": null
        }
      ],
      "workflows": []
    },
    {
      "automations": [
        {
          "scripts_to_run": "Create Artifact from Destination IP info",
          "type": "run_script",
          "value": null
        }
      ],
      "conditions": [],
      "enabled": true,
      "export_key": "Create artifact from Destination IP info",
      "id": 136,
      "logic_type": "all",
      "message_destinations": [],
      "name": "Create artifact from Destination IP info",
      "object_type": "qr_top_destination_ips",
      "tags": [],
      "timeout_seconds": 86400,
      "type": 1,
      "uuid": "aea0c9ec-256b-4ed6-b5be-eca496742d43",
      "view_items": [
        {
          "content": "73ba3586-ef44-49ef-be00-28172fc9a62e",
          "element": "field_uuid",
          "field_type": "actioninvocation",
          "show_if": null,
          "show_link_header": false,
          "step_label": null
        }
      ],
      "workflows": []
    },
    {
      "automations": [
        {
          "scripts_to_run": "Create Artifact from Events info",
          "type": "run_script",
          "value": null
        }
      ],
      "conditions": [],
      "enabled": true,
      "export_key": "Create Artifact from Events info",
      "id": 137,
      "logic_type": "all",
      "message_destinations": [],
      "name": "Create Artifact from Events info",
      "object_type": "qr_offense_top_events",
      "tags": [],
      "timeout_seconds": 86400,
      "type": 1,
      "uuid": "74440043-3014-4ec0-b603-c5f8237ca971",
      "view_items": [
        {
          "content": "da26c9f2-924b-425f-a641-c87d37968180",
          "element": "field_uuid",
          "field_type": "actioninvocation",
          "show_if": null,
          "show_link_header": false,
          "step_label": null
        }
      ],
      "workflows": []
    },
    {
      "automations": [
        {
          "scripts_to_run": "Create Artifact from Source IP info",
          "type": "run_script",
          "value": null
        }
      ],
      "conditions": [],
      "enabled": true,
      "export_key": "Create artifact from Source IP info",
      "id": 138,
      "logic_type": "all",
      "message_destinations": [],
      "name": "Create artifact from Source IP info",
      "object_type": "qr_top_source_ips",
      "tags": [],
      "timeout_seconds": 86400,
      "type": 1,
      "uuid": "a6ff072d-a0f3-42a9-97ce-cb71b1c86f77",
      "view_items": [
        {
          "content": "39f165e9-9b2e-4f84-b302-b5ea190191d1",
          "element": "field_uuid",
          "field_type": "actioninvocation",
          "show_if": null,
          "show_link_header": false,
          "step_label": null
        }
      ],
      "workflows": []
    },
    {
      "automations": [
        {
          "scripts_to_run": "Create Artifact from Flows info",
          "type": "run_script",
          "value": null
        }
      ],
      "conditions": [],
      "enabled": true,
      "export_key": "Create Artifacts from Flows Info ",
      "id": 139,
      "logic_type": "all",
      "message_destinations": [],
      "name": "Create Artifacts from Flows Info ",
      "object_type": "qr_flows",
      "tags": [],
      "timeout_seconds": 86400,
      "type": 1,
      "uuid": "125560d7-1a36-44a1-8e2b-2b1a33851ac9",
      "view_items": [
        {
          "content": "8aabde39-94fc-433c-9204-32fec7e84a8d",
          "element": "field_uuid",
          "field_type": "actioninvocation",
          "show_if": null,
          "show_link_header": false,
          "step_label": null
        }
      ],
      "workflows": []
    },
    {
      "automations": [],
      "conditions": [
        {
          "evaluation_id": null,
          "field_name": "incident.properties.qradar_id",
          "method": "has_a_value",
          "type": null,
          "value": null
        }
      ],
      "enabled": true,
      "export_key": "Example: QRadar Get QRadar Rule MITRE Reference",
      "id": 143,
      "logic_type": "all",
      "message_destinations": [],
      "name": "Example: QRadar Get QRadar Rule MITRE Reference",
      "object_type": "incident",
      "tags": [],
      "timeout_seconds": 86400,
      "type": 1,
      "uuid": "bd91b2fe-d6c1-4e69-a845-2c4c31ab9eba",
      "view_items": [],
      "workflows": [
        "example_qradar_get_mitre_reference_from_rules"
      ]
    },
    {
      "automations": [
        {
          "scripts_to_run": "Set Incident Last Updated Time",
          "type": "run_script",
          "value": null
        }
      ],
      "conditions": [
        {
          "evaluation_id": null,
          "field_name": "incident.properties.qradar_destination",
          "method": "has_a_value",
          "type": null,
          "value": null
        },
        {
          "evaluation_id": null,
          "field_name": "incident.properties.qradar_id",
          "method": "has_a_value",
          "type": null,
          "value": null
        }
      ],
      "enabled": true,
      "export_key": "QRadar Enhanced Data",
      "id": 140,
      "logic_type": "all",
      "message_destinations": [],
      "name": "QRadar Enhanced Data",
      "object_type": "incident",
      "tags": [],
      "timeout_seconds": 86400,
      "type": 0,
      "uuid": "ccbd85e5-8a84-4028-b472-d1fcd4547310",
      "view_items": [],
      "workflows": [
        "qradar_triggered_rules",
        "example_of_searching_qradar_flows_using_offense_id",
        "example_of_searching_qradar_top_events_using_offense_id",
        "qradar_assets_information",
        "qradar_categories",
        "qradar_destination_ips",
        "qradar_source_ips",
        "qradar_offense_summary"
      ]
    },
    {
      "automations": [],
      "conditions": [
        {
          "evaluation_id": null,
          "field_name": "incident.properties.qr_last_updated_time",
          "method": "changed",
          "type": null,
          "value": null
        },
        {
          "evaluation_id": null,
          "field_name": "incident.properties.qradar_destination",
          "method": "has_a_value",
          "type": null,
          "value": null
        },
        {
          "evaluation_id": null,
          "field_name": "incident.properties.qradar_id",
          "method": "has_a_value",
          "type": null,
          "value": null
        }
      ],
      "enabled": true,
      "export_key": "QRadar Enhanced Data Poller",
      "id": 141,
      "logic_type": "all",
      "message_destinations": [],
      "name": "QRadar Enhanced Data Poller",
      "object_type": "incident",
      "tags": [],
      "timeout_seconds": 86400,
      "type": 0,
      "uuid": "0b49ce9e-4e28-47b3-9666-7025f462c2b5",
      "view_items": [],
      "workflows": [
        "qradar_triggered_rules",
        "example_of_searching_qradar_flows_using_offense_id",
        "example_of_searching_qradar_top_events_using_offense_id",
        "qradar_assets_information",
        "qradar_categories",
        "qradar_destination_ips",
        "qradar_source_ips",
        "qradar_offense_summary"
      ]
    },
    {
      "automations": [
        {
          "scripts_to_run": "Set Incident Last Updated Time",
          "type": "run_script",
          "value": null
        }
      ],
      "conditions": [
        {
          "evaluation_id": null,
          "field_name": "incident.properties.qradar_destination",
          "method": "has_a_value",
          "type": null,
          "value": null
        },
        {
          "evaluation_id": null,
          "field_name": "incident.properties.qradar_id",
          "method": "has_a_value",
          "type": null,
          "value": null
        }
      ],
      "enabled": true,
      "export_key": "QRadar Enhanced Data Refresh",
      "id": 142,
      "logic_type": "all",
      "message_destinations": [],
      "name": "QRadar Enhanced Data Refresh",
      "object_type": "incident",
      "tags": [],
      "timeout_seconds": 86400,
      "type": 1,
      "uuid": "c3594283-9bde-4a9d-a071-6e66afd4c23d",
      "view_items": [
        {
          "content": "271acf00-3223-48a5-b507-d0787854d365",
          "element": "field_uuid",
          "field_type": "actioninvocation",
          "show_if": null,
          "show_link_header": false,
          "step_label": null
        }
      ],
      "workflows": [
        "qradar_triggered_rules",
        "example_of_searching_qradar_flows_using_offense_id",
        "example_of_searching_qradar_top_events_using_offense_id",
        "qradar_assets_information",
        "qradar_categories",
        "qradar_destination_ips",
        "qradar_source_ips",
        "qradar_offense_summary"
      ]
    }
  ],
  "apps": [],
  "automatic_tasks": [],
  "export_date": 1666356669120,
  "export_format_version": 2,
  "export_type": null,
  "fields": [
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "__function/qradar_search_param2",
      "hide_notification": false,
      "id": 2024,
      "input_type": "text",
      "internal": false,
      "is_tracked": false,
      "name": "qradar_search_param2",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": null,
      "read_only": false,
      "rich_text": false,
      "tags": [],
      "templates": [],
      "text": "qradar_search_param2",
      "tooltip": "",
      "type_id": 11,
      "uuid": "9d8fc645-5067-4ab0-b151-1d106dad91d6",
      "values": []
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "__function/qradar_search_param3",
      "hide_notification": false,
      "id": 2025,
      "input_type": "text",
      "internal": false,
      "is_tracked": false,
      "name": "qradar_search_param3",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": null,
      "read_only": false,
      "rich_text": false,
      "tags": [],
      "templates": [],
      "text": "qradar_search_param3",
      "tooltip": "",
      "type_id": 11,
      "uuid": "b6053690-d7b2-4204-b48a-f86503b419d9",
      "values": []
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "__function/qradar_offense_id",
      "hide_notification": false,
      "id": 2026,
      "input_type": "text",
      "internal": false,
      "is_tracked": false,
      "name": "qradar_offense_id",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": null,
      "read_only": false,
      "rich_text": false,
      "tags": [],
      "templates": [],
      "text": "qradar_offense_id",
      "tooltip": "The ID of the given offense",
      "type_id": 11,
      "uuid": "c6cac6bb-9718-40e9-8fb6-2cea26be7b5b",
      "values": []
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "__function/qradar_search_param6",
      "hide_notification": false,
      "id": 2027,
      "input_type": "text",
      "internal": false,
      "is_tracked": false,
      "name": "qradar_search_param6",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": null,
      "read_only": false,
      "rich_text": false,
      "tags": [],
      "templates": [],
      "text": "qradar_search_param6",
      "tooltip": "",
      "type_id": 11,
      "uuid": "ca2d7d7f-fe6d-4a21-b011-7953494c350c",
      "values": []
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "__function/qradar_search_param4",
      "hide_notification": false,
      "id": 2028,
      "input_type": "text",
      "internal": false,
      "is_tracked": false,
      "name": "qradar_search_param4",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": null,
      "read_only": false,
      "rich_text": false,
      "tags": [],
      "templates": [],
      "text": "qradar_search_param4",
      "tooltip": "",
      "type_id": 11,
      "uuid": "eefc9e56-2806-4534-b409-42e25154ecc9",
      "values": []
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "__function/qradar_search_param1",
      "hide_notification": false,
      "id": 2029,
      "input_type": "text",
      "internal": false,
      "is_tracked": false,
      "name": "qradar_search_param1",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": null,
      "read_only": false,
      "rich_text": false,
      "tags": [],
      "templates": [],
      "text": "qradar_search_param1",
      "tooltip": "",
      "type_id": 11,
      "uuid": "01ed4652-fe61-4dbf-bc83-f81c62c5563c",
      "values": []
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "__function/qradar_query_type",
      "hide_notification": false,
      "id": 2030,
      "input_type": "text",
      "internal": false,
      "is_tracked": false,
      "name": "qradar_query_type",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": null,
      "read_only": false,
      "rich_text": false,
      "tags": [],
      "templates": [],
      "text": "qradar_query_type",
      "tooltip": "",
      "type_id": 11,
      "uuid": "035f31c7-3bf8-4b5e-932f-4adb32198572",
      "values": []
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "__function/qradar_query",
      "hide_notification": false,
      "id": 2031,
      "input_type": "textarea",
      "internal": false,
      "is_tracked": false,
      "name": "qradar_query",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": null,
      "read_only": false,
      "rich_text": false,
      "tags": [],
      "templates": [
        {
          "id": 11,
          "name": "search events for username",
          "template": {
            "content": "SELECT %param1% FROM events WHERE username=%param2% LAST %param3% MINUTES",
            "format": "text"
          },
          "uuid": "34fa3300-c28c-4346-8d3b-745a1afa75dc"
        },
        {
          "id": 12,
          "name": "search ip address",
          "template": {
            "content": "SELECT %param1% FROM events WHERE sourceip=\u0027%param2%\u0027 LAST %param3% MINUTES",
            "format": "text"
          },
          "uuid": "dcbdb7c8-2068-4e97-8127-57131cbdccbf"
        },
        {
          "id": 13,
          "name": "search events for offense_id",
          "template": {
            "content": "SELECT %param1% FROM events WHERE INOFFENSE(%param2%) LAST %param3% MINUTES",
            "format": "text"
          },
          "uuid": "2b88eb5c-504a-4950-8674-0cf8f56f1a83"
        }
      ],
      "text": "qradar_query",
      "tooltip": "A QRadar query string with parameters",
      "type_id": 11,
      "uuid": "048ba39a-ab94-4d1f-a0f8-2462de3c044c",
      "values": []
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "__function/qradar_search_param7",
      "hide_notification": false,
      "id": 2032,
      "input_type": "text",
      "internal": false,
      "is_tracked": false,
      "name": "qradar_search_param7",
      "operation_perms": {},
      "operations": [],
      "placeholder": "5 days",
      "prefix": null,
      "read_only": false,
      "rich_text": false,
      "tags": [],
      "templates": [],
      "text": "qradar_search_param7",
      "tooltip": "Used for time to look back for graphql search",
      "type_id": 11,
      "uuid": "0b49c01e-c4b4-4d4d-869b-0eef6d08f258",
      "values": []
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "__function/qradar_search_param5",
      "hide_notification": false,
      "id": 2033,
      "input_type": "text",
      "internal": false,
      "is_tracked": false,
      "name": "qradar_search_param5",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": null,
      "read_only": false,
      "rich_text": false,
      "tags": [],
      "templates": [],
      "text": "qradar_search_param5",
      "tooltip": "",
      "type_id": 11,
      "uuid": "19778fc0-9a30-4b8b-9c21-642495f9eebc",
      "values": []
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "__function/soar_table_name",
      "hide_notification": false,
      "id": 2034,
      "input_type": "text",
      "internal": false,
      "is_tracked": false,
      "name": "soar_table_name",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": null,
      "read_only": false,
      "rich_text": false,
      "tags": [],
      "templates": [],
      "text": "soar_table_name",
      "tooltip": "Name of the data table that the workflow updates, so that it can be cleared if specified in the app.config",
      "type_id": 11,
      "uuid": "391ecb95-09ac-449e-a42d-b1684ef444fe",
      "values": []
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "__function/qradar_label",
      "hide_notification": false,
      "id": 2035,
      "input_type": "text",
      "internal": false,
      "is_tracked": false,
      "name": "qradar_label",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": null,
      "read_only": false,
      "rich_text": false,
      "tags": [],
      "templates": [],
      "text": "qradar_label",
      "tooltip": " Name of QRadar server to use from the app.config. If empty, the standard `[fn_qradar_integration]` server definition is used. See [1.2.0 Changes](#1.2.0-changes).",
      "type_id": 11,
      "uuid": "3ac8aa03-d83e-4fe1-91c1-bcc555a806b9",
      "values": []
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "__function/soar_incident_id",
      "hide_notification": false,
      "id": 2036,
      "input_type": "number",
      "internal": false,
      "is_tracked": false,
      "name": "soar_incident_id",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": null,
      "read_only": false,
      "rich_text": false,
      "tags": [],
      "templates": [],
      "text": "soar_incident_id",
      "tooltip": "ID of the SOAR incident the function is running in",
      "type_id": 11,
      "uuid": "7494d2ff-b4c2-4976-8076-e35224d26e89",
      "values": []
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "actioninvocation/select_to_create_artifact_from_flows_info",
      "hide_notification": false,
      "id": 2018,
      "input_type": "multiselect",
      "internal": false,
      "is_tracked": false,
      "name": "select_to_create_artifact_from_flows_info",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": "properties",
      "read_only": false,
      "required": "always",
      "rich_text": false,
      "tags": [],
      "templates": [],
      "text": "Select to create artifact from flows info",
      "tooltip": "",
      "type_id": 6,
      "uuid": "8aabde39-94fc-433c-9204-32fec7e84a8d",
      "values": [
        {
          "default": false,
          "enabled": true,
          "hidden": false,
          "label": "Source IP",
          "properties": null,
          "uuid": "80b2cb3e-43f8-4f5c-a921-db78b04b2fb2",
          "value": 312
        },
        {
          "default": false,
          "enabled": true,
          "hidden": false,
          "label": "Source Port",
          "properties": null,
          "uuid": "12b6be5c-847a-4ab3-9e50-3f9dd3bff126",
          "value": 313
        },
        {
          "default": false,
          "enabled": true,
          "hidden": false,
          "label": "Destination IP",
          "properties": null,
          "uuid": "0cee1802-e437-4138-b48e-12d357d98d9f",
          "value": 314
        },
        {
          "default": false,
          "enabled": true,
          "hidden": false,
          "label": "Destination Port",
          "properties": null,
          "uuid": "4ac9fe20-88fe-4c4c-b93c-b6071634c8ef",
          "value": 315
        }
      ]
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "actioninvocation/select_to_create_artifact",
      "hide_notification": false,
      "id": 2019,
      "input_type": "multiselect",
      "internal": false,
      "is_tracked": false,
      "name": "select_to_create_artifact",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": "properties",
      "read_only": false,
      "required": "always",
      "rich_text": false,
      "tags": [],
      "templates": [],
      "text": "Select to create artifact from events info",
      "tooltip": "",
      "type_id": 6,
      "uuid": "da26c9f2-924b-425f-a641-c87d37968180",
      "values": [
        {
          "default": false,
          "enabled": true,
          "hidden": false,
          "label": "Source IP",
          "properties": null,
          "uuid": "b7cef607-e4e0-482a-a4e1-dd353b045c7a",
          "value": 316
        },
        {
          "default": false,
          "enabled": true,
          "hidden": false,
          "label": "Destination IP",
          "properties": null,
          "uuid": "0ff0d4ff-ac43-4853-9670-5db505e7053b",
          "value": 317
        },
        {
          "default": false,
          "enabled": true,
          "hidden": false,
          "label": "Username",
          "properties": null,
          "uuid": "b0eb7729-5fde-43f2-ad7b-77acee1c0c5c",
          "value": 318
        }
      ]
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "actioninvocation/select_to_create_artifact_from_asset_info",
      "hide_notification": false,
      "id": 2020,
      "input_type": "multiselect",
      "internal": false,
      "is_tracked": false,
      "name": "select_to_create_artifact_from_asset_info",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": "properties",
      "read_only": false,
      "rich_text": false,
      "tags": [],
      "templates": [],
      "text": "Select to create artifact from asset info",
      "tooltip": "",
      "type_id": 6,
      "uuid": "09cb191e-152a-409c-80e2-dfdfb1490234",
      "values": [
        {
          "default": false,
          "enabled": true,
          "hidden": false,
          "label": "Name",
          "properties": null,
          "uuid": "222bd7e3-7492-4f88-ab70-f62eeb810e97",
          "value": 319
        },
        {
          "default": false,
          "enabled": true,
          "hidden": false,
          "label": "IP Address",
          "properties": null,
          "uuid": "51619aff-045b-4991-bbeb-e6dee7efd662",
          "value": 320
        }
      ]
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "actioninvocation/number_of_days_to_search",
      "hide_notification": false,
      "id": 2021,
      "input_type": "number",
      "internal": false,
      "is_tracked": false,
      "name": "number_of_days_to_search",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": "properties",
      "read_only": false,
      "rich_text": false,
      "tags": [],
      "templates": [],
      "text": "Number of Days to Search",
      "tooltip": "Set the number of says back to search for QRadar graphql query",
      "type_id": 6,
      "uuid": "271acf00-3223-48a5-b507-d0787854d365",
      "values": []
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "actioninvocation/select_to_create_artifact_from_sourceip",
      "hide_notification": false,
      "id": 2022,
      "input_type": "multiselect",
      "internal": false,
      "is_tracked": false,
      "name": "select_to_create_artifact_from_sourceip",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": "properties",
      "read_only": false,
      "rich_text": false,
      "tags": [],
      "templates": [],
      "text": "Select to create artifact from source info",
      "tooltip": "",
      "type_id": 6,
      "uuid": "39f165e9-9b2e-4f84-b302-b5ea190191d1",
      "values": [
        {
          "default": false,
          "enabled": true,
          "hidden": false,
          "label": "Source IP",
          "properties": null,
          "uuid": "644132a1-25bc-4f04-8fc0-488a7af8872d",
          "value": 321
        },
        {
          "default": false,
          "enabled": true,
          "hidden": false,
          "label": "MAC",
          "properties": null,
          "uuid": "35459b44-f439-4736-85c4-7f8052e3e732",
          "value": 322
        }
      ]
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "actioninvocation/select_to_create_artifact_from_destip",
      "hide_notification": false,
      "id": 2023,
      "input_type": "multiselect",
      "internal": false,
      "is_tracked": false,
      "name": "select_to_create_artifact_from_destip",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": "properties",
      "read_only": false,
      "rich_text": false,
      "tags": [],
      "templates": [],
      "text": "Select to create artifact from destination info",
      "tooltip": "",
      "type_id": 6,
      "uuid": "73ba3586-ef44-49ef-be00-28172fc9a62e",
      "values": [
        {
          "default": true,
          "enabled": true,
          "hidden": false,
          "label": "Destination IP",
          "properties": null,
          "uuid": "753758b7-c2b2-4e58-b68a-dcf9051b36df",
          "value": 323
        }
      ]
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "incident/qr_offense_status",
      "hide_notification": false,
      "id": 1935,
      "input_type": "text",
      "internal": false,
      "is_tracked": false,
      "name": "qr_offense_status",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": "properties",
      "read_only": false,
      "rich_text": false,
      "tags": [],
      "templates": [],
      "text": "QR Offense Status",
      "tooltip": "",
      "type_id": 0,
      "uuid": "8632d157-312f-49b7-9526-220077ef6131",
      "values": []
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "incident/qr_offense_last_updated_time",
      "hide_notification": false,
      "id": 1936,
      "input_type": "datetimepicker",
      "internal": false,
      "is_tracked": false,
      "name": "qr_offense_last_updated_time",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": "properties",
      "read_only": false,
      "rich_text": false,
      "tags": [],
      "templates": [],
      "text": "QR Offense Last Updated Time",
      "tooltip": "",
      "type_id": 0,
      "uuid": "8e612fca-fa1e-448d-9ecd-541ccb158573",
      "values": []
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "incident/qr_flow_count",
      "hide_notification": false,
      "id": 1937,
      "input_type": "textarea",
      "internal": false,
      "is_tracked": false,
      "name": "qr_flow_count",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": "properties",
      "read_only": false,
      "rich_text": true,
      "tags": [],
      "templates": [],
      "text": "QR Flow Count",
      "tooltip": "The no. of flows associated with the QRadar Offense",
      "type_id": 0,
      "uuid": "9bd3220a-33f8-4edc-b007-670cc3aaf59f",
      "values": []
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "incident/qradar_id",
      "hide_notification": false,
      "id": 1938,
      "input_type": "text",
      "internal": false,
      "is_tracked": false,
      "name": "qradar_id",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": "properties",
      "read_only": false,
      "rich_text": false,
      "tags": [],
      "templates": [],
      "text": "QR Offense Id",
      "tooltip": "",
      "type_id": 0,
      "uuid": "aedb7df6-642a-4438-824d-fe24be34cfc0",
      "values": []
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "incident/qr_offense_index_type",
      "hide_notification": false,
      "id": 1939,
      "input_type": "text",
      "internal": false,
      "is_tracked": false,
      "name": "qr_offense_index_type",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": "properties",
      "read_only": false,
      "rich_text": false,
      "tags": [],
      "templates": [],
      "text": "QR Offense Index Type",
      "tooltip": "The type on which the QRadar Offense is indexed",
      "type_id": 0,
      "uuid": "c34fdc0c-b8e4-4ac8-b149-20f0f30cba01",
      "values": []
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "incident/qr_assigned",
      "hide_notification": false,
      "id": 1940,
      "input_type": "textarea",
      "internal": false,
      "is_tracked": false,
      "name": "qr_assigned",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": "properties",
      "read_only": false,
      "rich_text": true,
      "tags": [],
      "templates": [],
      "text": "QR Assigned",
      "tooltip": "The analyst to whom the QRadar Offense is assigned to.",
      "type_id": 0,
      "uuid": "cee826b5-5bab-44ba-a177-3458d25199aa",
      "values": []
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "incident/qr_source_ip_count",
      "hide_notification": false,
      "id": 1941,
      "input_type": "textarea",
      "internal": false,
      "is_tracked": false,
      "name": "qr_source_ip_count",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": "properties",
      "read_only": false,
      "rich_text": true,
      "tags": [],
      "templates": [],
      "text": "QR Source IP Count",
      "tooltip": "The no. of Source IPs associated with the QRadar Offense",
      "type_id": 0,
      "uuid": "d9536f5a-3692-466b-b4c8-1045799d19ba",
      "values": []
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "incident/qr_credibility",
      "hide_notification": false,
      "id": 1942,
      "input_type": "textarea",
      "internal": false,
      "is_tracked": false,
      "name": "qr_credibility",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": "properties",
      "read_only": false,
      "rich_text": true,
      "tags": [],
      "templates": [],
      "text": "QR Credibility",
      "tooltip": "Indicates the integrity of the offense as determined by the credibility rating that is configured in the log source.",
      "type_id": 0,
      "uuid": "ea93f610-b018-413e-8ffd-24a710172490",
      "values": []
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "incident/qr_relevance",
      "hide_notification": false,
      "id": 1943,
      "input_type": "textarea",
      "internal": false,
      "is_tracked": false,
      "name": "qr_relevance",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": "properties",
      "read_only": false,
      "rich_text": true,
      "tags": [],
      "templates": [],
      "text": "QR Relevance",
      "tooltip": "Indicates the importance of the destination. QRadar determines the relevance by the weight that the administrator assigned to the networks and assets.",
      "type_id": 0,
      "uuid": "ef2b94ed-84b9-4388-9e7b-7cafedf6ea4e",
      "values": []
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "incident/qr_offense_index_value",
      "hide_notification": false,
      "id": 1944,
      "input_type": "text",
      "internal": false,
      "is_tracked": false,
      "name": "qr_offense_index_value",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": "properties",
      "read_only": false,
      "rich_text": false,
      "tags": [],
      "templates": [],
      "text": "QR Offense Index Value",
      "tooltip": "The value by which QRadar Offense is indexed",
      "type_id": 0,
      "uuid": "f4354ca9-d038-45a5-a91f-b4beaf3f5f18",
      "values": []
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "incident/qr_offense_start_time",
      "hide_notification": false,
      "id": 1945,
      "input_type": "datetimepicker",
      "internal": false,
      "is_tracked": false,
      "name": "qr_offense_start_time",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": "properties",
      "read_only": false,
      "rich_text": false,
      "tags": [],
      "templates": [],
      "text": "QR Offense Start Time",
      "tooltip": "",
      "type_id": 0,
      "uuid": "11e94c0e-62d5-4ef9-8b5d-7073f487b606",
      "values": []
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "incident/qr_severity",
      "hide_notification": false,
      "id": 1946,
      "input_type": "textarea",
      "internal": false,
      "is_tracked": false,
      "name": "qr_severity",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": "properties",
      "read_only": false,
      "rich_text": true,
      "tags": [],
      "templates": [],
      "text": "QR Severity",
      "tooltip": "Indicates the threat that an attack poses in relation to how prepared the destination is for the attack.",
      "type_id": 0,
      "uuid": "3659f126-c8df-47cb-bcdb-4dcbab3dbd84",
      "values": []
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "incident/qr_last_updated_time",
      "hide_notification": false,
      "id": 1947,
      "input_type": "datetimepicker",
      "internal": false,
      "is_tracked": false,
      "name": "qr_last_updated_time",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": "properties",
      "read_only": false,
      "rich_text": false,
      "tags": [],
      "templates": [],
      "text": "QR Incident Last Updated Time",
      "tooltip": "",
      "type_id": 0,
      "uuid": "39b1941a-8e62-4c19-a595-c7ab4ec4ec5f",
      "values": []
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "incident/qradar_destination",
      "hide_notification": false,
      "id": 1948,
      "input_type": "text",
      "internal": false,
      "is_tracked": false,
      "name": "qradar_destination",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": "properties",
      "read_only": false,
      "rich_text": false,
      "tags": [],
      "templates": [],
      "text": "QRadar Destination",
      "tooltip": "QRadar Destination to Sync With",
      "type_id": 0,
      "uuid": "3b119adb-7585-44b6-b578-7c5d5ee6958d",
      "values": []
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "incident/qr_event_count",
      "hide_notification": false,
      "id": 1949,
      "input_type": "textarea",
      "internal": false,
      "is_tracked": false,
      "name": "qr_event_count",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": "properties",
      "read_only": false,
      "rich_text": true,
      "tags": [],
      "templates": [],
      "text": "QR Event Count",
      "tooltip": "The no. of events associated with the QRadar Offense",
      "type_id": 0,
      "uuid": "4f7dfb5c-f38d-471b-b7a7-203e5d5790d9",
      "values": []
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "incident/qr_offense_domain",
      "hide_notification": false,
      "id": 1950,
      "input_type": "text",
      "internal": false,
      "is_tracked": false,
      "name": "qr_offense_domain",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": "properties",
      "read_only": false,
      "rich_text": false,
      "tags": [],
      "templates": [],
      "text": "QR Offense Domain",
      "tooltip": "",
      "type_id": 0,
      "uuid": "568fa360-983f-463d-af2d-01bd8c7a0e25",
      "values": []
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "incident/qr_destination_ip_count",
      "hide_notification": false,
      "id": 1951,
      "input_type": "textarea",
      "internal": false,
      "is_tracked": false,
      "name": "qr_destination_ip_count",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": "properties",
      "read_only": false,
      "rich_text": true,
      "tags": [],
      "templates": [],
      "text": "QR Destination IP Count",
      "tooltip": "The no. of Destination IPs associated with the QRadar Offense",
      "type_id": 0,
      "uuid": "5be7494c-2776-4df1-8d20-fa994ab11d93",
      "values": []
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "incident/qr_offense_source",
      "hide_notification": false,
      "id": 1952,
      "input_type": "text",
      "internal": false,
      "is_tracked": false,
      "name": "qr_offense_source",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": "properties",
      "read_only": false,
      "rich_text": false,
      "tags": [],
      "templates": [],
      "text": "QR Offense Source ",
      "tooltip": "The source for the QRadar Offense",
      "type_id": 0,
      "uuid": "6af22a10-7581-4faf-afc8-c46fb68057f1",
      "values": []
    },
    {
      "allow_default_value": false,
      "blank_option": false,
      "calculated": false,
      "changeable": true,
      "chosen": false,
      "default_chosen_by_server": false,
      "deprecated": false,
      "export_key": "incident/qr_magnitude",
      "hide_notification": false,
      "id": 1953,
      "input_type": "textarea",
      "internal": false,
      "is_tracked": false,
      "name": "qr_magnitude",
      "operation_perms": {},
      "operations": [],
      "placeholder": "",
      "prefix": "properties",
      "read_only": false,
      "rich_text": true,
      "tags": [],
      "templates": [],
      "text": "QR Magnitude",
      "tooltip": "Indicates the relative importance of the offense. This value is calculated based on the relevance, severity, and credibility ratings.",
      "type_id": 0,
      "uuid": "73b97697-3400-4bcc-be42-52020a0362a2",
      "values": []
    },
    {
      "export_key": "incident/internal_customizations_field",
      "id": 0,
      "input_type": "text",
      "internal": true,
      "name": "internal_customizations_field",
      "read_only": true,
      "text": "Customizations Field (internal)",
      "type_id": 0,
      "uuid": "bfeec2d4-3770-11e8-ad39-4a0004044aa1"
    }
  ],
  "functions": [
    {
      "created_date": 1666018029621,
      "description": {
        "content": "Get the MITRE Tactics and Techniques in relation to the rules that were fired to cause the offense in QRadar.",
        "format": "text"
      },
      "destination_handle": "fn_qradar_enhanced_data",
      "display_name": "QRadar Get Offense MITRE Reference",
      "export_key": "qradar_get_offense_mitre_reference",
      "id": 41,
      "last_modified_by": {
        "display_name": "Resilient Sysadmin",
        "id": 21,
        "name": "d@example.com",
        "type": "user"
      },
      "last_modified_time": 1666020026559,
      "name": "qradar_get_offense_mitre_reference",
      "tags": [],
      "uuid": "c72d73d8-29de-4166-875e-427e3add0859",
      "version": 4,
      "view_items": [
        {
          "content": "c6cac6bb-9718-40e9-8fb6-2cea26be7b5b",
          "element": "field_uuid",
          "field_type": "__function",
          "show_if": null,
          "show_link_header": false,
          "step_label": null
        },
        {
          "content": "3ac8aa03-d83e-4fe1-91c1-bcc555a806b9",
          "element": "field_uuid",
          "field_type": "__function",
          "show_if": null,
          "show_link_header": false,
          "step_label": null
        },
        {
          "content": "391ecb95-09ac-449e-a42d-b1684ef444fe",
          "element": "field_uuid",
          "field_type": "__function",
          "show_if": null,
          "show_link_header": false,
          "step_label": null
        },
        {
          "content": "7494d2ff-b4c2-4976-8076-e35224d26e89",
          "element": "field_uuid",
          "field_type": "__function",
          "show_if": null,
          "show_link_header": false,
          "step_label": null
        }
      ],
      "workflows": [
        {
          "actions": [],
          "description": null,
          "name": "Example: QRadar - Get MITRE Reference From Rules",
          "object_type": "incident",
          "programmatic_name": "example_qradar_get_mitre_reference_from_rules",
          "tags": [],
          "uuid": null,
          "workflow_id": 64
        }
      ]
    },
    {
      "created_date": 1666017864826,
      "description": {
        "content": "Fetch QRadar Offense Details",
        "format": "text"
      },
      "destination_handle": "fn_qradar_enhanced_data",
      "display_name": "QRadar Offense Summary",
      "export_key": "qradar_offense_summary",
      "id": 39,
      "last_modified_by": {
        "display_name": "Resilient Sysadmin",
        "id": 21,
        "name": "d@example.com",
        "type": "user"
      },
      "last_modified_time": 1666017864876,
      "name": "qradar_offense_summary",
      "tags": [],
      "uuid": "0aee36de-8719-4154-a1e7-6455f065bb94",
      "version": 1,
      "view_items": [
        {
          "content": "c6cac6bb-9718-40e9-8fb6-2cea26be7b5b",
          "element": "field_uuid",
          "field_type": "__function",
          "show_if": null,
          "show_link_header": false,
          "step_label": null
        },
        {
          "content": "035f31c7-3bf8-4b5e-932f-4adb32198572",
          "element": "field_uuid",
          "field_type": "__function",
          "show_if": null,
          "show_link_header": false,
          "step_label": null
        },
        {
          "content": "3ac8aa03-d83e-4fe1-91c1-bcc555a806b9",
          "element": "field_uuid",
          "field_type": "__function",
          "show_if": null,
          "show_link_header": false,
          "step_label": null
        },
        {
          "content": "391ecb95-09ac-449e-a42d-b1684ef444fe",
          "element": "field_uuid",
          "field_type": "__function",
          "show_if": null,
          "show_link_header": false,
          "step_label": null
        },
        {
          "content": "7494d2ff-b4c2-4976-8076-e35224d26e89",
          "element": "field_uuid",
          "field_type": "__function",
          "show_if": null,
          "show_link_header": false,
          "step_label": null
        }
      ],
      "workflows": [
        {
          "actions": [],
          "description": null,
          "name": "Example of fetching contributing rules for Offense",
          "object_type": "incident",
          "programmatic_name": "qradar_triggered_rules",
          "tags": [],
          "uuid": null,
          "workflow_id": 61
        },
        {
          "actions": [],
          "description": null,
          "name": "Example of searching and returning Assets information",
          "object_type": "incident",
          "programmatic_name": "qradar_assets_information",
          "tags": [],
          "uuid": null,
          "workflow_id": 62
        },
        {
          "actions": [],
          "description": null,
          "name": "QRadar Offense Summary",
          "object_type": "incident",
          "programmatic_name": "qradar_offense_summary",
          "tags": [],
          "uuid": null,
          "workflow_id": 60
        }
      ]
    },
    {
      "created_date": 1666017864909,
      "description": {
        "content": "Search QRadar Top events for the given Offense ID",
        "format": "text"
      },
      "destination_handle": "fn_qradar_enhanced_data",
      "display_name": "QRadar Top Events",
      "export_key": "qradar_top_events",
      "id": 40,
      "last_modified_by": {
        "display_name": "Resilient Sysadmin",
        "id": 21,
        "name": "d@example.com",
        "type": "user"
      },
      "last_modified_time": 1666017864956,
      "name": "qradar_top_events",
      "tags": [],
      "uuid": "d8e63812-0b58-4ca1-991a-49255f3dd04e",
      "version": 1,
      "view_items": [
        {
          "content": "035f31c7-3bf8-4b5e-932f-4adb32198572",
          "element": "field_uuid",
          "field_type": "__function",
          "show_if": null,
          "show_link_header": false,
          "step_label": null
        },
        {
          "content": "048ba39a-ab94-4d1f-a0f8-2462de3c044c",
          "element": "field_uuid",
          "field_type": "__function",
          "show_if": null,
          "show_link_header": false,
          "step_label": null
        },
        {
          "content": "01ed4652-fe61-4dbf-bc83-f81c62c5563c",
          "element": "field_uuid",
          "field_type": "__function",
          "show_if": null,
          "show_link_header": false,
          "step_label": null
        },
        {
          "content": "9d8fc645-5067-4ab0-b151-1d106dad91d6",
          "element": "field_uuid",
          "field_type": "__function",
          "show_if": null,
          "show_link_header": false,
          "step_label": null
        },
        {
          "content": "b6053690-d7b2-4204-b48a-f86503b419d9",
          "element": "field_uuid",
          "field_type": "__function",
          "show_if": null,
          "show_link_header": false,
          "step_label": null
        },
        {
          "content": "eefc9e56-2806-4534-b409-42e25154ecc9",
          "element": "field_uuid",
          "field_type": "__function",
          "show_if": null,
          "show_link_header": false,
          "step_label": null
        },
        {
          "content": "19778fc0-9a30-4b8b-9c21-642495f9eebc",
          "element": "field_uuid",
          "field_type": "__function",
          "show_if": null,
          "show_link_header": false,
          "step_label": null
        },
        {
          "content": "ca2d7d7f-fe6d-4a21-b011-7953494c350c",
          "element": "field_uuid",
          "field_type": "__function",
          "show_if": null,
          "show_link_header": false,
          "step_label": null
        },
        {
          "content": "0b49c01e-c4b4-4d4d-869b-0eef6d08f258",
          "element": "field_uuid",
          "field_type": "__function",
          "show_if": null,
          "show_link_header": false,
          "step_label": null
        },
        {
          "content": "3ac8aa03-d83e-4fe1-91c1-bcc555a806b9",
          "element": "field_uuid",
          "field_type": "__function",
          "show_if": null,
          "show_link_header": false,
          "step_label": null
        },
        {
          "content": "7494d2ff-b4c2-4976-8076-e35224d26e89",
          "element": "field_uuid",
          "field_type": "__function",
          "show_if": null,
          "show_link_header": false,
          "step_label": null
        },
        {
          "content": "391ecb95-09ac-449e-a42d-b1684ef444fe",
          "element": "field_uuid",
          "field_type": "__function",
          "show_if": null,
          "show_link_header": false,
          "step_label": null
        }
      ],
      "workflows": [
        {
          "actions": [],
          "description": null,
          "name": "Example of searching and returning Categories",
          "object_type": "incident",
          "programmatic_name": "qradar_categories",
          "tags": [],
          "uuid": null,
          "workflow_id": 57
        },
        {
          "actions": [],
          "description": null,
          "name": "Example of searching and returning Destination IPs information",
          "object_type": "incident",
          "programmatic_name": "qradar_destination_ips",
          "tags": [],
          "uuid": null,
          "workflow_id": 59
        },
        {
          "actions": [],
          "description": null,
          "name": "Example of searching and returning Source IPs information",
          "object_type": "incident",
          "programmatic_name": "qradar_source_ips",
          "tags": [],
          "uuid": null,
          "workflow_id": 63
        },
        {
          "actions": [],
          "description": null,
          "name": "Example of searching QRadar Flows using offense id",
          "object_type": "incident",
          "programmatic_name": "example_of_searching_qradar_flows_using_offense_id",
          "tags": [],
          "uuid": null,
          "workflow_id": 56
        },
        {
          "actions": [],
          "description": null,
          "name": "Example of searching QRadar Top Events using offense id",
          "object_type": "incident",
          "programmatic_name": "example_of_searching_qradar_top_events_using_offense_id",
          "tags": [],
          "uuid": null,
          "workflow_id": 58
        }
      ]
    }
  ],
  "geos": null,
  "groups": null,
  "id": 20,
  "inbound_destinations": [],
  "inbound_mailboxes": null,
  "incident_artifact_types": [],
  "incident_types": [
    {
      "create_date": 1666356667805,
      "description": "Customization Packages (internal)",
      "enabled": false,
      "export_key": "Customization Packages (internal)",
      "hidden": false,
      "id": 0,
      "name": "Customization Packages (internal)",
      "parent_id": null,
      "system": false,
      "update_date": 1666356667805,
      "uuid": "bfeec2d4-3770-11e8-ad39-4a0004044aa0"
    }
  ],
  "industries": null,
  "layouts": [],
  "locale": null,
  "message_destinations": [
    {
      "api_keys": [],
      "destination_type": 0,
      "expect_ack": true,
      "export_key": "fn_qradar_enhanced_data",
      "name": "fn_qradar_enhanced_data",
      "programmatic_name": "fn_qradar_enhanced_data",
      "tags": [],
      "users": [
        "d@example.com"
      ],
      "uuid": "2aafea6c-1710-4b20-98a7-3b495b210d44"
    }
  ],
  "notifications": null,
  "overrides": [],
  "phases": [],
  "playbooks": [],
  "regulators": null,
  "roles": [],
  "scripts": [
    {
      "actions": [],
      "created_date": 1666017863593,
      "description": "Create artifact from Assets info for the selected row",
      "enabled": false,
      "export_key": "Create Artifact from Assets info",
      "id": 15,
      "language": "python",
      "last_modified_by": "d@example.com",
      "last_modified_time": 1666017863617,
      "name": "Create Artifact from Assets info",
      "object_type": "qr_assets",
      "playbook_handle": null,
      "programmatic_name": "create_artifact_from_assets_info",
      "script_text": "#\n# We create artifacts according to how they can be mapped to\n# SOAR default artifacts. If user has custom artifacts, and wants\n# to map them as well, please modify the following mapping dict.\n#\n\ntype_mapping = {\n    \"IP Address\": \"IP Address\",\n    \"Name\": \"String\",\n}\n\nimport re\n\nartifact_types = rule.properties.select_to_create_artifact_from_asset_info\n\nfor type in artifact_types:\n  if type in type_mapping:\n    artifact_description = \"QRadar Offense {0}\".format(type)\n    if type==\"IP Address\":\n     incident.addArtifact(type_mapping[type], row.ip_address[\"content\"], artifact_description)\n    elif type==\"Name\":\n       incident.addArtifact(type_mapping[type], row.asset_name[\"content\"], artifact_description)\n",
      "tags": [],
      "uuid": "56272bd6-1c9c-4043-88aa-0b7f1aac382c"
    },
    {
      "actions": [],
      "created_date": 1666017863678,
      "description": "Create artifact from Destination IP info for the selected row",
      "enabled": false,
      "export_key": "Create Artifact from Destination IP info",
      "id": 16,
      "language": "python",
      "last_modified_by": "d@example.com",
      "last_modified_time": 1666017863704,
      "name": "Create Artifact from Destination IP info",
      "object_type": "qr_top_destination_ips",
      "playbook_handle": null,
      "programmatic_name": "create_artifact_from_destination_ip_info",
      "script_text": "#\n# We create artifacts for those observables according to how they can be mapped to\n# SOAR default artifacts. If user has custom artifacts, and wants\n# to map them as well, please modify the following mapping dict.\n#\n\ntype_mapping = {\n    \"Destination IP\": \"IP Address\",\n}\n\nimport re\n\nartifact_types = rule.properties.select_to_create_artifact_from_destip\n\nfor type in artifact_types:\n  if type in type_mapping:\n    artifact_description = \"QRadar Offense {0}\".format(type)\n    if type==\"Destination IP\":\n      incident.addArtifact(type_mapping[type], re.sub(\"\u003c[^\u003c\u003e]+\u003e\",\"\",row.destination_ip[\"content\"]), artifact_description)\n",
      "tags": [],
      "uuid": "b6f06359-9d13-4244-bd0f-123dc4068317"
    },
    {
      "actions": [],
      "created_date": 1666017863756,
      "description": "Create artifact from the Events info of the selected row",
      "enabled": false,
      "export_key": "Create Artifact from Events info",
      "id": 17,
      "language": "python",
      "last_modified_by": "d@example.com",
      "last_modified_time": 1666017863781,
      "name": "Create Artifact from Events info",
      "object_type": "qr_offense_top_events",
      "playbook_handle": null,
      "programmatic_name": "create_artifact_from_events_info",
      "script_text": "#\n# We create artifacts for those observables according to how they can be mapped to\n# SOAR default artifacts. If user has custom artifacts, and wants\n# to map them as well, please modify the following mapping dict.\n#\n\ntype_mapping = {\n    \"Source IP\": \"IP Address\",\n    \"Destination IP\": \"IP Address\",\n    \"Username\": \"User Account\"\n}\n\nimport re\n\nartifact_types = rule.properties.select_to_create_artifact\n\nfor type in artifact_types:\n  if type in type_mapping:\n    artifact_description = \"QRadar Offense {0}\".format(type)\n    if type==\"Source IP\":\n      incident.addArtifact(type_mapping[type],re.sub(\"\u003c[^\u003c\u003e]+\u003e\",\"\",row.source_ip[\"content\"]), artifact_description)\n    elif type==\"Destination IP\":\n      incident.addArtifact(type_mapping[type], re.sub(\"\u003c[^\u003c\u003e]+\u003e\",\"\",row.destination_ip[\"content\"]), artifact_description)\n    elif type==\"Username\":\n      incident.addArtifact(type_mapping[type], row.username, artifact_description)\n",
      "tags": [],
      "uuid": "c4c431c3-d175-4f68-b401-1b6fc6d69f1a"
    },
    {
      "actions": [],
      "created_date": 1666017863831,
      "description": "Create artifact from the Flows info of the selected row",
      "enabled": false,
      "export_key": "Create Artifact from Flows info",
      "id": 18,
      "language": "python",
      "last_modified_by": "d@example.com",
      "last_modified_time": 1666017863856,
      "name": "Create Artifact from Flows info",
      "object_type": "qr_flows",
      "playbook_handle": null,
      "programmatic_name": "create_artifact_from_flows_info",
      "script_text": "#\n# We create artifacts for those observables according to how they can be mapped to\n# SOAR default artifacts. If user has custom artifacts, and wants\n# to map them as well, please modify the following mapping dict.\n#\n\ntype_mapping = {\n    \"Source IP\": \"IP Address\",\n    \"Destination IP\": \"IP Address\",\n    \"Source Port\": \"Port\",\n    \"Destination Port\": \"Port\"\n}\n\nimport re\n\nartifact_types = rule.properties.select_to_create_artifact_from_flows_info\n\nfor type in artifact_types:\n  if type in type_mapping:\n    artifact_description = \"QRadar Offense {0}\".format(type)\n    if type==\"Source IP\":\n      incident.addArtifact(type_mapping[type],row.source_ip[\"content\"], artifact_description)\n    elif type==\"Destination IP\":\n      incident.addArtifact(type_mapping[type],row.destination_ip[\"content\"], artifact_description)\n    elif type==\"Source Port\":\n      incident.addArtifact(type_mapping[type],row.source_ip[\"content\"], artifact_description)\n    elif type==\"Destination Port\":\n      incident.addArtifact(type_mapping[type],row.destination_ip[\"content\"], artifact_description)\n",
      "tags": [],
      "uuid": "027a462d-ee0e-4e13-b7e6-fa8033888747"
    },
    {
      "actions": [],
      "created_date": 1666017863909,
      "description": "Create artifact from Source IP info for the selected row",
      "enabled": false,
      "export_key": "Create Artifact from Source IP info",
      "id": 19,
      "language": "python",
      "last_modified_by": "d@example.com",
      "last_modified_time": 1666017863935,
      "name": "Create Artifact from Source IP info",
      "object_type": "qr_top_source_ips",
      "playbook_handle": null,
      "programmatic_name": "create_artifact_from_source_ip_info",
      "script_text": "#\n# We create artifacts for those observables according to how they can be mapped to\n# SOAR default artifacts. If user has custom artifacts, and wants\n# to map them as well, please modify the following mapping dict.\n#\n\ntype_mapping = {\n    \"Source IP\": \"IP Address\",\n    \"MAC\": \"MAC Address\",\n}\n\nimport re\n\nartifact_types = rule.properties.select_to_create_artifact_from_sourceip\n\nfor type in artifact_types:\n  if type in type_mapping:\n    artifact_description = \"QRadar Offense {0}\".format(type)\n    if type==\"Source IP\":\n      incident.addArtifact(type_mapping[type],re.sub(\"\u003c[^\u003c\u003e]+\u003e\",\"\",row.source_ip[\"content\"]), artifact_description)\n    elif type==\"MAC\":\n      incident.addArtifact(type_mapping[type], row.mac, artifact_description)\n",
      "tags": [],
      "uuid": "9a02cf5d-a5ac-42f1-a421-b5a642e4feb7"
    },
    {
      "actions": [],
      "created_date": 1666017863988,
      "description": "qr_last_updated_time will be set to equal create_date for the incident on incident creation. qr_last_updated_time will be set to equal current time when manual refresh rule is run.",
      "enabled": false,
      "export_key": "Set Incident Last Updated Time",
      "id": 20,
      "language": "python",
      "last_modified_by": "d@example.com",
      "last_modified_time": 1666017864014,
      "name": "Set Incident Last Updated Time",
      "object_type": "incident",
      "playbook_handle": null,
      "programmatic_name": "get_current_time",
      "script_text": "import java.util.Date as Date\n\n# If qr_last_updated_time is empty (This will be true on incident creation)\nif not incident.properties.qr_last_updated_time:\n  incident.properties.qr_last_updated_time = incident.create_date\n# If qr_last_updated_time is not empty (This will be true when Manual refresh rule is run)\nelse:\n  incident.properties.qr_last_updated_time = Date()",
      "tags": [],
      "uuid": "e93008f7-1249-46a8-8d7e-75c0dc862f4f"
    }
  ],
  "server_version": {
    "build_number": 7585,
    "major": 44,
    "minor": 0,
    "version": "44.0.7585"
  },
  "tags": [],
  "task_order": [],
  "timeframes": null,
  "types": [
    {
      "actions": [],
      "display_name": "QRadar Rules and MITRE Tactics and Techniques",
      "export_key": "qradar_rules_and_mitre_tactics_and_techniques",
      "fields": {
        "mitre_tactic": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qradar_rules_and_mitre_tactics_and_techniques/mitre_tactic",
          "hide_notification": false,
          "id": 2037,
          "input_type": "text",
          "internal": false,
          "is_tracked": false,
          "name": "mitre_tactic",
          "operation_perms": {},
          "operations": [],
          "order": 3,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "short_text": "",
          "tags": [],
          "templates": [],
          "text": "MITRE Tactic",
          "tooltip": "",
          "type_id": 1025,
          "uuid": "0cde50a2-cec3-4686-b931-db09c5cc9c27",
          "values": [],
          "width": 50
        },
        "mitre_tactic_id": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qradar_rules_and_mitre_tactics_and_techniques/mitre_tactic_id",
          "hide_notification": false,
          "id": 2038,
          "input_type": "text",
          "internal": false,
          "is_tracked": false,
          "name": "mitre_tactic_id",
          "operation_perms": {},
          "operations": [],
          "order": 4,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "short_text": "",
          "tags": [],
          "templates": [],
          "text": "MITRE Tactic ID",
          "tooltip": "",
          "type_id": 1025,
          "uuid": "76455b2d-2229-4861-b69b-610548615025",
          "values": [],
          "width": 50
        },
        "mitre_technique": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qradar_rules_and_mitre_tactics_and_techniques/mitre_technique",
          "hide_notification": false,
          "id": 2039,
          "input_type": "text",
          "internal": false,
          "is_tracked": false,
          "name": "mitre_technique",
          "operation_perms": {},
          "operations": [],
          "order": 6,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "short_text": "",
          "tags": [],
          "templates": [],
          "text": "MITRE Technique",
          "tooltip": "",
          "type_id": 1025,
          "uuid": "249003bb-ce9b-4811-9c82-38b8fa00a8b6",
          "values": [],
          "width": 78
        },
        "mitre_technique_id": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qradar_rules_and_mitre_tactics_and_techniques/mitre_technique_id",
          "hide_notification": false,
          "id": 2040,
          "input_type": "text",
          "internal": false,
          "is_tracked": false,
          "name": "mitre_technique_id",
          "operation_perms": {},
          "operations": [],
          "order": 7,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "short_text": "",
          "tags": [],
          "templates": [],
          "text": "MITRE Technique ID",
          "tooltip": "",
          "type_id": 1025,
          "uuid": "71386a2f-010b-497b-854b-c4f6a1a701b9",
          "values": [],
          "width": 78
        },
        "rule_id": {
          "allow_default_value": false,
          "blank_option": true,
          "calculated": false,
          "changeable": true,
          "chosen": true,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qradar_rules_and_mitre_tactics_and_techniques/rule_id",
          "hide_notification": false,
          "id": 2041,
          "input_type": "text",
          "internal": false,
          "is_tracked": false,
          "name": "rule_id",
          "operation_perms": {},
          "operations": [],
          "order": 0,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "short_text": "",
          "tags": [],
          "templates": [],
          "text": "Rule ID",
          "tooltip": "",
          "type_id": 1025,
          "uuid": "853fde37-417b-434a-9351-faf84f1665bf",
          "values": [],
          "width": 34
        },
        "rule_identifier": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qradar_rules_and_mitre_tactics_and_techniques/rule_identifier",
          "hide_notification": false,
          "id": 2042,
          "input_type": "text",
          "internal": false,
          "is_tracked": false,
          "name": "rule_identifier",
          "operation_perms": {},
          "operations": [],
          "order": 1,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "short_text": "",
          "tags": [],
          "templates": [],
          "text": "Rule Identifier",
          "tooltip": "",
          "type_id": 1025,
          "uuid": "182b592a-c169-49e7-9d43-d6aa215f479a",
          "values": [],
          "width": 73
        },
        "rule_name": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qradar_rules_and_mitre_tactics_and_techniques/rule_name",
          "hide_notification": false,
          "id": 2043,
          "input_type": "text",
          "internal": false,
          "is_tracked": false,
          "name": "rule_name",
          "operation_perms": {},
          "operations": [],
          "order": 2,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "short_text": "",
          "tags": [],
          "templates": [],
          "text": "Rule Name",
          "tooltip": "",
          "type_id": 1025,
          "uuid": "46247059-8d98-4ed5-974d-6a39e50fe14b",
          "values": [],
          "width": 44
        },
        "tactic_confidence_level": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qradar_rules_and_mitre_tactics_and_techniques/tactic_confidence_level",
          "hide_notification": false,
          "id": 2044,
          "input_type": "text",
          "internal": false,
          "is_tracked": false,
          "name": "tactic_confidence_level",
          "operation_perms": {},
          "operations": [],
          "order": 5,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "short_text": "",
          "tags": [],
          "templates": [],
          "text": "Tactic Confidence Level",
          "tooltip": "",
          "type_id": 1025,
          "uuid": "69932faf-d86d-4b87-afc5-01008b09b417",
          "values": [],
          "width": 86
        },
        "technique_confidence_level": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qradar_rules_and_mitre_tactics_and_techniques/technique_confidence_level",
          "hide_notification": false,
          "id": 2045,
          "input_type": "text",
          "internal": false,
          "is_tracked": false,
          "name": "technique_confidence_level",
          "operation_perms": {},
          "operations": [],
          "order": 8,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "short_text": "",
          "tags": [],
          "templates": [],
          "text": "Technique Confidence Level",
          "tooltip": "",
          "type_id": 1025,
          "uuid": "a1c065c6-3cab-4580-9079-a40368e2f952",
          "values": [],
          "width": 86
        }
      },
      "for_actions": false,
      "for_custom_fields": false,
      "for_notifications": false,
      "for_workflows": false,
      "id": null,
      "parent_types": [
        "incident"
      ],
      "properties": {
        "can_create": false,
        "can_destroy": false,
        "for_who": []
      },
      "scripts": [],
      "tags": [],
      "type_id": 8,
      "type_name": "qradar_rules_and_mitre_tactics_and_techniques",
      "uuid": "b577bb11-ec32-4977-8fd7-5dd724fad3c5"
    },
    {
      "actions": [],
      "display_name": "QR Assets",
      "export_key": "qr_assets",
      "fields": {
        "aggregated_cvss": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_assets/aggregated_cvss",
          "hide_notification": false,
          "id": 1955,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "aggregated_cvss",
          "operation_perms": {},
          "operations": [],
          "order": 5,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "Aggregated CVSS",
          "tooltip": "",
          "type_id": 1018,
          "uuid": "be15635a-30b4-4691-bc27-e329350abc30",
          "values": [],
          "width": 126
        },
        "asset_id": {
          "allow_default_value": false,
          "blank_option": true,
          "calculated": false,
          "changeable": true,
          "chosen": true,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_assets/asset_id",
          "hide_notification": false,
          "id": 1956,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "asset_id",
          "operation_perms": {},
          "operations": [],
          "order": 1,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "ID",
          "tooltip": "",
          "type_id": 1018,
          "uuid": "70698b55-f7b6-4f60-94a2-1498a289d946",
          "values": [],
          "width": 45
        },
        "asset_name": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_assets/asset_name",
          "hide_notification": false,
          "id": 1957,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "asset_name",
          "operation_perms": {},
          "operations": [],
          "order": 2,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "Name",
          "tooltip": "",
          "type_id": 1018,
          "uuid": "b8fe29dc-e586-41c8-8e94-b37b79039c9a",
          "values": [],
          "width": 79
        },
        "ip_address": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_assets/ip_address",
          "hide_notification": false,
          "id": 1958,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "ip_address",
          "operation_perms": {},
          "operations": [],
          "order": 3,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "IP Address",
          "tooltip": "",
          "type_id": 1018,
          "uuid": "0b010be5-cc51-45c0-b9c0-35ede0fc3824",
          "values": [],
          "width": 86
        },
        "last_user": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_assets/last_user",
          "hide_notification": false,
          "id": 1959,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "last_user",
          "operation_perms": {},
          "operations": [],
          "order": 7,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "Last User",
          "tooltip": "",
          "type_id": 1018,
          "uuid": "9847f9a2-6558-4317-b587-7e66237fe75e",
          "values": [],
          "width": 76
        },
        "last_user_seen": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_assets/last_user_seen",
          "hide_notification": false,
          "id": 1960,
          "input_type": "datetimepicker",
          "internal": false,
          "is_tracked": false,
          "name": "last_user_seen",
          "operation_perms": {},
          "operations": [],
          "order": 8,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "tags": [],
          "templates": [],
          "text": "Last User Seen",
          "tooltip": "",
          "type_id": 1018,
          "uuid": "145809c5-d9b0-416c-9e96-a165d1166e05",
          "values": [],
          "width": 38
        },
        "operating_system": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_assets/operating_system",
          "hide_notification": false,
          "id": 1961,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "operating_system",
          "operation_perms": {},
          "operations": [],
          "order": 4,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "OS ID",
          "tooltip": "",
          "type_id": 1018,
          "uuid": "917685ae-7b50-4644-a874-eda876771767",
          "values": [],
          "width": 134
        },
        "reported_time": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_assets/reported_time",
          "hide_notification": false,
          "id": 1962,
          "input_type": "datetimepicker",
          "internal": false,
          "is_tracked": false,
          "name": "reported_time",
          "operation_perms": {},
          "operations": [],
          "order": 0,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "tags": [],
          "templates": [],
          "text": "Reported Time",
          "tooltip": "",
          "type_id": 1018,
          "uuid": "b938fb36-8494-4c31-a27c-74b9f1afb125",
          "values": [],
          "width": 70
        },
        "vulnerabilities": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_assets/vulnerabilities",
          "hide_notification": false,
          "id": 1963,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "vulnerabilities",
          "operation_perms": {},
          "operations": [],
          "order": 6,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "Vulnerabilities",
          "tooltip": "",
          "type_id": 1018,
          "uuid": "4ebea1ce-5d34-4c32-9db4-9a4c290a8e73",
          "values": [],
          "width": 112
        }
      },
      "for_actions": false,
      "for_custom_fields": false,
      "for_notifications": false,
      "for_workflows": false,
      "id": null,
      "parent_types": [
        "incident"
      ],
      "properties": {
        "can_create": false,
        "can_destroy": false,
        "for_who": []
      },
      "scripts": [],
      "tags": [],
      "type_id": 8,
      "type_name": "qr_assets",
      "uuid": "c3e0083f-90dc-4679-9808-cb400d779182"
    },
    {
      "actions": [],
      "display_name": "QR Categories",
      "export_key": "qr_categories",
      "fields": {
        "category_name": {
          "allow_default_value": false,
          "blank_option": true,
          "calculated": false,
          "changeable": true,
          "chosen": true,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_categories/category_name",
          "hide_notification": false,
          "id": 1964,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "category_name",
          "operation_perms": {},
          "operations": [],
          "order": 1,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "Category Name",
          "tooltip": "",
          "type_id": 1019,
          "uuid": "d9477e59-fdf3-4df0-8037-a51da1407d7b",
          "values": [],
          "width": 133
        },
        "destinationip_count": {
          "allow_default_value": false,
          "blank_option": true,
          "calculated": false,
          "changeable": true,
          "chosen": true,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_categories/destinationip_count",
          "hide_notification": false,
          "id": 1965,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "destinationip_count",
          "operation_perms": {},
          "operations": [],
          "order": 3,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "Destination IP",
          "tooltip": "",
          "type_id": 1019,
          "uuid": "9495b67a-31ff-4543-8bb3-613bf92c7fa3",
          "values": [],
          "width": 107
        },
        "event_count": {
          "allow_default_value": false,
          "blank_option": true,
          "calculated": false,
          "changeable": true,
          "chosen": true,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_categories/event_count",
          "hide_notification": false,
          "id": 1966,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "event_count",
          "operation_perms": {},
          "operations": [],
          "order": 5,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "Event Count",
          "tooltip": "",
          "type_id": 1019,
          "uuid": "0dd8491f-4b7f-4fbc-b7c1-c0c466141dc0",
          "values": [],
          "width": 92
        },
        "event_time": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_categories/event_time",
          "hide_notification": false,
          "id": 1967,
          "input_type": "datetimepicker",
          "internal": false,
          "is_tracked": false,
          "name": "event_time",
          "operation_perms": {},
          "operations": [],
          "order": 7,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "tags": [],
          "templates": [],
          "text": "Event Time",
          "tooltip": "",
          "type_id": 1019,
          "uuid": "19531e88-f976-4068-9b60-7e957bf0315b",
          "values": [],
          "width": 116
        },
        "flow_count": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_categories/flow_count",
          "hide_notification": false,
          "id": 1968,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "flow_count",
          "operation_perms": {},
          "operations": [],
          "order": 6,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "Flow Count",
          "tooltip": "",
          "type_id": 1019,
          "uuid": "ed9161b6-f730-4778-8e43-cd1ba49aa14f",
          "values": [],
          "width": 45
        },
        "last_packet_time": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_categories/last_packet_time",
          "hide_notification": false,
          "id": 1969,
          "input_type": "datetimepicker",
          "internal": false,
          "is_tracked": false,
          "name": "last_packet_time",
          "operation_perms": {},
          "operations": [],
          "order": 8,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "tags": [],
          "templates": [],
          "text": "Last Packet Time",
          "tooltip": "",
          "type_id": 1019,
          "uuid": "7242842a-0ed7-45bd-9801-dbe328b0126b",
          "values": [],
          "width": 52
        },
        "magnitude": {
          "allow_default_value": false,
          "blank_option": true,
          "calculated": false,
          "changeable": true,
          "chosen": true,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_categories/magnitude",
          "hide_notification": false,
          "id": 1970,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "magnitude",
          "operation_perms": {},
          "operations": [],
          "order": 4,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "Magnitude",
          "tooltip": "",
          "type_id": 1019,
          "uuid": "98724adf-ef07-4acf-b9fd-da6cf3e827d0",
          "values": [],
          "width": 89
        },
        "reported_time": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_categories/reported_time",
          "hide_notification": false,
          "id": 1971,
          "input_type": "datetimepicker",
          "internal": false,
          "is_tracked": false,
          "name": "reported_time",
          "operation_perms": {},
          "operations": [],
          "order": 0,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "tags": [],
          "templates": [],
          "text": "Reported Time",
          "tooltip": "",
          "type_id": 1019,
          "uuid": "e80372ea-904d-48e7-b50f-cd1608dead47",
          "values": [],
          "width": 70
        },
        "sourceip_count": {
          "allow_default_value": false,
          "blank_option": true,
          "calculated": false,
          "changeable": true,
          "chosen": true,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_categories/sourceip_count",
          "hide_notification": false,
          "id": 1972,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "sourceip_count",
          "operation_perms": {},
          "operations": [],
          "order": 2,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "Source IP",
          "tooltip": "",
          "type_id": 1019,
          "uuid": "44d3f83f-597c-4ecd-ab72-0d57269f591d",
          "values": [],
          "width": 94
        }
      },
      "for_actions": false,
      "for_custom_fields": false,
      "for_notifications": false,
      "for_workflows": false,
      "id": null,
      "parent_types": [
        "incident"
      ],
      "properties": {
        "can_create": false,
        "can_destroy": false,
        "for_who": []
      },
      "scripts": [],
      "tags": [],
      "type_id": 8,
      "type_name": "qr_categories",
      "uuid": "4fc89951-3acc-4d19-b05a-172305cf41fa"
    },
    {
      "actions": [],
      "display_name": "QR Flows",
      "export_key": "qr_flows",
      "fields": {
        "application": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_flows/application",
          "hide_notification": false,
          "id": 1973,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "application",
          "operation_perms": {},
          "operations": [],
          "order": 1,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "Application",
          "tooltip": "",
          "type_id": 1020,
          "uuid": "55ebd534-30f5-472c-8692-7bd9a5a849d3",
          "values": [],
          "width": 88
        },
        "destination_bytes": {
          "allow_default_value": false,
          "blank_option": true,
          "calculated": false,
          "changeable": true,
          "chosen": true,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_flows/destination_bytes",
          "hide_notification": false,
          "id": 1974,
          "input_type": "number",
          "internal": false,
          "is_tracked": false,
          "name": "destination_bytes",
          "operation_perms": {},
          "operations": [],
          "order": 10,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "tags": [],
          "templates": [],
          "text": "Destination Bytes",
          "tooltip": "",
          "type_id": 1020,
          "uuid": "d70568fe-21f6-4008-bab3-0f3fb1e89cf4",
          "values": [],
          "width": 88
        },
        "destination_ip": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_flows/destination_ip",
          "hide_notification": false,
          "id": 1975,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "destination_ip",
          "operation_perms": {},
          "operations": [],
          "order": 4,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "Destination IP",
          "tooltip": "",
          "type_id": 1020,
          "uuid": "e9e41246-f362-418a-a0f1-e7e0d80e3572",
          "values": [],
          "width": 88
        },
        "destination_packets": {
          "allow_default_value": false,
          "blank_option": true,
          "calculated": false,
          "changeable": true,
          "chosen": true,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_flows/destination_packets",
          "hide_notification": false,
          "id": 1976,
          "input_type": "number",
          "internal": false,
          "is_tracked": false,
          "name": "destination_packets",
          "operation_perms": {},
          "operations": [],
          "order": 11,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "tags": [],
          "templates": [],
          "text": "Destination Packets",
          "tooltip": "",
          "type_id": 1020,
          "uuid": "d7ca79dc-840d-4e0d-8be0-bb45c8be4494",
          "values": [],
          "width": 88
        },
        "destination_port": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_flows/destination_port",
          "hide_notification": false,
          "id": 1977,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "destination_port",
          "operation_perms": {},
          "operations": [],
          "order": 5,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "Destination Port",
          "tooltip": "",
          "type_id": 1020,
          "uuid": "367cd2f3-1f57-48ad-b06e-ec6d6d884b8e",
          "values": [],
          "width": 88
        },
        "first_packet_time": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_flows/first_packet_time",
          "hide_notification": false,
          "id": 1978,
          "input_type": "datetimepicker",
          "internal": false,
          "is_tracked": false,
          "name": "first_packet_time",
          "operation_perms": {},
          "operations": [],
          "order": 7,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "tags": [],
          "templates": [],
          "text": "First Packet Time",
          "tooltip": "",
          "type_id": 1020,
          "uuid": "fbeaa822-58b2-4ffd-b56a-3309acb865cf",
          "values": [],
          "width": 52
        },
        "protocol": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_flows/protocol",
          "hide_notification": false,
          "id": 1979,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "protocol",
          "operation_perms": {},
          "operations": [],
          "order": 6,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "tags": [],
          "templates": [],
          "text": "Protocol",
          "tooltip": "",
          "type_id": 1020,
          "uuid": "942b9607-fe2c-400e-971d-982f64d00dd4",
          "values": [],
          "width": 64
        },
        "reported_time": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_flows/reported_time",
          "hide_notification": false,
          "id": 1980,
          "input_type": "datetimepicker",
          "internal": false,
          "is_tracked": false,
          "name": "reported_time",
          "operation_perms": {},
          "operations": [],
          "order": 0,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "tags": [],
          "templates": [],
          "text": "Reported Time",
          "tooltip": "",
          "type_id": 1020,
          "uuid": "8120eaab-790e-4c45-80c4-95135d737818",
          "values": [],
          "width": 70
        },
        "source_bytes": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_flows/source_bytes",
          "hide_notification": false,
          "id": 1981,
          "input_type": "number",
          "internal": false,
          "is_tracked": false,
          "name": "source_bytes",
          "operation_perms": {},
          "operations": [],
          "order": 8,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "tags": [],
          "templates": [],
          "text": "Source Bytes",
          "tooltip": "",
          "type_id": 1020,
          "uuid": "e6db7fbe-9a3e-416a-96af-b7637634519d",
          "values": [],
          "width": 53
        },
        "source_ip": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_flows/source_ip",
          "hide_notification": false,
          "id": 1982,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "source_ip",
          "operation_perms": {},
          "operations": [],
          "order": 2,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "Source IP",
          "tooltip": "",
          "type_id": 1020,
          "uuid": "a949ff96-e8d1-470f-b2ec-1e6168d32165",
          "values": [],
          "width": 53
        },
        "source_packets": {
          "allow_default_value": false,
          "blank_option": true,
          "calculated": false,
          "changeable": true,
          "chosen": true,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_flows/source_packets",
          "hide_notification": false,
          "id": 1983,
          "input_type": "number",
          "internal": false,
          "is_tracked": false,
          "name": "source_packets",
          "operation_perms": {},
          "operations": [],
          "order": 9,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "tags": [],
          "templates": [],
          "text": "Source Packets",
          "tooltip": "",
          "type_id": 1020,
          "uuid": "ddc62352-55d4-4d7d-adeb-9b7283b2448f",
          "values": [],
          "width": 60
        },
        "source_port": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_flows/source_port",
          "hide_notification": false,
          "id": 1984,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "source_port",
          "operation_perms": {},
          "operations": [],
          "order": 3,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "Source Port",
          "tooltip": "",
          "type_id": 1020,
          "uuid": "c90c239d-e1da-4429-a32a-9323fc3ca16d",
          "values": [],
          "width": 53
        }
      },
      "for_actions": false,
      "for_custom_fields": false,
      "for_notifications": false,
      "for_workflows": false,
      "id": null,
      "parent_types": [
        "incident"
      ],
      "properties": {
        "can_create": false,
        "can_destroy": false,
        "for_who": []
      },
      "scripts": [],
      "tags": [],
      "type_id": 8,
      "type_name": "qr_flows",
      "uuid": "c21c1b7a-3f10-4c98-93f5-7853561929a3"
    },
    {
      "actions": [],
      "display_name": "QR Events (First 10 Events)",
      "export_key": "qr_offense_top_events",
      "fields": {
        "category": {
          "allow_default_value": false,
          "blank_option": true,
          "calculated": false,
          "changeable": true,
          "chosen": true,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_offense_top_events/category",
          "hide_notification": false,
          "id": 1985,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "category",
          "operation_perms": {},
          "operations": [],
          "order": 6,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "Category",
          "tooltip": "",
          "type_id": 1021,
          "uuid": "2ab36024-0e8a-4962-8f2f-f05b4b62fcdf",
          "values": [],
          "width": 142
        },
        "destination_ip": {
          "allow_default_value": false,
          "blank_option": true,
          "calculated": false,
          "changeable": true,
          "chosen": true,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_offense_top_events/destination_ip",
          "hide_notification": false,
          "id": 1986,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "destination_ip",
          "operation_perms": {},
          "operations": [],
          "order": 4,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "Destination IP",
          "tooltip": "",
          "type_id": 1021,
          "uuid": "c335a25f-91a9-4119-8a72-affe0ee09528",
          "values": [],
          "width": 155
        },
        "event_count": {
          "allow_default_value": false,
          "blank_option": true,
          "calculated": false,
          "changeable": true,
          "chosen": true,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_offense_top_events/event_count",
          "hide_notification": false,
          "id": 1987,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "event_count",
          "operation_perms": {},
          "operations": [],
          "order": 5,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "Event Count",
          "tooltip": "",
          "type_id": 1021,
          "uuid": "8eeb2a3b-50c8-42e7-907a-1ba44901bc61",
          "values": [],
          "width": 80
        },
        "event_name": {
          "allow_default_value": false,
          "blank_option": true,
          "calculated": false,
          "changeable": true,
          "chosen": true,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_offense_top_events/event_name",
          "hide_notification": false,
          "id": 1988,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "event_name",
          "operation_perms": {},
          "operations": [],
          "order": 1,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "Event Name",
          "tooltip": "",
          "type_id": 1021,
          "uuid": "c45d91a0-373b-4386-8866-cf61558e782e",
          "values": [],
          "width": 248
        },
        "event_time": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_offense_top_events/event_time",
          "hide_notification": false,
          "id": 1989,
          "input_type": "datetimepicker",
          "internal": false,
          "is_tracked": false,
          "name": "event_time",
          "operation_perms": {},
          "operations": [],
          "order": 9,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "tags": [],
          "templates": [],
          "text": "Event Time",
          "tooltip": "",
          "type_id": 1021,
          "uuid": "d0a29a03-f140-495f-94c8-e86cc0d4b8c5",
          "values": [],
          "width": 43
        },
        "log_source": {
          "allow_default_value": false,
          "blank_option": true,
          "calculated": false,
          "changeable": true,
          "chosen": true,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_offense_top_events/log_source",
          "hide_notification": false,
          "id": 1990,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "log_source",
          "operation_perms": {},
          "operations": [],
          "order": 2,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "Log Source",
          "tooltip": "",
          "type_id": 1021,
          "uuid": "c3b2de7d-16b0-4db3-9c57-8cf9c5968e62",
          "values": [],
          "width": 228
        },
        "magnitude": {
          "allow_default_value": false,
          "blank_option": true,
          "calculated": false,
          "changeable": true,
          "chosen": true,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_offense_top_events/magnitude",
          "hide_notification": false,
          "id": 1991,
          "input_type": "text",
          "internal": false,
          "is_tracked": false,
          "name": "magnitude",
          "operation_perms": {},
          "operations": [],
          "order": 8,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "tags": [],
          "templates": [],
          "text": "Magnitude",
          "tooltip": "",
          "type_id": 1021,
          "uuid": "cfb099aa-be4c-4e58-9dbe-6526957ee3a4",
          "values": [],
          "width": 81
        },
        "reported_time": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_offense_top_events/reported_time",
          "hide_notification": false,
          "id": 1992,
          "input_type": "datetimepicker",
          "internal": false,
          "is_tracked": false,
          "name": "reported_time",
          "operation_perms": {},
          "operations": [],
          "order": 0,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "tags": [],
          "templates": [],
          "text": "Reported Time",
          "tooltip": "",
          "type_id": 1021,
          "uuid": "09f90681-325f-45e3-b1b2-6d0fa3b7312d",
          "values": [],
          "width": 70
        },
        "source_ip": {
          "allow_default_value": false,
          "blank_option": true,
          "calculated": false,
          "changeable": true,
          "chosen": true,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_offense_top_events/source_ip",
          "hide_notification": false,
          "id": 1993,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "source_ip",
          "operation_perms": {},
          "operations": [],
          "order": 3,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "Source IP",
          "tooltip": "",
          "type_id": 1021,
          "uuid": "7ff661a9-2b89-4f5b-bc55-e91df4aeaea3",
          "values": [],
          "width": 146
        },
        "username": {
          "allow_default_value": false,
          "blank_option": true,
          "calculated": false,
          "changeable": true,
          "chosen": true,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_offense_top_events/username",
          "hide_notification": false,
          "id": 1994,
          "input_type": "text",
          "internal": false,
          "is_tracked": false,
          "name": "username",
          "operation_perms": {},
          "operations": [],
          "order": 7,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "tags": [],
          "templates": [],
          "text": "Username",
          "tooltip": "",
          "type_id": 1021,
          "uuid": "670ee674-e080-44ae-876c-4991fdaf2e27",
          "values": [],
          "width": 105
        }
      },
      "for_actions": false,
      "for_custom_fields": false,
      "for_notifications": false,
      "for_workflows": false,
      "id": null,
      "parent_types": [
        "incident"
      ],
      "properties": {
        "can_create": false,
        "can_destroy": false,
        "for_who": []
      },
      "scripts": [],
      "tags": [],
      "type_id": 8,
      "type_name": "qr_offense_top_events",
      "uuid": "5abb79cb-07a4-466a-93f6-7ada972d920e"
    },
    {
      "actions": [],
      "display_name": "QR Destination IPs (First 10)",
      "export_key": "qr_top_destination_ips",
      "fields": {
        "category_count": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_top_destination_ips/category_count",
          "hide_notification": false,
          "id": 1995,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "category_count",
          "operation_perms": {},
          "operations": [],
          "order": 4,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "Category Count",
          "tooltip": "",
          "type_id": 1022,
          "uuid": "d7fe1951-6136-46b2-8389-62525ada5c40",
          "values": [],
          "width": 108
        },
        "destination_ip": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_top_destination_ips/destination_ip",
          "hide_notification": false,
          "id": 1996,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "destination_ip",
          "operation_perms": {},
          "operations": [],
          "order": 1,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "Destination IP",
          "tooltip": "",
          "type_id": 1022,
          "uuid": "2bb02d45-dd54-4d55-be57-c96bf74dda27",
          "values": [],
          "width": 122
        },
        "event_count": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_top_destination_ips/event_count",
          "hide_notification": false,
          "id": 1997,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "event_count",
          "operation_perms": {},
          "operations": [],
          "order": 2,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "Event Count",
          "tooltip": "",
          "type_id": 1022,
          "uuid": "7bbc0dcc-82a7-4907-82b1-1e6f6cedcf73",
          "values": [],
          "width": 82
        },
        "flow_count": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_top_destination_ips/flow_count",
          "hide_notification": false,
          "id": 1998,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "flow_count",
          "operation_perms": {},
          "operations": [],
          "order": 3,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "Flow Count",
          "tooltip": "",
          "type_id": 1022,
          "uuid": "312615da-c065-4a0b-9e59-d3ac344fb32e",
          "values": [],
          "width": 374
        },
        "reported_time": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_top_destination_ips/reported_time",
          "hide_notification": false,
          "id": 1999,
          "input_type": "datetimepicker",
          "internal": false,
          "is_tracked": false,
          "name": "reported_time",
          "operation_perms": {},
          "operations": [],
          "order": 0,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "tags": [],
          "templates": [],
          "text": "Reported Time",
          "tooltip": "",
          "type_id": 1022,
          "uuid": "ae412689-6c07-47ab-8750-03c468692882",
          "values": [],
          "width": 70
        }
      },
      "for_actions": false,
      "for_custom_fields": false,
      "for_notifications": false,
      "for_workflows": false,
      "id": null,
      "parent_types": [
        "incident"
      ],
      "properties": {
        "can_create": false,
        "can_destroy": false,
        "for_who": []
      },
      "scripts": [],
      "tags": [],
      "type_id": 8,
      "type_name": "qr_top_destination_ips",
      "uuid": "36c24e29-ff9d-4557-b32b-05bdb7fdf298"
    },
    {
      "actions": [],
      "display_name": "QR Source IPs (First 10)",
      "export_key": "qr_top_source_ips",
      "fields": {
        "category_count": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_top_source_ips/category_count",
          "hide_notification": false,
          "id": 2000,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "category_count",
          "operation_perms": {},
          "operations": [],
          "order": 4,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "Category Count",
          "tooltip": "",
          "type_id": 1023,
          "uuid": "ec8d00a0-ab2b-4f08-bdf3-717c5b1ba908",
          "values": [],
          "width": 109
        },
        "domain": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_top_source_ips/domain",
          "hide_notification": false,
          "id": 2001,
          "input_type": "text",
          "internal": false,
          "is_tracked": false,
          "name": "domain",
          "operation_perms": {},
          "operations": [],
          "order": 7,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "tags": [],
          "templates": [],
          "text": "Domain",
          "tooltip": "",
          "type_id": 1023,
          "uuid": "2d51d52d-ad65-4263-909e-68242d4befd1",
          "values": [],
          "width": 78
        },
        "event_count": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_top_source_ips/event_count",
          "hide_notification": false,
          "id": 2002,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "event_count",
          "operation_perms": {},
          "operations": [],
          "order": 2,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "Event Count",
          "tooltip": "",
          "type_id": 1023,
          "uuid": "7a6917ee-041e-45a8-a6f3-4d9fa7d46621",
          "values": [],
          "width": 106
        },
        "flow_count": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_top_source_ips/flow_count",
          "hide_notification": false,
          "id": 2003,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "flow_count",
          "operation_perms": {},
          "operations": [],
          "order": 3,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "Flow Count",
          "tooltip": "",
          "type_id": 1023,
          "uuid": "d6aa26ad-5d57-4aa3-9c5d-84d4a9fc48f8",
          "values": [],
          "width": 45
        },
        "mac": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_top_source_ips/mac",
          "hide_notification": false,
          "id": 2004,
          "input_type": "text",
          "internal": false,
          "is_tracked": false,
          "name": "mac",
          "operation_perms": {},
          "operations": [],
          "order": 8,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "tags": [],
          "templates": [],
          "text": "MAC",
          "tooltip": "",
          "type_id": 1023,
          "uuid": "ad4d32ee-0900-41d0-a86c-95b82d8f8cdf",
          "values": [],
          "width": 77
        },
        "network": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_top_source_ips/network",
          "hide_notification": false,
          "id": 2005,
          "input_type": "text",
          "internal": false,
          "is_tracked": false,
          "name": "network",
          "operation_perms": {},
          "operations": [],
          "order": 6,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "tags": [],
          "templates": [],
          "text": "Network",
          "tooltip": "",
          "type_id": 1023,
          "uuid": "899cf2a4-c347-46bc-8b00-a3721c537def",
          "values": [],
          "width": 65
        },
        "reported_time": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_top_source_ips/reported_time",
          "hide_notification": false,
          "id": 2006,
          "input_type": "datetimepicker",
          "internal": false,
          "is_tracked": false,
          "name": "reported_time",
          "operation_perms": {},
          "operations": [],
          "order": 0,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "tags": [],
          "templates": [],
          "text": "Reported Time",
          "tooltip": "",
          "type_id": 1023,
          "uuid": "95e4152a-c0ee-4827-a055-f29144ad1a62",
          "values": [],
          "width": 70
        },
        "source_ip": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_top_source_ips/source_ip",
          "hide_notification": false,
          "id": 2007,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "source_ip",
          "operation_perms": {},
          "operations": [],
          "order": 1,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "Source IP",
          "tooltip": "",
          "type_id": 1023,
          "uuid": "8e61b967-7937-4038-ae97-1d3682735173",
          "values": [],
          "width": 113
        },
        "usernames": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_top_source_ips/usernames",
          "hide_notification": false,
          "id": 2008,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "usernames",
          "operation_perms": {},
          "operations": [],
          "order": 9,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "Usernames",
          "tooltip": "",
          "type_id": 1023,
          "uuid": "3d8c9e6c-c5c9-48c6-84d5-b3a8905e3a03",
          "values": [],
          "width": 99
        },
        "vulnerability_count": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_top_source_ips/vulnerability_count",
          "hide_notification": false,
          "id": 2009,
          "input_type": "number",
          "internal": false,
          "is_tracked": false,
          "name": "vulnerability_count",
          "operation_perms": {},
          "operations": [],
          "order": 5,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "tags": [],
          "templates": [],
          "text": "Vulnerability Count",
          "tooltip": "",
          "type_id": 1023,
          "uuid": "a6f51997-4742-4093-b7f6-a2f7650076f4",
          "values": [],
          "width": 131
        }
      },
      "for_actions": false,
      "for_custom_fields": false,
      "for_notifications": false,
      "for_workflows": false,
      "id": null,
      "parent_types": [
        "incident"
      ],
      "properties": {
        "can_create": false,
        "can_destroy": false,
        "for_who": []
      },
      "scripts": [],
      "tags": [],
      "type_id": 8,
      "type_name": "qr_top_source_ips",
      "uuid": "66a57247-1835-4f18-811f-7a4ad96a295c"
    },
    {
      "actions": [],
      "display_name": "QR Triggered Rules",
      "export_key": "qr_triggered_rules",
      "fields": {
        "date_created": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_triggered_rules/date_created",
          "hide_notification": false,
          "id": 2010,
          "input_type": "datetimepicker",
          "internal": false,
          "is_tracked": false,
          "name": "date_created",
          "operation_perms": {},
          "operations": [],
          "order": 5,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "tags": [],
          "templates": [],
          "text": "Date Created",
          "tooltip": "",
          "type_id": 1024,
          "uuid": "10916f01-0d40-49d4-aa23-ee292a881abd",
          "values": [],
          "width": 60
        },
        "enabled": {
          "allow_default_value": false,
          "blank_option": true,
          "calculated": false,
          "changeable": true,
          "chosen": true,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_triggered_rules/enabled",
          "hide_notification": false,
          "id": 2011,
          "input_type": "text",
          "internal": false,
          "is_tracked": false,
          "name": "enabled",
          "operation_perms": {},
          "operations": [],
          "order": 7,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "tags": [],
          "templates": [],
          "text": "Enabled",
          "tooltip": "",
          "type_id": 1024,
          "uuid": "51cd6776-43b2-4845-bcdb-aea84ffccd18",
          "values": [],
          "width": 62
        },
        "last_modified": {
          "allow_default_value": false,
          "blank_option": false,
          "calculated": false,
          "changeable": true,
          "chosen": false,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_triggered_rules/last_modified",
          "hide_notification": false,
          "id": 2012,
          "input_type": "datetimepicker",
          "internal": false,
          "is_tracked": false,
          "name": "last_modified",
          "operation_perms": {},
          "operations": [],
          "order": 6,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "tags": [],
          "templates": [],
          "text": "Last Modified",
          "tooltip": "",
          "type_id": 1024,
          "uuid": "f1bac767-7798-4572-a86b-2f0bc0a851a1",
          "values": [],
          "width": 67
        },
        "reported_time": {
          "allow_default_value": false,
          "blank_option": true,
          "calculated": false,
          "changeable": true,
          "chosen": true,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_triggered_rules/reported_time",
          "hide_notification": false,
          "id": 2013,
          "input_type": "datetimepicker",
          "internal": false,
          "is_tracked": false,
          "name": "reported_time",
          "operation_perms": {},
          "operations": [],
          "order": 0,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "tags": [],
          "templates": [],
          "text": "Reported Time",
          "tooltip": "",
          "type_id": 1024,
          "uuid": "bd13ffd1-b303-4b86-9eef-b8a44702cd80",
          "values": [],
          "width": 70
        },
        "response": {
          "allow_default_value": false,
          "blank_option": true,
          "calculated": false,
          "changeable": true,
          "chosen": true,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_triggered_rules/response",
          "hide_notification": false,
          "id": 2014,
          "input_type": "text",
          "internal": false,
          "is_tracked": false,
          "name": "response",
          "operation_perms": {},
          "operations": [],
          "order": 4,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "tags": [],
          "templates": [],
          "text": "Response",
          "tooltip": "",
          "type_id": 1024,
          "uuid": "34ff5942-6417-424e-bbdb-a6fa3f32774e",
          "values": [],
          "width": 74
        },
        "rule_group": {
          "allow_default_value": false,
          "blank_option": true,
          "calculated": false,
          "changeable": true,
          "chosen": true,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_triggered_rules/rule_group",
          "hide_notification": false,
          "id": 2015,
          "input_type": "text",
          "internal": false,
          "is_tracked": false,
          "name": "rule_group",
          "operation_perms": {},
          "operations": [],
          "order": 2,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "tags": [],
          "templates": [],
          "text": "Rule Group",
          "tooltip": "",
          "type_id": 1024,
          "uuid": "71634383-1e15-4f00-9f74-adeb79d5f1e9",
          "values": [],
          "width": 47
        },
        "rule_name": {
          "allow_default_value": false,
          "blank_option": true,
          "calculated": false,
          "changeable": true,
          "chosen": true,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_triggered_rules/rule_name",
          "hide_notification": false,
          "id": 2016,
          "input_type": "textarea",
          "internal": false,
          "is_tracked": false,
          "name": "rule_name",
          "operation_perms": {},
          "operations": [],
          "order": 1,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": true,
          "tags": [],
          "templates": [],
          "text": "Rule Name",
          "tooltip": "",
          "type_id": 1024,
          "uuid": "c516c54d-af5f-47f5-a82f-16d0ac196323",
          "values": [],
          "width": 286
        },
        "rule_type": {
          "allow_default_value": false,
          "blank_option": true,
          "calculated": false,
          "changeable": true,
          "chosen": true,
          "default_chosen_by_server": false,
          "deprecated": false,
          "export_key": "qr_triggered_rules/rule_type",
          "hide_notification": false,
          "id": 2017,
          "input_type": "text",
          "internal": false,
          "is_tracked": false,
          "name": "rule_type",
          "operation_perms": {},
          "operations": [],
          "order": 3,
          "placeholder": "",
          "prefix": null,
          "read_only": false,
          "rich_text": false,
          "tags": [],
          "templates": [],
          "text": "Rule Type",
          "tooltip": "",
          "type_id": 1024,
          "uuid": "2cc90f9c-3579-48d0-8445-e93090d0e48a",
          "values": [],
          "width": 71
        }
      },
      "for_actions": false,
      "for_custom_fields": false,
      "for_notifications": false,
      "for_workflows": false,
      "id": null,
      "parent_types": [
        "incident"
      ],
      "properties": {
        "can_create": false,
        "can_destroy": false,
        "for_who": []
      },
      "scripts": [],
      "tags": [],
      "type_id": 8,
      "type_name": "qr_triggered_rules",
      "uuid": "34b2b00a-56eb-439a-a93c-74daf0554db2"
    }
  ],
  "workflows": [
    {
      "actions": [],
      "content": {
        "version": 9,
        "workflow_id": "qradar_categories",
        "xml": "\u003c?xml version=\"1.0\" encoding=\"UTF-8\"?\u003e\u003cdefinitions xmlns=\"http://www.omg.org/spec/BPMN/20100524/MODEL\" xmlns:bpmndi=\"http://www.omg.org/spec/BPMN/20100524/DI\" xmlns:omgdc=\"http://www.omg.org/spec/DD/20100524/DC\" xmlns:omgdi=\"http://www.omg.org/spec/DD/20100524/DI\" xmlns:resilient=\"http://resilient.ibm.com/bpmn\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" targetNamespace=\"http://www.camunda.org/test\"\u003e\u003cprocess id=\"qradar_categories\" isExecutable=\"true\" name=\"Example of searching and returning Categories\"\u003e\u003cdocumentation\u003eReturns event categories for the QRadar Offense\u003c/documentation\u003e\u003cstartEvent id=\"StartEvent_155asxm\"\u003e\u003coutgoing\u003eSequenceFlow_0cg7aq8\u003c/outgoing\u003e\u003coutgoing\u003eSequenceFlow_16sbi3h\u003c/outgoing\u003e\u003c/startEvent\u003e\u003cserviceTask id=\"ServiceTask_193h79o\" name=\"QRadar Top Events\" resilient:type=\"function\"\u003e\u003cextensionElements\u003e\u003cresilient:function uuid=\"d8e63812-0b58-4ca1-991a-49255f3dd04e\"\u003e{\"inputs\":{\"048ba39a-ab94-4d1f-a0f8-2462de3c044c\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_content_value\":{\"format\":\"text\",\"content\":\"SELECT %param1% FROM events %param2% %param4% %param5% %param6% LAST %param7% PARAMETERS PROGRESSDETAILSRESOLUTION=60\"}}},\"01ed4652-fe61-4dbf-bc83-f81c62c5563c\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"CATEGORYNAME(category) as categoryname,magnitude as magnitude,SUM(eventcount) as eventcount,max(starttime) as eventtime,UNIQUECOUNT(sourceip) as sourceipcount,UNIQUECOUNT(destinationip) as destinationipcount \"}},\"9d8fc645-5067-4ab0-b151-1d106dad91d6\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"WHERE INOFFENSE(%param3%)\"}},\"eefc9e56-2806-4534-b409-42e25154ecc9\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"GROUP BY category,magnitude\"}},\"19778fc0-9a30-4b8b-9c21-642495f9eebc\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"ORDER BY max(starttime)  DESC\"}},\"ca2d7d7f-fe6d-4a21-b011-7953494c350c\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"LIMIT 10\"}}},\"post_processing_script\":\"link = \\\"\u0026lt;a href=\\\\\\\"https://\\\" + results.get(\\\"content\\\")[\\\"qrhost\\\"] + \\\"/console/ui/offenses/{0}/events?filter={1}%3B%3D%3B%3B{2}\u0026amp;page=1\u0026amp;pagesize=10\\\\\\\" target=\\\\\\\"_blank\\\\\\\"\u0026gt;{3}\u0026lt;/a\u0026gt;\\\"\\n\\nfor event in results.get(\\\"content\\\")[\\\"events\\\"]:\\n  qradar_event = incident.addRow(\\\"qr_categories\\\")\\n  qradar_event.category_name = link.format(results.get(\\\"content\\\")[\\\"offenseid\\\"], \\\"category_name\\\", event.categoryname, event.categoryname)\\n  qradar_event.magnitude = link.format(results.get(\\\"content\\\")[\\\"offenseid\\\"], \\\"category_name\\\", event.categoryname, event.magnitude)\\n  qradar_event.event_count = link.format(results.get(\\\"content\\\")[\\\"offenseid\\\"], \\\"category_name\\\", event.categoryname, event.eventcount)\\n  qradar_event.event_time = int(event.eventtime)\\n  qradar_event.sourceip_count = link.format(results.get(\\\"content\\\")[\\\"offenseid\\\"], \\\"category_name\\\", event.categoryname, event.sourceipcount)\\n  qradar_event.destinationip_count = link.format(results.get(\\\"content\\\")[\\\"offenseid\\\"], \\\"category_name\\\", event.categoryname, event.destinationipcount)\\n  qradar_event.reported_time = results.get(\\\"content\\\")[\\\"current_time\\\"]\",\"post_processing_script_language\":\"python\",\"pre_processing_script\":\"inputs.qradar_search_param3 = incident.properties.qradar_id\\ninputs.qradar_query_type = \\\"categories\\\"\\ninputs.qradar_label = incident.properties.qradar_destination\\ninputs.soar_table_name = \\\"qr_categories\\\"\\ninputs.soar_incident_id = incident.id\\n\\n# QRadar graphql search look back time default is 5 days\\ninputs.qradar_search_param7 = \\\"5 days\\\"\\n# If the poller is running and the qr_last_updated_time is changed the\\n# the QRadar graphql look back time will change to 2 days\\nif incident.properties.qr_last_updated_time != incident.create_date:\\n  inputs.qradar_search_param7 = \\\"2 days\\\"\\n# If manual QRadar Update rule is run set the number if days to search to the\\n# user entered number\\nif rule.properties.number_of_days_to_search:\\n  inputs.qradar_search_param7 = str(rule.properties.number_of_days_to_search) + \\\" days\\\"\",\"pre_processing_script_language\":\"python\",\"result_name\":\"\"}\u003c/resilient:function\u003e\u003c/extensionElements\u003e\u003cincoming\u003eSequenceFlow_0cg7aq8\u003c/incoming\u003e\u003coutgoing\u003eSequenceFlow_08d3uem\u003c/outgoing\u003e\u003c/serviceTask\u003e\u003cendEvent id=\"EndEvent_0zz07yv\"\u003e\u003cincoming\u003eSequenceFlow_08d3uem\u003c/incoming\u003e\u003c/endEvent\u003e\u003csequenceFlow id=\"SequenceFlow_08d3uem\" sourceRef=\"ServiceTask_193h79o\" targetRef=\"EndEvent_0zz07yv\"/\u003e\u003csequenceFlow id=\"SequenceFlow_0cg7aq8\" sourceRef=\"StartEvent_155asxm\" targetRef=\"ServiceTask_193h79o\"/\u003e\u003cserviceTask id=\"ServiceTask_0psm0xv\" name=\"QRadar Top Events\" resilient:type=\"function\"\u003e\u003cextensionElements\u003e\u003cresilient:function uuid=\"d8e63812-0b58-4ca1-991a-49255f3dd04e\"\u003e{\"inputs\":{\"048ba39a-ab94-4d1f-a0f8-2462de3c044c\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_content_value\":{\"format\":\"text\",\"content\":\"SELECT %param1% FROM flows %param2% %param4% %param5% %param6% LAST %param7% PARAMETERS PROGRESSDETAILSRESOLUTION=60\"}}},\"01ed4652-fe61-4dbf-bc83-f81c62c5563c\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"CATEGORYNAME(category) as categoryname,COUNT(*) as flowcount,max(lastpackettime) as lastpackettime,UNIQUECOUNT(sourceip) as sourceipcount,UNIQUECOUNT(destinationip) as destinationipcount \"}},\"9d8fc645-5067-4ab0-b151-1d106dad91d6\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"WHERE INOFFENSE(%param3%)\"}},\"eefc9e56-2806-4534-b409-42e25154ecc9\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"GROUP BY category\"}},\"19778fc0-9a30-4b8b-9c21-642495f9eebc\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"ORDER BY lastpackettime  DESC\"}},\"ca2d7d7f-fe6d-4a21-b011-7953494c350c\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"LIMIT 10\"}}},\"post_processing_script\":\"link = \\\"\u0026lt;a href=\\\\\\\"https://\\\" + results.get(\\\"content\\\")[\\\"qrhost\\\"] + \\\"/console/ui/offenses/{0}/events?filter={1}%3B%3D%3B%3B{2}\u0026amp;page=1\u0026amp;pagesize=10\\\\\\\" target=\\\\\\\"_blank\\\\\\\"\u0026gt;{3}\u0026lt;/a\u0026gt;\\\"\\n\\nfor event in results.get(\\\"content\\\")[\\\"events\\\"]:\\n  qradar_event = incident.addRow(\\\"qr_categories\\\")\\n  qradar_event.category_name = event.categoryname\\n  qradar_event.flow_count = event.flowcount\\n  qradar_event.last_packet_time = int(event.lastpackettime)\\n  qradar_event.sourceip_count = event.sourceipcount\\n  qradar_event.destinationip_count = event.destinationipcount\\n  qradar_event.reported_time = results.get(\\\"content\\\")[\\\"current_time\\\"]\",\"post_processing_script_language\":\"python\",\"pre_processing_script\":\"inputs.qradar_search_param3 = incident.properties.qradar_id\\ninputs.qradar_query_type = \\\"categories\\\"\\ninputs.qradar_label = incident.properties.qradar_destination\\ninputs.soar_table_name = \\\"qr_categories\\\"\\ninputs.soar_incident_id = incident.id\\n\\n# QRadar graphql search look back time default is 5 days\\ninputs.qradar_search_param7 = \\\"5 days\\\"\\n# If the poller is running and the qr_last_updated_time is changed the\\n# the QRadar graphql look back time will change to 2 days\\nif incident.properties.qr_last_updated_time != incident.create_date:\\n  inputs.qradar_search_param7 = \\\"2 days\\\"\\n# If manual QRadar Update rule is run set the number if days to search to the\\n# user entered number\\nif rule.properties.number_of_days_to_search:\\n  inputs.qradar_search_param7 = str(rule.properties.number_of_days_to_search) + \\\" days\\\"\",\"pre_processing_script_language\":\"python\",\"result_name\":\"\"}\u003c/resilient:function\u003e\u003c/extensionElements\u003e\u003cincoming\u003eSequenceFlow_16sbi3h\u003c/incoming\u003e\u003coutgoing\u003eSequenceFlow_17dxnd1\u003c/outgoing\u003e\u003c/serviceTask\u003e\u003csequenceFlow id=\"SequenceFlow_16sbi3h\" sourceRef=\"StartEvent_155asxm\" targetRef=\"ServiceTask_0psm0xv\"/\u003e\u003cendEvent id=\"EndEvent_1ar2znf\"\u003e\u003cincoming\u003eSequenceFlow_17dxnd1\u003c/incoming\u003e\u003c/endEvent\u003e\u003csequenceFlow id=\"SequenceFlow_17dxnd1\" sourceRef=\"ServiceTask_0psm0xv\" targetRef=\"EndEvent_1ar2znf\"/\u003e\u003ctextAnnotation id=\"TextAnnotation_1kxxiyt\"\u003e\u003ctext\u003eStart your workflow here\u003c/text\u003e\u003c/textAnnotation\u003e\u003cassociation id=\"Association_1seuj48\" sourceRef=\"StartEvent_155asxm\" targetRef=\"TextAnnotation_1kxxiyt\"/\u003e\u003c/process\u003e\u003cbpmndi:BPMNDiagram id=\"BPMNDiagram_1\"\u003e\u003cbpmndi:BPMNPlane bpmnElement=\"undefined\" id=\"BPMNPlane_1\"\u003e\u003cbpmndi:BPMNShape bpmnElement=\"StartEvent_155asxm\" id=\"StartEvent_155asxm_di\"\u003e\u003comgdc:Bounds height=\"36\" width=\"36\" x=\"162\" y=\"188\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"0\" width=\"90\" x=\"157\" y=\"223\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNShape bpmnElement=\"TextAnnotation_1kxxiyt\" id=\"TextAnnotation_1kxxiyt_di\"\u003e\u003comgdc:Bounds height=\"30\" width=\"100\" x=\"99\" y=\"254\"/\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"Association_1seuj48\" id=\"Association_1seuj48_di\"\u003e\u003comgdi:waypoint x=\"169\" xsi:type=\"omgdc:Point\" y=\"220\"/\u003e\u003comgdi:waypoint x=\"153\" xsi:type=\"omgdc:Point\" y=\"254\"/\u003e\u003c/bpmndi:BPMNEdge\u003e\u003cbpmndi:BPMNShape bpmnElement=\"ServiceTask_193h79o\" id=\"ServiceTask_193h79o_di\"\u003e\u003comgdc:Bounds height=\"80\" width=\"100\" x=\"345\" y=\"166\"/\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNShape bpmnElement=\"EndEvent_0zz07yv\" id=\"EndEvent_0zz07yv_di\"\u003e\u003comgdc:Bounds height=\"36\" width=\"36\" x=\"601\" y=\"188\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"13\" width=\"0\" x=\"619\" y=\"227\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"SequenceFlow_08d3uem\" id=\"SequenceFlow_08d3uem_di\"\u003e\u003comgdi:waypoint x=\"445\" xsi:type=\"omgdc:Point\" y=\"206\"/\u003e\u003comgdi:waypoint x=\"601\" xsi:type=\"omgdc:Point\" y=\"206\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"13\" width=\"0\" x=\"523\" y=\"184\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNEdge\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"SequenceFlow_0cg7aq8\" id=\"SequenceFlow_0cg7aq8_di\"\u003e\u003comgdi:waypoint x=\"198\" xsi:type=\"omgdc:Point\" y=\"206\"/\u003e\u003comgdi:waypoint x=\"345\" xsi:type=\"omgdc:Point\" y=\"206\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"13\" width=\"0\" x=\"271.5\" y=\"184\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNEdge\u003e\u003cbpmndi:BPMNShape bpmnElement=\"ServiceTask_0psm0xv\" id=\"ServiceTask_0psm0xv_di\"\u003e\u003comgdc:Bounds height=\"80\" width=\"100\" x=\"345\" y=\"297\"/\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"SequenceFlow_16sbi3h\" id=\"SequenceFlow_16sbi3h_di\"\u003e\u003comgdi:waypoint x=\"188\" xsi:type=\"omgdc:Point\" y=\"222\"/\u003e\u003comgdi:waypoint x=\"272\" xsi:type=\"omgdc:Point\" y=\"337\"/\u003e\u003comgdi:waypoint x=\"345\" xsi:type=\"omgdc:Point\" y=\"337\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"14\" width=\"0\" x=\"230\" y=\"257.5\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNEdge\u003e\u003cbpmndi:BPMNShape bpmnElement=\"EndEvent_1ar2znf\" id=\"EndEvent_1ar2znf_di\"\u003e\u003comgdc:Bounds height=\"36\" width=\"36\" x=\"611.4709897610921\" y=\"318.86120591581346\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"14\" width=\"0\" x=\"629.4709897610921\" y=\"357.86120591581346\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"SequenceFlow_17dxnd1\" id=\"SequenceFlow_17dxnd1_di\"\u003e\u003comgdi:waypoint x=\"445\" xsi:type=\"omgdc:Point\" y=\"337\"/\u003e\u003comgdi:waypoint x=\"530\" xsi:type=\"omgdc:Point\" y=\"337\"/\u003e\u003comgdi:waypoint x=\"530\" xsi:type=\"omgdc:Point\" y=\"337\"/\u003e\u003comgdi:waypoint x=\"611\" xsi:type=\"omgdc:Point\" y=\"337\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"14\" width=\"0\" x=\"545\" y=\"330\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNEdge\u003e\u003c/bpmndi:BPMNPlane\u003e\u003c/bpmndi:BPMNDiagram\u003e\u003c/definitions\u003e"
      },
      "content_version": 9,
      "description": "Returns event categories for the QRadar Offense",
      "export_key": "qradar_categories",
      "last_modified_by": "d@example.com",
      "last_modified_time": 1666185704592,
      "name": "Example of searching and returning Categories",
      "object_type": "incident",
      "programmatic_name": "qradar_categories",
      "tags": [],
      "uuid": "0bf4dc61-c103-4805-88bd-91e1c2304538",
      "workflow_id": 57
    },
    {
      "actions": [],
      "content": {
        "version": 6,
        "workflow_id": "qradar_source_ips",
        "xml": "\u003c?xml version=\"1.0\" encoding=\"UTF-8\"?\u003e\u003cdefinitions xmlns=\"http://www.omg.org/spec/BPMN/20100524/MODEL\" xmlns:bpmndi=\"http://www.omg.org/spec/BPMN/20100524/DI\" xmlns:omgdc=\"http://www.omg.org/spec/DD/20100524/DC\" xmlns:omgdi=\"http://www.omg.org/spec/DD/20100524/DI\" xmlns:resilient=\"http://resilient.ibm.com/bpmn\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" targetNamespace=\"http://www.camunda.org/test\"\u003e\u003cprocess id=\"qradar_source_ips\" isExecutable=\"true\" name=\"Example of searching and returning Source IPs information\"\u003e\u003cdocumentation\u003eReturns the information about the source IPs associated with the QRadar Offense ID\u003c/documentation\u003e\u003cstartEvent id=\"StartEvent_155asxm\"\u003e\u003coutgoing\u003eSequenceFlow_1ykiehr\u003c/outgoing\u003e\u003coutgoing\u003eSequenceFlow_1bpm9fv\u003c/outgoing\u003e\u003c/startEvent\u003e\u003cserviceTask id=\"ServiceTask_0llgtav\" name=\"QRadar Top Events\" resilient:type=\"function\"\u003e\u003cextensionElements\u003e\u003cresilient:function uuid=\"d8e63812-0b58-4ca1-991a-49255f3dd04e\"\u003e{\"inputs\":{\"048ba39a-ab94-4d1f-a0f8-2462de3c044c\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_content_value\":{\"format\":\"text\",\"content\":\"SELECT %param1% FROM events %param2% %param4% %param5% %param6% LAST %param7% PARAMETERS PROGRESSDETAILSRESOLUTION=60\"}}},\"01ed4652-fe61-4dbf-bc83-f81c62c5563c\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"sourceip as sourceip,SUM(eventcount) as eventcount,UNIQUECOUNT(category) as categorycount,UNIQUECOUNT(username) as usernamecount,max(starttime)\"}},\"9d8fc645-5067-4ab0-b151-1d106dad91d6\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"WHERE INOFFENSE(%param3%)\"}},\"eefc9e56-2806-4534-b409-42e25154ecc9\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"GROUP BY sourceip\"}},\"19778fc0-9a30-4b8b-9c21-642495f9eebc\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"ORDER BY max(starttime) DESC\"}},\"ca2d7d7f-fe6d-4a21-b011-7953494c350c\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"LIMIT 10\"}}},\"post_processing_script\":\"link = \\\"\u0026lt;a href=\\\\\\\"https://\\\" + results.get(\\\"content\\\")[\\\"qrhost\\\"] + \\\"/console/ui/offenses/{0}/events?filter={1}%3B%3D%3B%3B{2}\u0026amp;page=1\u0026amp;pagesize=10\\\\\\\" target=\\\\\\\"_blank\\\\\\\"\u0026gt;{3}\u0026lt;/a\u0026gt;\\\"\\n\\nfor event in results.get(\\\"content\\\")[\\\"events\\\"]:\\n  qradar_event = incident.addRow(\\\"qr_top_source_ips\\\")\\n  qradar_event.source_ip = link.format(results.get(\\\"content\\\")[\\\"offenseid\\\"], \\\"sourceip\\\", event.sourceip, event.sourceip)\\n  qradar_event.vulnerability_count = event.vulnerabilityCount\\n  qradar_event.mac = event.macAddress\\n  qradar_event.network = event.network\\n  qradar_event.domain = event.domain\\n  qradar_event.usernames = event.usernamecount\\n  qradar_event.event_count = link.format(results.get(\\\"content\\\")[\\\"offenseid\\\"], \\\"sourceip\\\", event.sourceip, event.eventcount)\\n  qradar_event.category_count = link.format(results.get(\\\"content\\\")[\\\"offenseid\\\"], \\\"sourceip\\\", event.sourceip, event.categorycount)\\n  qradar_event.reported_time = results.get(\\\"content\\\")[\\\"current_time\\\"]\",\"post_processing_script_language\":\"python\",\"pre_processing_script\":\"inputs.qradar_search_param3 = incident.properties.qradar_id\\ninputs.qradar_query_type = \\\"sourceip\\\"\\ninputs.qradar_label = incident.properties.qradar_destination\\ninputs.soar_incident_id = incident.id\\ninputs.soar_table_name = \\\"qr_top_source_ips\\\"\\n\\n# QRadar graphql search look back time default is 5 days\\ninputs.qradar_search_param7 = \\\"5 days\\\"\\n# If the poller is running and the qr_last_updated_time is changed the\\n# the QRadar graphql look back time will change to 2 days\\nif incident.properties.qr_last_updated_time != incident.create_date:\\n  inputs.qradar_search_param7 = \\\"2 days\\\"\\n# If manual QRadar Update rule is run set the number if days to search to the\\n# user entered number\\nif rule.properties.number_of_days_to_search:\\n  inputs.qradar_search_param7 = str(rule.properties.number_of_days_to_search) + \\\" days\\\"\",\"pre_processing_script_language\":\"python\",\"result_name\":\"\"}\u003c/resilient:function\u003e\u003c/extensionElements\u003e\u003cincoming\u003eSequenceFlow_1ykiehr\u003c/incoming\u003e\u003coutgoing\u003eSequenceFlow_1xf36er\u003c/outgoing\u003e\u003c/serviceTask\u003e\u003cendEvent id=\"EndEvent_0dfgyrw\"\u003e\u003cincoming\u003eSequenceFlow_0ebhn0g\u003c/incoming\u003e\u003c/endEvent\u003e\u003csequenceFlow id=\"SequenceFlow_1bpm9fv\" sourceRef=\"StartEvent_155asxm\" targetRef=\"ServiceTask_01babo2\"/\u003e\u003csequenceFlow id=\"SequenceFlow_1ykiehr\" sourceRef=\"StartEvent_155asxm\" targetRef=\"ServiceTask_0llgtav\"/\u003e\u003cserviceTask id=\"ServiceTask_01babo2\" name=\"QRadar Top Events\" resilient:type=\"function\"\u003e\u003cextensionElements\u003e\u003cresilient:function uuid=\"d8e63812-0b58-4ca1-991a-49255f3dd04e\"\u003e{\"inputs\":{\"048ba39a-ab94-4d1f-a0f8-2462de3c044c\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_content_value\":{\"format\":\"text\",\"content\":\"SELECT %param1% FROM flows %param2% %param4% %param5% %param6% LAST %param7% PARAMETERS PROGRESSDETAILSRESOLUTION=60\"}}},\"01ed4652-fe61-4dbf-bc83-f81c62c5563c\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"sourceip as sourceip,COUNT(*) as flowcount,UNIQUECOUNT(category) as categorycount,max(lastpackettime)\"}},\"9d8fc645-5067-4ab0-b151-1d106dad91d6\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"WHERE INOFFENSE(%param3%)\"}},\"eefc9e56-2806-4534-b409-42e25154ecc9\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"GROUP BY sourceip\"}},\"19778fc0-9a30-4b8b-9c21-642495f9eebc\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"ORDER BY max(lastpackettime) DESC\"}},\"ca2d7d7f-fe6d-4a21-b011-7953494c350c\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"LIMIT 10\"}}},\"post_processing_script\":\"link = \\\"\u0026lt;a href=\\\\\\\"https://\\\" + results.get(\\\"content\\\")[\\\"qrhost\\\"] + \\\"/console/ui/offenses/{0}/events?filter={1}%3B%3D%3B%3B{2}\u0026amp;page=1\u0026amp;pagesize=10\\\\\\\" target=\\\\\\\"_blank\\\\\\\"\u0026gt;{3}\u0026lt;/a\u0026gt;\\\"\\n\\nfor event in results.get(\\\"content\\\")[\\\"events\\\"]:\\n  qradar_event = incident.addRow(\\\"qr_top_source_ips\\\")\\n  qradar_event.source_ip = event.sourceip\\n  qradar_event.vulnerability_count = event.vulnerabilityCount\\n  qradar_event.mac = event.macAddress\\n  qradar_event.network = event.network\\n  qradar_event.domain = event.domain\\n  qradar_event.usernames = event.usernamecount\\n  qradar_event.flow_count = event.flowcount\\n  qradar_event.category_count = event.categorycount\\n  qradar_event.reported_time = results.get(\\\"content\\\")[\\\"current_time\\\"]\",\"post_processing_script_language\":\"python\",\"pre_processing_script\":\"inputs.qradar_search_param3 = incident.properties.qradar_id\\ninputs.qradar_query_type = \\\"sourceip\\\"\\ninputs.qradar_label = incident.properties.qradar_destination\\ninputs.soar_incident_id = incident.id\\ninputs.soar_table_name = \\\"qr_top_source_ips\\\"\\n\\n# QRadar graphql search look back time default is 5 days\\ninputs.qradar_search_param7 = \\\"5 days\\\"\\n# If the poller is running and the qr_last_updated_time is changed the\\n# the QRadar graphql look back time will change to 2 days\\nif incident.properties.qr_last_updated_time != incident.create_date:\\n  inputs.qradar_search_param7 = \\\"2 days\\\"\\n# If manual QRadar Update rule is run set the number if days to search to the\\n# user entered number\\nif rule.properties.number_of_days_to_search:\\n  inputs.qradar_search_param7 = str(rule.properties.number_of_days_to_search) + \\\" days\\\"\",\"pre_processing_script_language\":\"python\",\"result_name\":\"\"}\u003c/resilient:function\u003e\u003c/extensionElements\u003e\u003cincoming\u003eSequenceFlow_1bpm9fv\u003c/incoming\u003e\u003coutgoing\u003eSequenceFlow_0ebhn0g\u003c/outgoing\u003e\u003c/serviceTask\u003e\u003csequenceFlow id=\"SequenceFlow_0ebhn0g\" sourceRef=\"ServiceTask_01babo2\" targetRef=\"EndEvent_0dfgyrw\"/\u003e\u003cendEvent id=\"EndEvent_1hcovth\"\u003e\u003cincoming\u003eSequenceFlow_1xf36er\u003c/incoming\u003e\u003c/endEvent\u003e\u003csequenceFlow id=\"SequenceFlow_1xf36er\" sourceRef=\"ServiceTask_0llgtav\" targetRef=\"EndEvent_1hcovth\"/\u003e\u003ctextAnnotation id=\"TextAnnotation_1kxxiyt\"\u003e\u003ctext\u003eStart your workflow here\u003c/text\u003e\u003c/textAnnotation\u003e\u003cassociation id=\"Association_1seuj48\" sourceRef=\"StartEvent_155asxm\" targetRef=\"TextAnnotation_1kxxiyt\"/\u003e\u003c/process\u003e\u003cbpmndi:BPMNDiagram id=\"BPMNDiagram_1\"\u003e\u003cbpmndi:BPMNPlane bpmnElement=\"undefined\" id=\"BPMNPlane_1\"\u003e\u003cbpmndi:BPMNShape bpmnElement=\"StartEvent_155asxm\" id=\"StartEvent_155asxm_di\"\u003e\u003comgdc:Bounds height=\"36\" width=\"36\" x=\"162\" y=\"188\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"0\" width=\"90\" x=\"157\" y=\"223\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNShape bpmnElement=\"TextAnnotation_1kxxiyt\" id=\"TextAnnotation_1kxxiyt_di\"\u003e\u003comgdc:Bounds height=\"30\" width=\"100\" x=\"99\" y=\"254\"/\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"Association_1seuj48\" id=\"Association_1seuj48_di\"\u003e\u003comgdi:waypoint x=\"169\" xsi:type=\"omgdc:Point\" y=\"220\"/\u003e\u003comgdi:waypoint x=\"153\" xsi:type=\"omgdc:Point\" y=\"254\"/\u003e\u003c/bpmndi:BPMNEdge\u003e\u003cbpmndi:BPMNShape bpmnElement=\"ServiceTask_0llgtav\" id=\"ServiceTask_0llgtav_di\"\u003e\u003comgdc:Bounds height=\"80\" width=\"100\" x=\"324\" y=\"166\"/\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNShape bpmnElement=\"EndEvent_0dfgyrw\" id=\"EndEvent_0dfgyrw_di\"\u003e\u003comgdc:Bounds height=\"36\" width=\"36\" x=\"521\" y=\"315\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"13\" width=\"90\" x=\"494\" y=\"354\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"SequenceFlow_1bpm9fv\" id=\"SequenceFlow_1bpm9fv_di\"\u003e\u003comgdi:waypoint x=\"198\" xsi:type=\"omgdc:Point\" y=\"225\"/\u003e\u003comgdi:waypoint x=\"228\" xsi:type=\"omgdc:Point\" y=\"225\"/\u003e\u003comgdi:waypoint x=\"228\" xsi:type=\"omgdc:Point\" y=\"333\"/\u003e\u003comgdi:waypoint x=\"330\" xsi:type=\"omgdc:Point\" y=\"333\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"13\" width=\"90\" x=\"198\" y=\"272.5\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNEdge\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"SequenceFlow_1ykiehr\" id=\"SequenceFlow_1ykiehr_di\"\u003e\u003comgdi:waypoint x=\"198\" xsi:type=\"omgdc:Point\" y=\"206\"/\u003e\u003comgdi:waypoint x=\"324\" xsi:type=\"omgdc:Point\" y=\"206\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"13\" width=\"0\" x=\"261\" y=\"184\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNEdge\u003e\u003cbpmndi:BPMNShape bpmnElement=\"ServiceTask_01babo2\" id=\"ServiceTask_01babo2_di\"\u003e\u003comgdc:Bounds height=\"80\" width=\"100\" x=\"330\" y=\"293\"/\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"SequenceFlow_0ebhn0g\" id=\"SequenceFlow_0ebhn0g_di\"\u003e\u003comgdi:waypoint x=\"430\" xsi:type=\"omgdc:Point\" y=\"333\"/\u003e\u003comgdi:waypoint x=\"521\" xsi:type=\"omgdc:Point\" y=\"333\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"14\" width=\"90\" x=\"430.5\" y=\"311\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNEdge\u003e\u003cbpmndi:BPMNShape bpmnElement=\"EndEvent_1hcovth\" id=\"EndEvent_1hcovth_di\"\u003e\u003comgdc:Bounds height=\"36\" width=\"36\" x=\"521\" y=\"188\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"14\" width=\"0\" x=\"539\" y=\"227\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"SequenceFlow_1xf36er\" id=\"SequenceFlow_1xf36er_di\"\u003e\u003comgdi:waypoint x=\"424\" xsi:type=\"omgdc:Point\" y=\"206\"/\u003e\u003comgdi:waypoint x=\"472\" xsi:type=\"omgdc:Point\" y=\"206\"/\u003e\u003comgdi:waypoint x=\"472\" xsi:type=\"omgdc:Point\" y=\"206\"/\u003e\u003comgdi:waypoint x=\"521\" xsi:type=\"omgdc:Point\" y=\"206\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"14\" width=\"0\" x=\"487\" y=\"199\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNEdge\u003e\u003c/bpmndi:BPMNPlane\u003e\u003c/bpmndi:BPMNDiagram\u003e\u003c/definitions\u003e"
      },
      "content_version": 6,
      "description": "Returns the information about the source IPs associated with the QRadar Offense ID",
      "export_key": "qradar_source_ips",
      "last_modified_by": "d@example.com",
      "last_modified_time": 1666186004134,
      "name": "Example of searching and returning Source IPs information",
      "object_type": "incident",
      "programmatic_name": "qradar_source_ips",
      "tags": [],
      "uuid": "7f26897c-eeda-45ea-8c8a-55045f91c4fa",
      "workflow_id": 63
    },
    {
      "actions": [],
      "content": {
        "version": 11,
        "workflow_id": "qradar_triggered_rules",
        "xml": "\u003c?xml version=\"1.0\" encoding=\"UTF-8\"?\u003e\u003cdefinitions xmlns=\"http://www.omg.org/spec/BPMN/20100524/MODEL\" xmlns:bpmndi=\"http://www.omg.org/spec/BPMN/20100524/DI\" xmlns:omgdc=\"http://www.omg.org/spec/DD/20100524/DC\" xmlns:omgdi=\"http://www.omg.org/spec/DD/20100524/DI\" xmlns:resilient=\"http://resilient.ibm.com/bpmn\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" targetNamespace=\"http://www.camunda.org/test\"\u003e\u003cprocess id=\"qradar_triggered_rules\" isExecutable=\"true\" name=\"Example of fetching contributing rules for Offense\"\u003e\u003cdocumentation\u003eReturns the contributing rules for the Offense\u003c/documentation\u003e\u003cstartEvent id=\"StartEvent_155asxm\"\u003e\u003coutgoing\u003eSequenceFlow_0j7w99n\u003c/outgoing\u003e\u003c/startEvent\u003e\u003cserviceTask id=\"ServiceTask_0j6nll2\" name=\"QRadar Offense Summary\" resilient:type=\"function\"\u003e\u003cextensionElements\u003e\u003cresilient:function uuid=\"0aee36de-8719-4154-a1e7-6455f065bb94\"\u003e{\"inputs\":{},\"post_processing_script\":\"link = \\\"\u0026lt;a href=\\\\\\\"https://\\\" + results.get(\\\"content\\\").get(\\\"qrhost\\\") + \\\"/console/ui/offenses?filter={0}%3B%3D%3B%3B{1}\u0026amp;page=1\u0026amp;pagesize=10\\\\\\\" target=\\\\\\\"_blank\\\\\\\"\u0026gt;{2}\u0026lt;/a\u0026gt;\\\"\\n\\nfor event in results.get(\\\"content\\\").get(\\\"rules_data\\\"):\\n  qradar_event = incident.addRow(\\\"qr_triggered_rules\\\")\\n  qradar_event.rule_name = link.format(\\\"rules\\\", event.id, event.name)\\n  qradar_event.rule_group = \\\", \\\".join(list(map(lambda x: x.name, list(filter(lambda x: x.name is not None, event.groups))))) if len(event.groups) \u0026gt; 0 else \\\"\\\"\\n  qradar_event.rule_type = event.type\\n  qradar_event.enabled = \\\"True\\\" if event.enabled else \\\"False\\\"\\n  qradar_event.response = \\\"Yes\\\" if event.responses.newEvents or event.responses.email or event.responses.log or event.responses.addToReferenceData or event.responses.addToReferenceSet or event.responses.removeFromReferenceData or event.responses.removeFromReferenceSet or event.responses.notify or event.responses.notifySeverityOverride or event.responses.selectiveForwardingResponse or event.responses.customAction else \\\"No\\\"\\n  qradar_event.date_created = int(event.creationDate)\\n  qradar_event.last_modified = int(event.modificationDate)\\n  qradar_event.reported_time = results.get(\\\"content\\\").get(\\\"current_time\\\")\",\"post_processing_script_language\":\"python\",\"pre_processing_script\":\"inputs.qradar_offense_id = incident.properties.qradar_id\\ninputs.qradar_query_type = \\\"offenserules\\\"\\ninputs.qradar_label = incident.properties.qradar_destination\\ninputs.soar_table_name = \\\"qr_triggered_rules\\\"\\ninputs.soar_incident_id = incident.id\",\"pre_processing_script_language\":\"python\",\"result_name\":\"\"}\u003c/resilient:function\u003e\u003c/extensionElements\u003e\u003cincoming\u003eSequenceFlow_0j7w99n\u003c/incoming\u003e\u003coutgoing\u003eSequenceFlow_1emdbk1\u003c/outgoing\u003e\u003c/serviceTask\u003e\u003csequenceFlow id=\"SequenceFlow_0j7w99n\" sourceRef=\"StartEvent_155asxm\" targetRef=\"ServiceTask_0j6nll2\"/\u003e\u003cendEvent id=\"EndEvent_1njimqc\"\u003e\u003cincoming\u003eSequenceFlow_1emdbk1\u003c/incoming\u003e\u003c/endEvent\u003e\u003csequenceFlow id=\"SequenceFlow_1emdbk1\" sourceRef=\"ServiceTask_0j6nll2\" targetRef=\"EndEvent_1njimqc\"/\u003e\u003ctextAnnotation id=\"TextAnnotation_1kxxiyt\"\u003e\u003ctext\u003eStart your workflow here\u003c/text\u003e\u003c/textAnnotation\u003e\u003cassociation id=\"Association_1seuj48\" sourceRef=\"StartEvent_155asxm\" targetRef=\"TextAnnotation_1kxxiyt\"/\u003e\u003c/process\u003e\u003cbpmndi:BPMNDiagram id=\"BPMNDiagram_1\"\u003e\u003cbpmndi:BPMNPlane bpmnElement=\"undefined\" id=\"BPMNPlane_1\"\u003e\u003cbpmndi:BPMNShape bpmnElement=\"StartEvent_155asxm\" id=\"StartEvent_155asxm_di\"\u003e\u003comgdc:Bounds height=\"36\" width=\"36\" x=\"162\" y=\"188\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"0\" width=\"90\" x=\"157\" y=\"223\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNShape bpmnElement=\"TextAnnotation_1kxxiyt\" id=\"TextAnnotation_1kxxiyt_di\"\u003e\u003comgdc:Bounds height=\"30\" width=\"100\" x=\"99\" y=\"254\"/\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"Association_1seuj48\" id=\"Association_1seuj48_di\"\u003e\u003comgdi:waypoint x=\"169\" xsi:type=\"omgdc:Point\" y=\"220\"/\u003e\u003comgdi:waypoint x=\"153\" xsi:type=\"omgdc:Point\" y=\"254\"/\u003e\u003c/bpmndi:BPMNEdge\u003e\u003cbpmndi:BPMNShape bpmnElement=\"ServiceTask_0j6nll2\" id=\"ServiceTask_0j6nll2_di\"\u003e\u003comgdc:Bounds height=\"80\" width=\"100\" x=\"365\" y=\"166\"/\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"SequenceFlow_0j7w99n\" id=\"SequenceFlow_0j7w99n_di\"\u003e\u003comgdi:waypoint x=\"198\" xsi:type=\"omgdc:Point\" y=\"206\"/\u003e\u003comgdi:waypoint x=\"365\" xsi:type=\"omgdc:Point\" y=\"206\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"13\" width=\"0\" x=\"281.5\" y=\"184\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNEdge\u003e\u003cbpmndi:BPMNShape bpmnElement=\"EndEvent_1njimqc\" id=\"EndEvent_1njimqc_di\"\u003e\u003comgdc:Bounds height=\"36\" width=\"36\" x=\"678.7668269230769\" y=\"188\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"13\" width=\"0\" x=\"696.7668269230769\" y=\"227\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"SequenceFlow_1emdbk1\" id=\"SequenceFlow_1emdbk1_di\"\u003e\u003comgdi:waypoint x=\"465\" xsi:type=\"omgdc:Point\" y=\"206\"/\u003e\u003comgdi:waypoint x=\"679\" xsi:type=\"omgdc:Point\" y=\"206\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"13\" width=\"0\" x=\"572\" y=\"184\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNEdge\u003e\u003c/bpmndi:BPMNPlane\u003e\u003c/bpmndi:BPMNDiagram\u003e\u003c/definitions\u003e"
      },
      "content_version": 11,
      "description": "Returns the contributing rules for the Offense",
      "export_key": "qradar_triggered_rules",
      "last_modified_by": "d@example.com",
      "last_modified_time": 1666184644736,
      "name": "Example of fetching contributing rules for Offense",
      "object_type": "incident",
      "programmatic_name": "qradar_triggered_rules",
      "tags": [],
      "uuid": "6b47dac2-3b5e-4916-a57b-59c5a6dbed0e",
      "workflow_id": 61
    },
    {
      "actions": [],
      "content": {
        "version": 6,
        "workflow_id": "qradar_assets_information",
        "xml": "\u003c?xml version=\"1.0\" encoding=\"UTF-8\"?\u003e\u003cdefinitions xmlns=\"http://www.omg.org/spec/BPMN/20100524/MODEL\" xmlns:bpmndi=\"http://www.omg.org/spec/BPMN/20100524/DI\" xmlns:omgdc=\"http://www.omg.org/spec/DD/20100524/DC\" xmlns:omgdi=\"http://www.omg.org/spec/DD/20100524/DI\" xmlns:resilient=\"http://resilient.ibm.com/bpmn\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" targetNamespace=\"http://www.camunda.org/test\"\u003e\u003cprocess id=\"qradar_assets_information\" isExecutable=\"true\" name=\"Example of searching and returning Assets information\"\u003e\u003cdocumentation\u003eReturns the information on the Assets associated with the QRadar Offense ID\u003c/documentation\u003e\u003cstartEvent id=\"StartEvent_155asxm\"\u003e\u003coutgoing\u003eSequenceFlow_0jajzph\u003c/outgoing\u003e\u003c/startEvent\u003e\u003cserviceTask id=\"ServiceTask_0w4xfyt\" name=\"QRadar Offense Summary\" resilient:type=\"function\"\u003e\u003cextensionElements\u003e\u003cresilient:function uuid=\"0aee36de-8719-4154-a1e7-6455f065bb94\"\u003e{\"inputs\":{},\"post_processing_script\":\"if results.get(\\\"content\\\"):\\n  for event in results[\\\"content\\\"].get(\\\"assets\\\"):\\n    qradar_event = incident.addRow(\\\"qr_assets\\\")\\n    qradar_event.asset_id = event.id\\n    qradar_event.asset_name = event.name\\n    qradar_event.ip_address = event.sourceip\\n    qradar_event.operating_system = event.osid\\n    qradar_event.aggregated_cvss = event.riskScoreSum\\n    qradar_event.vulnerabilities = str(event.vulnerabilityCount)\\n    qradar_event.last_user = event.users[0].username if len(event.users) \u0026gt; 0 and event.users[0].username else \\\"\\\"\\n    qradar_event.last_user_seen = int(event.users[0].lastSeenProfiler) if len(event.users) \u0026gt; 0 and event.users[0].lastSeenProfiler else \\\"\\\"\\n    qradar_event.reported_time = results[\\\"content\\\"].get(\\\"current_time\\\")\",\"post_processing_script_language\":\"python\",\"pre_processing_script\":\"inputs.qradar_query_type = \\\"offenseassets\\\"\\ninputs.qradar_offense_id = incident.properties.qradar_id\\ninputs.qradar_label = incident.properties.qradar_destination\\ninputs.soar_table_name = \\\"qr_assets\\\"\\ninputs.soar_incident_id = incident.id\",\"pre_processing_script_language\":\"python\",\"result_name\":\"\"}\u003c/resilient:function\u003e\u003c/extensionElements\u003e\u003cincoming\u003eSequenceFlow_0jajzph\u003c/incoming\u003e\u003coutgoing\u003eSequenceFlow_03x7why\u003c/outgoing\u003e\u003c/serviceTask\u003e\u003cendEvent id=\"EndEvent_00nirf1\"\u003e\u003cincoming\u003eSequenceFlow_03x7why\u003c/incoming\u003e\u003c/endEvent\u003e\u003csequenceFlow id=\"SequenceFlow_03x7why\" sourceRef=\"ServiceTask_0w4xfyt\" targetRef=\"EndEvent_00nirf1\"/\u003e\u003csequenceFlow id=\"SequenceFlow_0jajzph\" sourceRef=\"StartEvent_155asxm\" targetRef=\"ServiceTask_0w4xfyt\"/\u003e\u003ctextAnnotation id=\"TextAnnotation_1kxxiyt\"\u003e\u003ctext\u003eStart your workflow here\u003c/text\u003e\u003c/textAnnotation\u003e\u003cassociation id=\"Association_1seuj48\" sourceRef=\"StartEvent_155asxm\" targetRef=\"TextAnnotation_1kxxiyt\"/\u003e\u003c/process\u003e\u003cbpmndi:BPMNDiagram id=\"BPMNDiagram_1\"\u003e\u003cbpmndi:BPMNPlane bpmnElement=\"undefined\" id=\"BPMNPlane_1\"\u003e\u003cbpmndi:BPMNShape bpmnElement=\"StartEvent_155asxm\" id=\"StartEvent_155asxm_di\"\u003e\u003comgdc:Bounds height=\"36\" width=\"36\" x=\"164\" y=\"158\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"0\" width=\"90\" x=\"159\" y=\"193\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNShape bpmnElement=\"TextAnnotation_1kxxiyt\" id=\"TextAnnotation_1kxxiyt_di\"\u003e\u003comgdc:Bounds height=\"30\" width=\"100\" x=\"99\" y=\"254\"/\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"Association_1seuj48\" id=\"Association_1seuj48_di\"\u003e\u003comgdi:waypoint x=\"173\" xsi:type=\"omgdc:Point\" y=\"191\"/\u003e\u003comgdi:waypoint x=\"151\" xsi:type=\"omgdc:Point\" y=\"254\"/\u003e\u003c/bpmndi:BPMNEdge\u003e\u003cbpmndi:BPMNShape bpmnElement=\"ServiceTask_0w4xfyt\" id=\"ServiceTask_0w4xfyt_di\"\u003e\u003comgdc:Bounds height=\"80\" width=\"100\" x=\"340\" y=\"136\"/\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNShape bpmnElement=\"EndEvent_00nirf1\" id=\"EndEvent_00nirf1_di\"\u003e\u003comgdc:Bounds height=\"36\" width=\"36\" x=\"629\" y=\"158\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"13\" width=\"0\" x=\"647\" y=\"197\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"SequenceFlow_03x7why\" id=\"SequenceFlow_03x7why_di\"\u003e\u003comgdi:waypoint x=\"440\" xsi:type=\"omgdc:Point\" y=\"176\"/\u003e\u003comgdi:waypoint x=\"629\" xsi:type=\"omgdc:Point\" y=\"176\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"13\" width=\"0\" x=\"534.5\" y=\"154\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNEdge\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"SequenceFlow_0jajzph\" id=\"SequenceFlow_0jajzph_di\"\u003e\u003comgdi:waypoint x=\"200\" xsi:type=\"omgdc:Point\" y=\"176\"/\u003e\u003comgdi:waypoint x=\"269\" xsi:type=\"omgdc:Point\" y=\"176\"/\u003e\u003comgdi:waypoint x=\"269\" xsi:type=\"omgdc:Point\" y=\"176\"/\u003e\u003comgdi:waypoint x=\"340\" xsi:type=\"omgdc:Point\" y=\"176\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"13\" width=\"0\" x=\"284\" y=\"169.5\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNEdge\u003e\u003c/bpmndi:BPMNPlane\u003e\u003c/bpmndi:BPMNDiagram\u003e\u003c/definitions\u003e"
      },
      "content_version": 6,
      "description": "Returns the information on the Assets associated with the QRadar Offense ID",
      "export_key": "qradar_assets_information",
      "last_modified_by": "d@example.com",
      "last_modified_time": 1666185491207,
      "name": "Example of searching and returning Assets information",
      "object_type": "incident",
      "programmatic_name": "qradar_assets_information",
      "tags": [],
      "uuid": "a617ff13-51d0-4697-b729-7590252740bd",
      "workflow_id": 62
    },
    {
      "actions": [],
      "content": {
        "version": 4,
        "workflow_id": "example_of_searching_qradar_top_events_using_offense_id",
        "xml": "\u003c?xml version=\"1.0\" encoding=\"UTF-8\"?\u003e\u003cdefinitions xmlns=\"http://www.omg.org/spec/BPMN/20100524/MODEL\" xmlns:bpmndi=\"http://www.omg.org/spec/BPMN/20100524/DI\" xmlns:omgdc=\"http://www.omg.org/spec/DD/20100524/DC\" xmlns:omgdi=\"http://www.omg.org/spec/DD/20100524/DI\" xmlns:resilient=\"http://resilient.ibm.com/bpmn\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" targetNamespace=\"http://www.camunda.org/test\"\u003e\u003cprocess id=\"example_of_searching_qradar_top_events_using_offense_id\" isExecutable=\"true\" name=\"Example of searching QRadar Top Events using offense id\"\u003e\u003cdocumentation\u003eUse the qradar_id field of the incident to searchQRadar events, and update the data table, qr_offense_top_events, with the top 10 events.\u003c/documentation\u003e\u003cstartEvent id=\"StartEvent_155asxm\"\u003e\u003coutgoing\u003eSequenceFlow_1rcy3j4\u003c/outgoing\u003e\u003c/startEvent\u003e\u003cserviceTask id=\"ServiceTask_0ulfeez\" name=\"QRadar Top Events\" resilient:type=\"function\"\u003e\u003cextensionElements\u003e\u003cresilient:function uuid=\"d8e63812-0b58-4ca1-991a-49255f3dd04e\"\u003e{\"inputs\":{\"048ba39a-ab94-4d1f-a0f8-2462de3c044c\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_content_value\":{\"format\":\"text\",\"content\":\"SELECT %param1% FROM events %param2% %param4% %param5% %param6% LAST %param7% PARAMETERS PROGRESSDETAILSRESOLUTION=60\"}}},\"01ed4652-fe61-4dbf-bc83-f81c62c5563c\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"QIDNAME(qid) as event_name,categoryname(category) as category_name,logsourcename(logsourceid) as logsourcename,username,magnitude,destinationip,sourceip,eventcount,starttime as event_time \"}},\"9d8fc645-5067-4ab0-b151-1d106dad91d6\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"WHERE INOFFENSE(%param3%)\"}},\"19778fc0-9a30-4b8b-9c21-642495f9eebc\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"ORDER BY starttime\"}},\"ca2d7d7f-fe6d-4a21-b011-7953494c350c\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"LIMIT 10\"}}},\"post_processing_script\":\"link = \\\"\u0026lt;a href=\\\\\\\"https://\\\" + results.get(\\\"content\\\")[\\\"qrhost\\\"] + \\\"/console/ui/offenses/{0}/events?filter={1}%3B%3D%3B%3B{2}\u0026amp;page=1\u0026amp;pagesize=10\\\\\\\" target=\\\\\\\"_blank\\\\\\\"\u0026gt;{3}\u0026lt;/a\u0026gt;\\\"\\n\\nfor event in results.get(\\\"content\\\")[\\\"events\\\"]:\\n  qradar_event = incident.addRow(\\\"qr_offense_top_events\\\")\\n  qradar_event.event_name = link.format(results.get(\\\"content\\\")[\\\"offenseid\\\"], \\\"event_name\\\", event.event_name, event.event_name)\\n  qradar_event.category = link.format(results.get(\\\"content\\\")[\\\"offenseid\\\"], \\\"category_name\\\", event.category_name, event.category_name)\\n  qradar_event.source_ip = link.format(results.get(\\\"content\\\")[\\\"offenseid\\\"], \\\"sourceip\\\", event.sourceip, event.sourceip)\\n  qradar_event.destination_ip = link.format(results.get(\\\"content\\\")[\\\"offenseid\\\"], \\\"destinationip\\\", event.destinationip, event.destinationip)\\n  qradar_event.log_source = link.format(results.get(\\\"content\\\")[\\\"offenseid\\\"], \\\"log_source_name\\\", event.logsourcename, event.logsourcename)\\n  qradar_event.event_count = link.format(results.get(\\\"content\\\")[\\\"offenseid\\\"], \\\"event_name\\\", event.event_name, event.eventcount)\\n  qradar_event.event_time = int(event.event_time)\\n  qradar_event.magnitude = event.magnitude\\n  qradar_event.username = event.username\\n  qradar_event.reported_time = results.get(\\\"content\\\")[\\\"current_time\\\"]\",\"post_processing_script_language\":\"python\",\"pre_processing_script\":\"inputs.qradar_search_param3 = incident.properties.qradar_id\\ninputs.qradar_query_type = \\\"topevents\\\"\\ninputs.qradar_label = incident.properties.qradar_destination\\ninputs.soar_incident_id = incident.id\\ninputs.soar_table_name = \\\"qr_offense_top_events\\\"\\n\\n# QRadar graphql search look back time default is 5 days\\ninputs.qradar_search_param7 = \\\"5 days\\\"\\n# If the poller is running and the qr_last_updated_time is changed the\\n# the QRadar graphql look back time will change to 2 days\\nif incident.properties.qr_last_updated_time != incident.create_date:\\n  inputs.qradar_search_param7 = \\\"2 days\\\"\\n# If manual QRadar Update rule is run set the number if days to search to the\\n# user entered number\\nif rule.properties.number_of_days_to_search:\\n  inputs.qradar_search_param7 = str(rule.properties.number_of_days_to_search) + \\\" days\\\"\",\"pre_processing_script_language\":\"python\",\"result_name\":\"\"}\u003c/resilient:function\u003e\u003c/extensionElements\u003e\u003cincoming\u003eSequenceFlow_1rcy3j4\u003c/incoming\u003e\u003coutgoing\u003eSequenceFlow_0bwc288\u003c/outgoing\u003e\u003c/serviceTask\u003e\u003csequenceFlow id=\"SequenceFlow_1rcy3j4\" sourceRef=\"StartEvent_155asxm\" targetRef=\"ServiceTask_0ulfeez\"/\u003e\u003cendEvent id=\"EndEvent_031o32j\"\u003e\u003cincoming\u003eSequenceFlow_0bwc288\u003c/incoming\u003e\u003c/endEvent\u003e\u003csequenceFlow id=\"SequenceFlow_0bwc288\" sourceRef=\"ServiceTask_0ulfeez\" targetRef=\"EndEvent_031o32j\"/\u003e\u003ctextAnnotation id=\"TextAnnotation_1kxxiyt\"\u003e\u003ctext\u003eStart your workflow here\u003c/text\u003e\u003c/textAnnotation\u003e\u003cassociation id=\"Association_1seuj48\" sourceRef=\"StartEvent_155asxm\" targetRef=\"TextAnnotation_1kxxiyt\"/\u003e\u003ctextAnnotation id=\"TextAnnotation_1bqz36v\"\u003e\u003ctext\u003eReturn results in qradar_offense_top_events datatable\u003c/text\u003e\u003c/textAnnotation\u003e\u003cassociation id=\"Association_0cmw5gy\" sourceRef=\"ServiceTask_0ulfeez\" targetRef=\"TextAnnotation_1bqz36v\"/\u003e\u003c/process\u003e\u003cbpmndi:BPMNDiagram id=\"BPMNDiagram_1\"\u003e\u003cbpmndi:BPMNPlane bpmnElement=\"undefined\" id=\"BPMNPlane_1\"\u003e\u003cbpmndi:BPMNShape bpmnElement=\"StartEvent_155asxm\" id=\"StartEvent_155asxm_di\"\u003e\u003comgdc:Bounds height=\"36\" width=\"36\" x=\"164\" y=\"96\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"0\" width=\"90\" x=\"159\" y=\"131\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNShape bpmnElement=\"TextAnnotation_1kxxiyt\" id=\"TextAnnotation_1kxxiyt_di\"\u003e\u003comgdc:Bounds height=\"30\" width=\"100\" x=\"-28\" y=\"99\"/\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"Association_1seuj48\" id=\"Association_1seuj48_di\"\u003e\u003comgdi:waypoint x=\"164\" xsi:type=\"omgdc:Point\" y=\"114\"/\u003e\u003comgdi:waypoint x=\"72\" xsi:type=\"omgdc:Point\" y=\"114\"/\u003e\u003c/bpmndi:BPMNEdge\u003e\u003cbpmndi:BPMNShape bpmnElement=\"ServiceTask_0ulfeez\" id=\"ServiceTask_0ulfeez_di\"\u003e\u003comgdc:Bounds height=\"80\" width=\"100\" x=\"348\" y=\"74\"/\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"SequenceFlow_1rcy3j4\" id=\"SequenceFlow_1rcy3j4_di\"\u003e\u003comgdi:waypoint x=\"200\" xsi:type=\"omgdc:Point\" y=\"114\"/\u003e\u003comgdi:waypoint x=\"348\" xsi:type=\"omgdc:Point\" y=\"114\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"13\" width=\"0\" x=\"274\" y=\"92.5\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNEdge\u003e\u003cbpmndi:BPMNShape bpmnElement=\"EndEvent_031o32j\" id=\"EndEvent_031o32j_di\"\u003e\u003comgdc:Bounds height=\"36\" width=\"36\" x=\"597.0289855072464\" y=\"96\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"13\" width=\"0\" x=\"615.0289855072464\" y=\"135\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"SequenceFlow_0bwc288\" id=\"SequenceFlow_0bwc288_di\"\u003e\u003comgdi:waypoint x=\"448\" xsi:type=\"omgdc:Point\" y=\"114\"/\u003e\u003comgdi:waypoint x=\"597\" xsi:type=\"omgdc:Point\" y=\"114\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"13\" width=\"0\" x=\"522.5\" y=\"92.5\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNEdge\u003e\u003cbpmndi:BPMNShape bpmnElement=\"TextAnnotation_1bqz36v\" id=\"TextAnnotation_1bqz36v_di\"\u003e\u003comgdc:Bounds height=\"64\" width=\"137\" x=\"440\" y=\"-4\"/\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"Association_0cmw5gy\" id=\"Association_0cmw5gy_di\"\u003e\u003comgdi:waypoint x=\"443\" xsi:type=\"omgdc:Point\" y=\"79\"/\u003e\u003comgdi:waypoint x=\"467\" xsi:type=\"omgdc:Point\" y=\"60\"/\u003e\u003c/bpmndi:BPMNEdge\u003e\u003c/bpmndi:BPMNPlane\u003e\u003c/bpmndi:BPMNDiagram\u003e\u003c/definitions\u003e"
      },
      "content_version": 4,
      "description": "Use the qradar_id field of the incident to searchQRadar events, and update the data table, qr_offense_top_events, with the top 10 events.",
      "export_key": "example_of_searching_qradar_top_events_using_offense_id",
      "last_modified_by": "d@example.com",
      "last_modified_time": 1666186263606,
      "name": "Example of searching QRadar Top Events using offense id",
      "object_type": "incident",
      "programmatic_name": "example_of_searching_qradar_top_events_using_offense_id",
      "tags": [],
      "uuid": "0bd5327c-9ff5-4f70-9392-6eac92cb3304",
      "workflow_id": 58
    },
    {
      "actions": [],
      "content": {
        "version": 10,
        "workflow_id": "qradar_offense_summary",
        "xml": "\u003c?xml version=\"1.0\" encoding=\"UTF-8\"?\u003e\u003cdefinitions xmlns=\"http://www.omg.org/spec/BPMN/20100524/MODEL\" xmlns:bpmndi=\"http://www.omg.org/spec/BPMN/20100524/DI\" xmlns:omgdc=\"http://www.omg.org/spec/DD/20100524/DC\" xmlns:omgdi=\"http://www.omg.org/spec/DD/20100524/DI\" xmlns:resilient=\"http://resilient.ibm.com/bpmn\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" targetNamespace=\"http://www.camunda.org/test\"\u003e\u003cprocess id=\"qradar_offense_summary\" isExecutable=\"true\" name=\"QRadar Offense Summary\"\u003e\u003cdocumentation\u003eReturns information about the Offense\u003c/documentation\u003e\u003cstartEvent id=\"StartEvent_155asxm\"\u003e\u003coutgoing\u003eSequenceFlow_05iwsyi\u003c/outgoing\u003e\u003c/startEvent\u003e\u003cserviceTask id=\"ServiceTask_1apebpb\" name=\"QRadar Offense Summary\" resilient:type=\"function\"\u003e\u003cextensionElements\u003e\u003cresilient:function uuid=\"0aee36de-8719-4154-a1e7-6455f065bb94\"\u003e{\"inputs\":{},\"post_processing_script\":\"link = \\\"\u0026lt;a href=\\\\\\\"https://\\\"+results.get(\\\"content\\\").get(\\\"qrhost\\\")+\\\"/console/ui/offenses/{0}{1}\\\\\\\" target=\\\\\\\"_blank\\\\\\\"\u0026gt;{2}\u0026lt;/a\u0026gt;\\\"\\n  \\nincident.qr_offense_index_type = results.get(\\\"content\\\")[\\\"offense\\\"][\\\"offenseType\\\"][\\\"name\\\"]\\nincident.qr_offense_index_value = results.get(\\\"content\\\")[\\\"offense\\\"][\\\"offenseSource\\\"]\\nincident.qr_offense_source = results.get(\\\"content\\\")[\\\"offense\\\"][\\\"offenseSource\\\"]\\nincident.qr_source_ip_count = link.format(results.get(\\\"content\\\")[\\\"offenseid\\\"], \\\"\\\", results.get(\\\"content\\\")[\\\"offense\\\"][\\\"sourceCount\\\"])\\nincident.qr_destination_ip_count = link.format(results.get(\\\"content\\\")[\\\"offenseid\\\"], \\\"\\\", results.get(\\\"content\\\")[\\\"offense\\\"][\\\"remoteDestinationCount\\\"] + results.get(\\\"content\\\")[\\\"offense\\\"][\\\"localDestinationCount\\\"])\\nincident.qr_event_count = link.format(results.get(\\\"content\\\")[\\\"offenseid\\\"], \\\"/events?page=1\u0026amp;pagesize=10\\\", results.get(\\\"content\\\")[\\\"offense\\\"][\\\"eventCount\\\"])\\nincident.qr_flow_count =  link.format(results.get(\\\"content\\\")[\\\"offenseid\\\"], \\\"/flows?page=1\u0026amp;pagesize=10\\\", results.get(\\\"content\\\")[\\\"offense\\\"][\\\"flowCount\\\"])\\nincident.qr_assigned = link.format(\\\"\\\", \\\"?filter=status%3B%3D%3BOpen%3BOPEN\u0026amp;filter=assignedTo%3B%3D%3B%3B\\\" + (results.get(\\\"content\\\")[\\\"offense\\\"][\\\"assignedTo\\\"] if results.get(\\\"content\\\")[\\\"offense\\\"][\\\"assignedTo\\\"] else \\\"\\\") + \\\"\u0026amp;page=1\u0026amp;pagesize=10\\\", results.get(\\\"content\\\")[\\\"offense\\\"][\\\"assignedTo\\\"]) if results.get(\\\"content\\\")[\\\"offense\\\"][\\\"assignedTo\\\"] else \\\"Unassigned\\\"\\nincident.qr_magnitude = link.format(results.get(\\\"content\\\")[\\\"offenseid\\\"], \\\"\\\", results.get(\\\"content\\\")[\\\"offense\\\"][\\\"magnitude\\\"])\\nincident.qr_credibility = link.format(results.get(\\\"content\\\")[\\\"offenseid\\\"], \\\"\\\", results.get(\\\"content\\\")[\\\"offense\\\"][\\\"credibility\\\"])\\nincident.qr_severity = link.format(results.get(\\\"content\\\")[\\\"offenseid\\\"], \\\"\\\", results.get(\\\"content\\\")[\\\"offense\\\"][\\\"severity\\\"])\\nincident.qr_relevance = link.format(results.get(\\\"content\\\")[\\\"offenseid\\\"], \\\"\\\", results.get(\\\"content\\\")[\\\"offense\\\"][\\\"relevance\\\"])\\nincident.qr_offense_status = results.get(\\\"content\\\")[\\\"offense\\\"][\\\"status\\\"]\\nincident.qr_offense_domain = \\\"Default Domain\\\"\\nif results.get(\\\"content\\\")[\\\"offense\\\"][\\\"domain\\\"]:\\n  incident.qr_offense_domain = results.get(\\\"content\\\")[\\\"offense\\\"][\\\"domain\\\"][\\\"name\\\"]\\n  \\nincident.qr_offense_start_time = int(results.get(\\\"content\\\")[\\\"offense\\\"][\\\"startTime\\\"])\\nincident.qr_offense_last_updated_time = int(results.get(\\\"content\\\")[\\\"offense\\\"][\\\"lastUpdatedTime\\\"])\",\"post_processing_script_language\":\"python\",\"pre_processing_script\":\"inputs.qradar_offense_id = incident.properties.qradar_id\\ninputs.qradar_query_type = \\\"offensesummary\\\"\\ninputs.qradar_label = incident.properties.qradar_destination\",\"pre_processing_script_language\":\"python\",\"result_name\":\"\"}\u003c/resilient:function\u003e\u003c/extensionElements\u003e\u003cincoming\u003eSequenceFlow_05iwsyi\u003c/incoming\u003e\u003coutgoing\u003eSequenceFlow_1ajef8g\u003c/outgoing\u003e\u003c/serviceTask\u003e\u003csequenceFlow id=\"SequenceFlow_05iwsyi\" sourceRef=\"StartEvent_155asxm\" targetRef=\"ServiceTask_1apebpb\"/\u003e\u003cendEvent id=\"EndEvent_0l7mn1m\"\u003e\u003cincoming\u003eSequenceFlow_1ajef8g\u003c/incoming\u003e\u003c/endEvent\u003e\u003csequenceFlow id=\"SequenceFlow_1ajef8g\" sourceRef=\"ServiceTask_1apebpb\" targetRef=\"EndEvent_0l7mn1m\"/\u003e\u003ctextAnnotation id=\"TextAnnotation_1kxxiyt\"\u003e\u003ctext\u003eStart your workflow here\u003c/text\u003e\u003c/textAnnotation\u003e\u003cassociation id=\"Association_1seuj48\" sourceRef=\"StartEvent_155asxm\" targetRef=\"TextAnnotation_1kxxiyt\"/\u003e\u003c/process\u003e\u003cbpmndi:BPMNDiagram id=\"BPMNDiagram_1\"\u003e\u003cbpmndi:BPMNPlane bpmnElement=\"undefined\" id=\"BPMNPlane_1\"\u003e\u003cbpmndi:BPMNShape bpmnElement=\"StartEvent_155asxm\" id=\"StartEvent_155asxm_di\"\u003e\u003comgdc:Bounds height=\"36\" width=\"36\" x=\"162\" y=\"188\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"0\" width=\"90\" x=\"157\" y=\"223\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNShape bpmnElement=\"TextAnnotation_1kxxiyt\" id=\"TextAnnotation_1kxxiyt_di\"\u003e\u003comgdc:Bounds height=\"30\" width=\"100\" x=\"99\" y=\"254\"/\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"Association_1seuj48\" id=\"Association_1seuj48_di\"\u003e\u003comgdi:waypoint x=\"169\" xsi:type=\"omgdc:Point\" y=\"220\"/\u003e\u003comgdi:waypoint x=\"153\" xsi:type=\"omgdc:Point\" y=\"254\"/\u003e\u003c/bpmndi:BPMNEdge\u003e\u003cbpmndi:BPMNShape bpmnElement=\"ServiceTask_1apebpb\" id=\"ServiceTask_1apebpb_di\"\u003e\u003comgdc:Bounds height=\"80\" width=\"100\" x=\"331\" y=\"166\"/\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"SequenceFlow_05iwsyi\" id=\"SequenceFlow_05iwsyi_di\"\u003e\u003comgdi:waypoint x=\"198\" xsi:type=\"omgdc:Point\" y=\"206\"/\u003e\u003comgdi:waypoint x=\"331\" xsi:type=\"omgdc:Point\" y=\"206\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"13\" width=\"0\" x=\"264.5\" y=\"184\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNEdge\u003e\u003cbpmndi:BPMNShape bpmnElement=\"EndEvent_0l7mn1m\" id=\"EndEvent_0l7mn1m_di\"\u003e\u003comgdc:Bounds height=\"36\" width=\"36\" x=\"587\" y=\"188\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"13\" width=\"0\" x=\"605\" y=\"227\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"SequenceFlow_1ajef8g\" id=\"SequenceFlow_1ajef8g_di\"\u003e\u003comgdi:waypoint x=\"431\" xsi:type=\"omgdc:Point\" y=\"206\"/\u003e\u003comgdi:waypoint x=\"587\" xsi:type=\"omgdc:Point\" y=\"206\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"13\" width=\"0\" x=\"509\" y=\"184\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNEdge\u003e\u003c/bpmndi:BPMNPlane\u003e\u003c/bpmndi:BPMNDiagram\u003e\u003c/definitions\u003e"
      },
      "content_version": 10,
      "description": "Returns information about the Offense",
      "export_key": "qradar_offense_summary",
      "last_modified_by": "d@example.com",
      "last_modified_time": 1666186273062,
      "name": "QRadar Offense Summary",
      "object_type": "incident",
      "programmatic_name": "qradar_offense_summary",
      "tags": [],
      "uuid": "c136a786-5d9f-4a80-bf1d-dda574ea75ba",
      "workflow_id": 60
    },
    {
      "actions": [],
      "content": {
        "version": 6,
        "workflow_id": "example_of_searching_qradar_flows_using_offense_id",
        "xml": "\u003c?xml version=\"1.0\" encoding=\"UTF-8\"?\u003e\u003cdefinitions xmlns=\"http://www.omg.org/spec/BPMN/20100524/MODEL\" xmlns:bpmndi=\"http://www.omg.org/spec/BPMN/20100524/DI\" xmlns:omgdc=\"http://www.omg.org/spec/DD/20100524/DC\" xmlns:omgdi=\"http://www.omg.org/spec/DD/20100524/DI\" xmlns:resilient=\"http://resilient.ibm.com/bpmn\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" targetNamespace=\"http://www.camunda.org/test\"\u003e\u003cprocess id=\"example_of_searching_qradar_flows_using_offense_id\" isExecutable=\"true\" name=\"Example of searching QRadar Flows using offense id\"\u003e\u003cdocumentation\u003eUse the qradar_id field of the incident to searchQRadar events, and update the data table, qr_offense_top_events, with the top 10 events.\u003c/documentation\u003e\u003cstartEvent id=\"StartEvent_155asxm\"\u003e\u003coutgoing\u003eSequenceFlow_1f2h23f\u003c/outgoing\u003e\u003c/startEvent\u003e\u003cserviceTask id=\"ServiceTask_0zpihlh\" name=\"QRadar Top Events\" resilient:type=\"function\"\u003e\u003cextensionElements\u003e\u003cresilient:function uuid=\"d8e63812-0b58-4ca1-991a-49255f3dd04e\"\u003e{\"inputs\":{\"048ba39a-ab94-4d1f-a0f8-2462de3c044c\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_content_value\":{\"format\":\"text\",\"content\":\"SELECT %param1% FROM flows %param2% %param4% LAST %param7% PARAMETERS PROGRESSDETAILSRESOLUTION=60\"}}},\"01ed4652-fe61-4dbf-bc83-f81c62c5563c\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"APPLICATIONNAME(applicationid) as \u0027Application\u0027, sourceip, sourceport, destinationip, destinationport, PROTOCOLNAME(protocolid) as \u0027Protocol\u0027, firstpackettime AS \u0027FirstPacketTime\u0027, sourcebytes, sourcepackets, destinationbytes, destinationpackets\"}},\"9d8fc645-5067-4ab0-b151-1d106dad91d6\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"WHERE INOFFENSE(%param3%)\"}},\"eefc9e56-2806-4534-b409-42e25154ecc9\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"LIMIT 10\"}}},\"post_processing_script\":\"link = \\\"\u0026lt;a href=\\\\\\\"https://\\\" + results.get(\\\"content\\\")[\\\"qrhost\\\"] + \\\"/console/ui/offenses/{0}/flows?filter={1}%3B%3D%3B%3B{2}\u0026amp;page=1\u0026amp;pagesize=10\\\\\\\" target=\\\\\\\"_blank\\\\\\\"\u0026gt;{3}\u0026lt;/a\u0026gt;\\\"\\n\\nfor flow in results.get(\\\"content\\\")[\\\"events\\\"]:\\n  qradar_event = incident.addRow(\\\"qr_flows\\\")\\n  qradar_event.application = link.format(results.get(\\\"content\\\")[\\\"offenseid\\\"], \\\"application_name\\\", flow.Application, flow.Application)\\n  qradar_event.source_ip = link.format(results.get(\\\"content\\\")[\\\"offenseid\\\"], \\\"sourceip\\\", flow.sourceip, flow.sourceip)\\n  qradar_event.source_port = link.format(results.get(\\\"content\\\")[\\\"offenseid\\\"], \\\"sourceport\\\", flow.sourceport, flow.sourceport)\\n  qradar_event.destination_ip = link.format(results.get(\\\"content\\\")[\\\"offenseid\\\"], \\\"destinationip\\\", flow.destinationip, flow.destinationip)\\n  qradar_event.destination_port = link.format(results.get(\\\"content\\\")[\\\"offenseid\\\"], \\\"destinationport\\\", flow.destinationport, flow.destinationport)\\n  qradar_event.protocol = flow.Protocol\\n  qradar_event.first_packet_time = int(flow.FirstPacketTime)\\n  qradar_event.source_bytes = int(flow.sourcebytes)\\n  qradar_event.source_packets = int(flow.sourcepackets)\\n  qradar_event.destination_bytes = int(flow.destinationbytes)\\n  qradar_event.destination_packets = int(flow.destinationpackets)\\n  qradar_event.reported_time = results.get(\\\"content\\\")[\\\"current_time\\\"]\",\"post_processing_script_language\":\"python\",\"pre_processing_script\":\"inputs.qradar_search_param3 = incident.properties.qradar_id\\ninputs.qradar_query_type = \\\"flows\\\"\\ninputs.qradar_label = incident.properties.qradar_destination\\ninputs.soar_incident_id = incident.id\\ninputs.soar_table_name = \\\"qr_flows\\\"\\n\\n# QRadar graphql search look back time default is 5 days\\ninputs.qradar_search_param7 = \\\"5 days\\\"\\n# If the poller is running and the qr_last_updated_time is changed the\\n# the QRadar graphql look back time will change to 2 days\\nif incident.properties.qr_last_updated_time != incident.create_date:\\n  inputs.qradar_search_param7 = \\\"2 days\\\"\\n# If manual QRadar Update rule is run set the number if days to search to the\\n# user entered number\\nif rule.properties.number_of_days_to_search:\\n  inputs.qradar_search_param7 = str(rule.properties.number_of_days_to_search) + \\\" days\\\"\",\"pre_processing_script_language\":\"python\",\"result_name\":\"\"}\u003c/resilient:function\u003e\u003c/extensionElements\u003e\u003cincoming\u003eSequenceFlow_1f2h23f\u003c/incoming\u003e\u003coutgoing\u003eSequenceFlow_05ysyi1\u003c/outgoing\u003e\u003c/serviceTask\u003e\u003csequenceFlow id=\"SequenceFlow_1f2h23f\" sourceRef=\"StartEvent_155asxm\" targetRef=\"ServiceTask_0zpihlh\"/\u003e\u003cendEvent id=\"EndEvent_0om05cp\"\u003e\u003cincoming\u003eSequenceFlow_05ysyi1\u003c/incoming\u003e\u003c/endEvent\u003e\u003csequenceFlow id=\"SequenceFlow_05ysyi1\" sourceRef=\"ServiceTask_0zpihlh\" targetRef=\"EndEvent_0om05cp\"/\u003e\u003ctextAnnotation id=\"TextAnnotation_1kxxiyt\"\u003e\u003ctext\u003eStart your workflow here\u003c/text\u003e\u003c/textAnnotation\u003e\u003cassociation id=\"Association_1seuj48\" sourceRef=\"StartEvent_155asxm\" targetRef=\"TextAnnotation_1kxxiyt\"/\u003e\u003c/process\u003e\u003cbpmndi:BPMNDiagram id=\"BPMNDiagram_1\"\u003e\u003cbpmndi:BPMNPlane bpmnElement=\"undefined\" id=\"BPMNPlane_1\"\u003e\u003cbpmndi:BPMNShape bpmnElement=\"StartEvent_155asxm\" id=\"StartEvent_155asxm_di\"\u003e\u003comgdc:Bounds height=\"36\" width=\"36\" x=\"162\" y=\"188\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"0\" width=\"90\" x=\"157\" y=\"223\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNShape bpmnElement=\"TextAnnotation_1kxxiyt\" id=\"TextAnnotation_1kxxiyt_di\"\u003e\u003comgdc:Bounds height=\"30\" width=\"100\" x=\"99\" y=\"254\"/\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"Association_1seuj48\" id=\"Association_1seuj48_di\"\u003e\u003comgdi:waypoint x=\"169\" xsi:type=\"omgdc:Point\" y=\"220\"/\u003e\u003comgdi:waypoint x=\"153\" xsi:type=\"omgdc:Point\" y=\"254\"/\u003e\u003c/bpmndi:BPMNEdge\u003e\u003cbpmndi:BPMNShape bpmnElement=\"ServiceTask_0zpihlh\" id=\"ServiceTask_0zpihlh_di\"\u003e\u003comgdc:Bounds height=\"80\" width=\"100\" x=\"339\" y=\"166\"/\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"SequenceFlow_1f2h23f\" id=\"SequenceFlow_1f2h23f_di\"\u003e\u003comgdi:waypoint x=\"198\" xsi:type=\"omgdc:Point\" y=\"206\"/\u003e\u003comgdi:waypoint x=\"339\" xsi:type=\"omgdc:Point\" y=\"206\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"14\" width=\"0\" x=\"268.5\" y=\"184\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNEdge\u003e\u003cbpmndi:BPMNShape bpmnElement=\"EndEvent_0om05cp\" id=\"EndEvent_0om05cp_di\"\u003e\u003comgdc:Bounds height=\"36\" width=\"36\" x=\"653\" y=\"188\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"14\" width=\"0\" x=\"671\" y=\"227\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"SequenceFlow_05ysyi1\" id=\"SequenceFlow_05ysyi1_di\"\u003e\u003comgdi:waypoint x=\"439\" xsi:type=\"omgdc:Point\" y=\"206\"/\u003e\u003comgdi:waypoint x=\"653\" xsi:type=\"omgdc:Point\" y=\"206\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"14\" width=\"0\" x=\"546\" y=\"184\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNEdge\u003e\u003c/bpmndi:BPMNPlane\u003e\u003c/bpmndi:BPMNDiagram\u003e\u003c/definitions\u003e"
      },
      "content_version": 6,
      "description": "Use the qradar_id field of the incident to searchQRadar events, and update the data table, qr_offense_top_events, with the top 10 events.",
      "export_key": "example_of_searching_qradar_flows_using_offense_id",
      "last_modified_by": "d@example.com",
      "last_modified_time": 1666186122668,
      "name": "Example of searching QRadar Flows using offense id",
      "object_type": "incident",
      "programmatic_name": "example_of_searching_qradar_flows_using_offense_id",
      "tags": [],
      "uuid": "248ce036-9d02-4982-b4bd-541d17c23a3d",
      "workflow_id": 56
    },
    {
      "actions": [],
      "content": {
        "version": 14,
        "workflow_id": "example_qradar_get_mitre_reference_from_rules",
        "xml": "\u003c?xml version=\"1.0\" encoding=\"UTF-8\"?\u003e\u003cdefinitions xmlns=\"http://www.omg.org/spec/BPMN/20100524/MODEL\" xmlns:bpmndi=\"http://www.omg.org/spec/BPMN/20100524/DI\" xmlns:omgdc=\"http://www.omg.org/spec/DD/20100524/DC\" xmlns:omgdi=\"http://www.omg.org/spec/DD/20100524/DI\" xmlns:resilient=\"http://resilient.ibm.com/bpmn\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" targetNamespace=\"http://www.camunda.org/test\"\u003e\u003cprocess id=\"example_qradar_get_mitre_reference_from_rules\" isExecutable=\"true\" name=\"Example: QRadar - Get MITRE Reference From Rules\"\u003e\u003cdocumentation\u003eUsing the QRadar ID for the offense in question, this workflow will go and retrieve the related MITRE Tactics and Techniques from QRadar based on the defined rules.\u003c/documentation\u003e\u003cstartEvent id=\"StartEvent_155asxm\"\u003e\u003coutgoing\u003eSequenceFlow_0niqtas\u003c/outgoing\u003e\u003c/startEvent\u003e\u003cserviceTask id=\"ServiceTask_1djo2ij\" name=\"QRadar Get Offense MITRE Referenc...\" resilient:type=\"function\"\u003e\u003cextensionElements\u003e\u003cresilient:function uuid=\"c72d73d8-29de-4166-875e-427e3add0859\"\u003e{\"inputs\":{},\"post_processing_script\":\"for item in results.content[\u0027rules\u0027]:\\n  mapping = item[\u0027mapping\u0027]\\n  if mapping:\\n    for tactic in list(mapping):\\n      techniques = mapping[tactic][\u0027techniques\u0027]\\n      if techniques:\\n        for technique in list(techniques):\\n          new_row = incident.addRow(\u0027qradar_rules_and_mitre_tactics_and_techniques\u0027)\\n          new_row.rule_id = item[\u0027id\u0027]\\n          new_row.rule_identifier = item[\u0027identifier\u0027]\\n          new_row.rule_name = item[\u0027name\u0027]\\n          new_row.mitre_tactic = tactic\\n          new_row.mitre_tactic_id = mapping[tactic][\u0027id\u0027]\\n          new_row.tactic_confidence_level = mapping[tactic][\u0027confidence\u0027]\\n          new_row.mitre_technique = technique\\n          new_row.mitre_technique_id = techniques[technique][\u0027id\u0027]\\n          new_row.technique_confidence_level = techniques[technique][\u0027confidence\u0027]\\n      else:\\n        new_row = incident.addRow(\u0027qradar_rules_and_mitre_tactics_and_techniques\u0027)\\n        new_row.rule_id = item[\u0027id\u0027]\\n        new_row.rule_identifier = item[\u0027identifier\u0027]\\n        new_row.rule_name = item[\u0027name\u0027]\\n        new_row.mitre_tactic = tactic\\n        new_row.mitre_tactic_id = mapping[tactic][\u0027id\u0027]\\n        new_row.tactic_confidence_level = mapping[tactic][\u0027confidence\u0027]\\n  else:\\n    new_row = incident.addRow(\u0027qradar_rules_and_mitre_tactics_and_techniques\u0027)\\n    new_row.rule_id = item[\u0027id\u0027]\\n    new_row.rule_identifier = item[\u0027identifier\u0027]\\n    new_row.rule_name = item[\u0027name\u0027]\",\"post_processing_script_language\":\"python\",\"pre_processing_script\":\"inputs.qradar_offense_id = incident.properties.qradar_id\\ninputs.qradar_label = incident.properties.qradar_destination\\ninputs.soar_incident_id = incident.id\\ninputs.soar_table_name = \\\"qradar_rules_and_mitre_tactics_and_techniques\\\"\",\"pre_processing_script_language\":\"python\"}\u003c/resilient:function\u003e\u003c/extensionElements\u003e\u003cincoming\u003eSequenceFlow_0niqtas\u003c/incoming\u003e\u003coutgoing\u003eSequenceFlow_09gew9g\u003c/outgoing\u003e\u003c/serviceTask\u003e\u003csequenceFlow id=\"SequenceFlow_0niqtas\" sourceRef=\"StartEvent_155asxm\" targetRef=\"ServiceTask_1djo2ij\"/\u003e\u003cendEvent id=\"EndEvent_070qbbl\"\u003e\u003cincoming\u003eSequenceFlow_09gew9g\u003c/incoming\u003e\u003c/endEvent\u003e\u003csequenceFlow id=\"SequenceFlow_09gew9g\" sourceRef=\"ServiceTask_1djo2ij\" targetRef=\"EndEvent_070qbbl\"/\u003e\u003ctextAnnotation id=\"TextAnnotation_1kxxiyt\"\u003e\u003ctext\u003eStart your workflow here\u003c/text\u003e\u003c/textAnnotation\u003e\u003cassociation id=\"Association_1seuj48\" sourceRef=\"StartEvent_155asxm\" targetRef=\"TextAnnotation_1kxxiyt\"/\u003e\u003c/process\u003e\u003cbpmndi:BPMNDiagram id=\"BPMNDiagram_1\"\u003e\u003cbpmndi:BPMNPlane bpmnElement=\"undefined\" id=\"BPMNPlane_1\"\u003e\u003cbpmndi:BPMNShape bpmnElement=\"StartEvent_155asxm\" id=\"StartEvent_155asxm_di\"\u003e\u003comgdc:Bounds height=\"36\" width=\"36\" x=\"162\" y=\"188\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"0\" width=\"90\" x=\"157\" y=\"223\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNShape bpmnElement=\"TextAnnotation_1kxxiyt\" id=\"TextAnnotation_1kxxiyt_di\"\u003e\u003comgdc:Bounds height=\"30\" width=\"100\" x=\"99\" y=\"254\"/\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"Association_1seuj48\" id=\"Association_1seuj48_di\"\u003e\u003comgdi:waypoint x=\"169\" xsi:type=\"omgdc:Point\" y=\"220\"/\u003e\u003comgdi:waypoint x=\"153\" xsi:type=\"omgdc:Point\" y=\"254\"/\u003e\u003c/bpmndi:BPMNEdge\u003e\u003cbpmndi:BPMNShape bpmnElement=\"ServiceTask_1djo2ij\" id=\"ServiceTask_1djo2ij_di\"\u003e\u003comgdc:Bounds height=\"80\" width=\"100\" x=\"276\" y=\"166\"/\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"SequenceFlow_0niqtas\" id=\"SequenceFlow_0niqtas_di\"\u003e\u003comgdi:waypoint x=\"198\" xsi:type=\"omgdc:Point\" y=\"206\"/\u003e\u003comgdi:waypoint x=\"276\" xsi:type=\"omgdc:Point\" y=\"206\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"13\" width=\"0\" x=\"237\" y=\"184\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNEdge\u003e\u003cbpmndi:BPMNShape bpmnElement=\"EndEvent_070qbbl\" id=\"EndEvent_070qbbl_di\"\u003e\u003comgdc:Bounds height=\"36\" width=\"36\" x=\"439\" y=\"188\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"13\" width=\"0\" x=\"457\" y=\"227\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"SequenceFlow_09gew9g\" id=\"SequenceFlow_09gew9g_di\"\u003e\u003comgdi:waypoint x=\"376\" xsi:type=\"omgdc:Point\" y=\"206\"/\u003e\u003comgdi:waypoint x=\"439\" xsi:type=\"omgdc:Point\" y=\"206\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"13\" width=\"0\" x=\"407.5\" y=\"184\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNEdge\u003e\u003c/bpmndi:BPMNPlane\u003e\u003c/bpmndi:BPMNDiagram\u003e\u003c/definitions\u003e"
      },
      "content_version": 14,
      "description": "Using the QRadar ID for the offense in question, this workflow will go and retrieve the related MITRE Tactics and Techniques from QRadar based on the defined rules.",
      "export_key": "example_qradar_get_mitre_reference_from_rules",
      "last_modified_by": "d@example.com",
      "last_modified_time": 1666186269001,
      "name": "Example: QRadar - Get MITRE Reference From Rules",
      "object_type": "incident",
      "programmatic_name": "example_qradar_get_mitre_reference_from_rules",
      "tags": [],
      "uuid": "6eda4f06-f073-4f06-8666-6b5c38988487",
      "workflow_id": 64
    },
    {
      "actions": [],
      "content": {
        "version": 7,
        "workflow_id": "qradar_destination_ips",
        "xml": "\u003c?xml version=\"1.0\" encoding=\"UTF-8\"?\u003e\u003cdefinitions xmlns=\"http://www.omg.org/spec/BPMN/20100524/MODEL\" xmlns:bpmndi=\"http://www.omg.org/spec/BPMN/20100524/DI\" xmlns:omgdc=\"http://www.omg.org/spec/DD/20100524/DC\" xmlns:omgdi=\"http://www.omg.org/spec/DD/20100524/DI\" xmlns:resilient=\"http://resilient.ibm.com/bpmn\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" targetNamespace=\"http://www.camunda.org/test\"\u003e\u003cprocess id=\"qradar_destination_ips\" isExecutable=\"true\" name=\"Example of searching and returning Destination IPs information\"\u003e\u003cdocumentation\u003eReturns the information on the Destination IPS associated with the QRadar Offense ID\u003c/documentation\u003e\u003cstartEvent id=\"StartEvent_155asxm\"\u003e\u003coutgoing\u003eSequenceFlow_1ie3uhp\u003c/outgoing\u003e\u003coutgoing\u003eSequenceFlow_1uuhodb\u003c/outgoing\u003e\u003c/startEvent\u003e\u003cserviceTask id=\"ServiceTask_0wa32wd\" name=\"QRadar Top Events\" resilient:type=\"function\"\u003e\u003cextensionElements\u003e\u003cresilient:function uuid=\"d8e63812-0b58-4ca1-991a-49255f3dd04e\"\u003e{\"inputs\":{\"048ba39a-ab94-4d1f-a0f8-2462de3c044c\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_content_value\":{\"format\":\"text\",\"content\":\"SELECT %param1% FROM events %param2% %param4% %param5% %param6% LAST %param7% PARAMETERS PROGRESSDETAILSRESOLUTION=60\"}}},\"01ed4652-fe61-4dbf-bc83-f81c62c5563c\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"destinationip as destinationip,UNIQUECOUNT(category) as categorycount,SUM(eventcount) as eventcount,max(starttime)\"}},\"9d8fc645-5067-4ab0-b151-1d106dad91d6\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"WHERE INOFFENSE(%param3%)\"}},\"eefc9e56-2806-4534-b409-42e25154ecc9\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"GROUP BY destinationip \"}},\"19778fc0-9a30-4b8b-9c21-642495f9eebc\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"ORDER BY max(starttime) DESC\"}},\"ca2d7d7f-fe6d-4a21-b011-7953494c350c\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"LIMIT 10\"}}},\"post_processing_script\":\"link = \\\"\u0026lt;a href=\\\\\\\"https://\\\" + results.get(\\\"content\\\")[\\\"qrhost\\\"] + \\\"/console/ui/offenses/{0}/events?filter={1}%3B%3D%3B%3B{2}\u0026amp;page=1\u0026amp;pagesize=10\\\\\\\" target=\\\\\\\"_blank\\\\\\\"\u0026gt;{3}\u0026lt;/a\u0026gt;\\\"\\n\\nfor event in results.get(\\\"content\\\")[\\\"events\\\"]:\\n  qradar_event = incident.addRow(\\\"qr_top_destination_ips\\\")\\n  qradar_event.destination_ip = link.format(results.get(\\\"content\\\")[\\\"offenseid\\\"], \\\"destinationip\\\", event.destinationip, event.destinationip)\\n  qradar_event.event_count = link.format(results.get(\\\"content\\\")[\\\"offenseid\\\"], \\\"destinationip\\\", event.destinationip, event.eventcount)\\n  qradar_event.category_count = link.format(results.get(\\\"content\\\")[\\\"offenseid\\\"], \\\"destinationip\\\", event.destinationip, event.categorycount)\\n  qradar_event.reported_time = results.get(\\\"content\\\")[\\\"current_time\\\"]\",\"post_processing_script_language\":\"python\",\"pre_processing_script\":\"inputs.qradar_search_param3 = incident.properties.qradar_id\\ninputs.qradar_query_type = \\\"destinationip\\\"\\ninputs.qradar_label = incident.properties.qradar_destination\\ninputs.soar_incident_id = incident.id\\ninputs.soar_table_name = \\\"qr_top_destination_ips\\\"\\n\\n# QRadar graphql search look back time default is 5 days\\ninputs.qradar_search_param7 = \\\"5 days\\\"\\n# If the poller is running and the qr_last_updated_time is changed the\\n# the QRadar graphql look back time will change to 2 days\\nif incident.properties.qr_last_updated_time != incident.create_date:\\n  inputs.qradar_search_param7 = \\\"2 days\\\"\\n# If manual QRadar Update rule is run set the number if days to search to the\\n# user entered number\\nif rule.properties.number_of_days_to_search:\\n  inputs.qradar_search_param7 = str(rule.properties.number_of_days_to_search) + \\\" days\\\"\",\"pre_processing_script_language\":\"python\",\"result_name\":\"\"}\u003c/resilient:function\u003e\u003c/extensionElements\u003e\u003cincoming\u003eSequenceFlow_1ie3uhp\u003c/incoming\u003e\u003coutgoing\u003eSequenceFlow_1ge3qyy\u003c/outgoing\u003e\u003c/serviceTask\u003e\u003csequenceFlow id=\"SequenceFlow_1ie3uhp\" sourceRef=\"StartEvent_155asxm\" targetRef=\"ServiceTask_0wa32wd\"/\u003e\u003cendEvent id=\"EndEvent_0tf1zg5\"\u003e\u003cincoming\u003eSequenceFlow_1ge3qyy\u003c/incoming\u003e\u003c/endEvent\u003e\u003csequenceFlow id=\"SequenceFlow_1ge3qyy\" sourceRef=\"ServiceTask_0wa32wd\" targetRef=\"EndEvent_0tf1zg5\"/\u003e\u003cserviceTask id=\"ServiceTask_164pxep\" name=\"QRadar Top Events\" resilient:type=\"function\"\u003e\u003cextensionElements\u003e\u003cresilient:function uuid=\"d8e63812-0b58-4ca1-991a-49255f3dd04e\"\u003e{\"inputs\":{\"048ba39a-ab94-4d1f-a0f8-2462de3c044c\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_content_value\":{\"format\":\"text\",\"content\":\"SELECT %param1% FROM flows %param2% %param4% %param5% %param6% LAST %param7% PARAMETERS PROGRESSDETAILSRESOLUTION=60\"}}},\"01ed4652-fe61-4dbf-bc83-f81c62c5563c\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"destinationip as destinationip,UNIQUECOUNT(category) as categorycount,COUNT(*) as flowcount,max(lastpackettime)\"}},\"9d8fc645-5067-4ab0-b151-1d106dad91d6\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"WHERE INOFFENSE(%param3%)\"}},\"eefc9e56-2806-4534-b409-42e25154ecc9\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"GROUP BY destinationip \"}},\"19778fc0-9a30-4b8b-9c21-642495f9eebc\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"ORDER BY max(lastpackettime) DESC\"}},\"ca2d7d7f-fe6d-4a21-b011-7953494c350c\":{\"input_type\":\"static\",\"static_input\":{\"multiselect_value\":[],\"text_value\":\"LIMIT 10\"}}},\"post_processing_script\":\"link = \\\"\u0026lt;a href=\\\\\\\"https://\\\" + results.get(\\\"content\\\")[\\\"qrhost\\\"] + \\\"/console/ui/offenses/{0}/events?filter={1}%3B%3D%3B%3B{2}\u0026amp;page=1\u0026amp;pagesize=10\\\\\\\" target=\\\\\\\"_blank\\\\\\\"\u0026gt;{3}\u0026lt;/a\u0026gt;\\\"\\n\\nfor event in results.get(\\\"content\\\")[\\\"events\\\"]:\\n  qradar_event = incident.addRow(\\\"qr_top_destination_ips\\\")\\n  qradar_event.destination_ip = event.destinationip\\n  qradar_event.flow_count = event.flowcount\\n  qradar_event.category_count = event.categorycount\\n  qradar_event.reported_time = results.get(\\\"content\\\")[\\\"current_time\\\"]\",\"post_processing_script_language\":\"python\",\"pre_processing_script\":\"inputs.qradar_search_param3 = incident.properties.qradar_id\\ninputs.qradar_query_type = \\\"destinationip\\\"\\ninputs.qradar_label = incident.properties.qradar_destination\\ninputs.soar_incident_id = incident.id\\ninputs.soar_table_name = \\\"qr_top_destination_ips\\\"\\n\\n# QRadar graphql search look back time default is 5 days\\ninputs.qradar_search_param7 = \\\"5 days\\\"\\n# If the poller is running and the qr_last_updated_time is changed the\\n# the QRadar graphql look back time will change to 2 days\\nif incident.properties.qr_last_updated_time != incident.create_date:\\n  inputs.qradar_search_param7 = \\\"2 days\\\"\\n# If manual QRadar Update rule is run set the number if days to search to the\\n# user entered number\\nif rule.properties.number_of_days_to_search:\\n  inputs.qradar_search_param7 = str(rule.properties.number_of_days_to_search) + \\\" days\\\"\",\"pre_processing_script_language\":\"python\",\"result_name\":\"\"}\u003c/resilient:function\u003e\u003c/extensionElements\u003e\u003cincoming\u003eSequenceFlow_1uuhodb\u003c/incoming\u003e\u003coutgoing\u003eSequenceFlow_16irdjm\u003c/outgoing\u003e\u003c/serviceTask\u003e\u003csequenceFlow id=\"SequenceFlow_1uuhodb\" sourceRef=\"StartEvent_155asxm\" targetRef=\"ServiceTask_164pxep\"/\u003e\u003cendEvent id=\"EndEvent_1707dmc\"\u003e\u003cincoming\u003eSequenceFlow_16irdjm\u003c/incoming\u003e\u003c/endEvent\u003e\u003csequenceFlow id=\"SequenceFlow_16irdjm\" sourceRef=\"ServiceTask_164pxep\" targetRef=\"EndEvent_1707dmc\"/\u003e\u003ctextAnnotation id=\"TextAnnotation_1kxxiyt\"\u003e\u003ctext\u003eStart your workflow here\u003c/text\u003e\u003c/textAnnotation\u003e\u003cassociation id=\"Association_1seuj48\" sourceRef=\"StartEvent_155asxm\" targetRef=\"TextAnnotation_1kxxiyt\"/\u003e\u003c/process\u003e\u003cbpmndi:BPMNDiagram id=\"BPMNDiagram_1\"\u003e\u003cbpmndi:BPMNPlane bpmnElement=\"undefined\" id=\"BPMNPlane_1\"\u003e\u003cbpmndi:BPMNShape bpmnElement=\"StartEvent_155asxm\" id=\"StartEvent_155asxm_di\"\u003e\u003comgdc:Bounds height=\"36\" width=\"36\" x=\"162\" y=\"188\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"0\" width=\"90\" x=\"157\" y=\"223\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNShape bpmnElement=\"TextAnnotation_1kxxiyt\" id=\"TextAnnotation_1kxxiyt_di\"\u003e\u003comgdc:Bounds height=\"30\" width=\"100\" x=\"99\" y=\"254\"/\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"Association_1seuj48\" id=\"Association_1seuj48_di\"\u003e\u003comgdi:waypoint x=\"169\" xsi:type=\"omgdc:Point\" y=\"220\"/\u003e\u003comgdi:waypoint x=\"153\" xsi:type=\"omgdc:Point\" y=\"254\"/\u003e\u003c/bpmndi:BPMNEdge\u003e\u003cbpmndi:BPMNShape bpmnElement=\"ServiceTask_0wa32wd\" id=\"ServiceTask_0wa32wd_di\"\u003e\u003comgdc:Bounds height=\"80\" width=\"100\" x=\"326\" y=\"152\"/\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"SequenceFlow_1ie3uhp\" id=\"SequenceFlow_1ie3uhp_di\"\u003e\u003comgdi:waypoint x=\"198\" xsi:type=\"omgdc:Point\" y=\"206\"/\u003e\u003comgdi:waypoint x=\"262\" xsi:type=\"omgdc:Point\" y=\"206\"/\u003e\u003comgdi:waypoint x=\"262\" xsi:type=\"omgdc:Point\" y=\"192\"/\u003e\u003comgdi:waypoint x=\"326\" xsi:type=\"omgdc:Point\" y=\"192\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"13\" width=\"0\" x=\"277\" y=\"192\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNEdge\u003e\u003cbpmndi:BPMNShape bpmnElement=\"EndEvent_0tf1zg5\" id=\"EndEvent_0tf1zg5_di\"\u003e\u003comgdc:Bounds height=\"36\" width=\"36\" x=\"559\" y=\"174\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"13\" width=\"0\" x=\"577\" y=\"213\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"SequenceFlow_1ge3qyy\" id=\"SequenceFlow_1ge3qyy_di\"\u003e\u003comgdi:waypoint x=\"426\" xsi:type=\"omgdc:Point\" y=\"192\"/\u003e\u003comgdi:waypoint x=\"559\" xsi:type=\"omgdc:Point\" y=\"192\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"13\" width=\"0\" x=\"492.5\" y=\"170\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNEdge\u003e\u003cbpmndi:BPMNShape bpmnElement=\"ServiceTask_164pxep\" id=\"ServiceTask_164pxep_di\"\u003e\u003comgdc:Bounds height=\"80\" width=\"100\" x=\"326\" y=\"268\"/\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"SequenceFlow_1uuhodb\" id=\"SequenceFlow_1uuhodb_di\"\u003e\u003comgdi:waypoint x=\"189\" xsi:type=\"omgdc:Point\" y=\"222\"/\u003e\u003comgdi:waypoint x=\"262\" xsi:type=\"omgdc:Point\" y=\"308\"/\u003e\u003comgdi:waypoint x=\"326\" xsi:type=\"omgdc:Point\" y=\"308\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"14\" width=\"0\" x=\"225.5\" y=\"243\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNEdge\u003e\u003cbpmndi:BPMNShape bpmnElement=\"EndEvent_1707dmc\" id=\"EndEvent_1707dmc_di\"\u003e\u003comgdc:Bounds height=\"36\" width=\"36\" x=\"559\" y=\"290\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"14\" width=\"0\" x=\"577\" y=\"329\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNShape\u003e\u003cbpmndi:BPMNEdge bpmnElement=\"SequenceFlow_16irdjm\" id=\"SequenceFlow_16irdjm_di\"\u003e\u003comgdi:waypoint x=\"426\" xsi:type=\"omgdc:Point\" y=\"308\"/\u003e\u003comgdi:waypoint x=\"559\" xsi:type=\"omgdc:Point\" y=\"308\"/\u003e\u003cbpmndi:BPMNLabel\u003e\u003comgdc:Bounds height=\"14\" width=\"0\" x=\"492.5\" y=\"286\"/\u003e\u003c/bpmndi:BPMNLabel\u003e\u003c/bpmndi:BPMNEdge\u003e\u003c/bpmndi:BPMNPlane\u003e\u003c/bpmndi:BPMNDiagram\u003e\u003c/definitions\u003e"
      },
      "content_version": 7,
      "description": "Returns the information on the Destination IPS associated with the QRadar Offense ID",
      "export_key": "qradar_destination_ips",
      "last_modified_by": "d@example.com",
      "last_modified_time": 1666185863891,
      "name": "Example of searching and returning Destination IPs information",
      "object_type": "incident",
      "programmatic_name": "qradar_destination_ips",
      "tags": [],
      "uuid": "c2c032ad-3789-49cd-892c-81112eabc81c",
      "workflow_id": 59
    }
  ],
  "workspaces": []
}
