...
def call_umbrella_timeline_function(circuits, function_params, timeout=5):
...
class TestUmbrellaTimeline:
    ...
    mock_inputs_1 = {
        "umbinv_resource": "googlevideo.com"
    }

    expected_results_1 = mock_umbrella.timeline_return
    ...
    def test_success(self, circuits_app, mock_inputs, expected_results):
        ...
        with patch("fn_cisco_umbrella_inv.components.umbrella_timeline.investigateClient") as patch_client:
            patch_client.return_value = mock_umbrella.mock_client()
            results = call_umbrella_timeline_function(circuits_app, mock_inputs)
            assert(expected_results == results.get("content", {}).get("timeline"))
