...
def call_umbrella_domain_related_domains_function(circuits, function_params, timeout=5):
...
class TestUmbrellaDomainRelatedDomains:
    ...
    expected_results_1 = mock_umbrella.related_return
    ...
    def test_success(self, circuits_app, mock_inputs, expected_results):
        ...
        with patch("fn_cisco_umbrella_inv.components.umbrella_domain_related_domains.investigateClient") as patch_client:
            patch_client.return_value = mock_umbrella.mock_client()
            results = call_umbrella_domain_related_domains_function(circuits_app, mock_inputs)
            assert(expected_results == results.get("content", {}).get("related_domains", {}))
