...
def call_umbrella_domain_security_info_function(circuits, function_params, timeout=5):
...
class TestUmbrellaDomainSecurityInfo:
    ...
    expected_results_1 = mock_umbrella.security_return
    ...
    def test_success(self, circuits_app, mock_inputs, expected_results):
        ...
        with patch("fn_cisco_umbrella_inv.components.umbrella_domain_security_info.investigateClient") as patch_client:
            patch_client.return_value = mock_umbrella.mock_client()
            results = call_umbrella_domain_security_info_function(circuits_app, mock_inputs)
            assert(expected_results == results.get("content", {}).get("security_info", {}))
