...
def call_umbrella_ip_latest_malicious_domains_function(circuits, function_params, timeout=5):
...
class TestUmbrellaIpLatestMaliciousDomains:
    ...
    mock_inputs_1 = {
        "umbinv_ipaddr": "1.2.3.4"
    }

    expected_results_1 = ['www.cxhyly.com', 'cxhyly.com']
    ...
    def test_success(self, circuits_app, mock_inputs, expected_results):
        ...
        with patch("fn_cisco_umbrella_inv.components.umbrella_ip_latest_malicious_domains.investigateClient") as patch_client:
            patch_client.return_value = mock_umbrella.mock_client()
            results = call_umbrella_ip_latest_malicious_domains_function(circuits_app, mock_inputs)
            assert(expected_results == results.get("content", {}).get("latest_malicious_domains"))
