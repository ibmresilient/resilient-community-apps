...
def call_fn_aws_iam_add_user_to_groups_function(circuits, function_params, timeout=10):
    # Fire a message to the function
    evt = SubmitTestFunction("fn_aws_iam_add_user_to_groups", function_params)
    circuits.manager.fire(evt)
    event = circuits.watcher.wait("fn_aws_iam_add_user_to_groups_result", parent=evt, timeout=timeout)
    assert event
...
class TestFnAwsIamAddUserToGroups:
    """ Tests for the fn_aws_iam_add_user_to_groups function"""
