...
FUNCTION_NAME = "fn_aws_iam_list_user_access_key_ids"
...
def call_fn_aws_iam_list_user_access_key_ids_function(circuits, function_params, timeout=10):
    # Fire a message to the function
    evt = SubmitTestFunction("fn_aws_iam_list_user_access_key_ids", function_params)
    circuits.manager.fire(evt)
    event = circuits.watcher.wait("fn_aws_iam_list_user_access_key_ids_result", parent=evt, timeout=timeout)
    assert event
...
class TestFnAwsIamListUserAccessKeyIds:
    """ Tests for the fn_aws_iam_list_user_access_key_ids function"""
    ...
    def test_success(self, mock_post, circuits_app, aws_iam_user_name, expected_results):
        ...
        results = call_fn_aws_iam_list_user_access_key_ids_function(circuits_app, function_params)
