...
def call_fn_watsonx_analyst_converse_via_notes_function(circuits, function_params, timeout=5):
    ...
    # else return the FunctionComponent's results
        ...
        event = circuits.watcher.wait("fn_watsonx_analyst_converse_via_notes_result", parent=evt, timeout=timeout)
        assert event
...
class TestFnWatsonxConverseViaNotes:
    ...
    @pytest.mark.parametrize("mock_inputs, expected_results", [
        (mock_inputs_2, artifact_expected_results)
    ])
