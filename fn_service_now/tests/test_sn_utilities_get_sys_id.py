...
FUNCTION_NAME = "sn_utilities_get_sys_id"
...
def call_sn_utilities_get_sys_id_function(circuits, function_params, timeout=10):
    # Fire a message to the function
    evt = SubmitTestFunction("sn_utilities_get_sys_id", function_params)
    circuits.manager.fire(evt)
    event = circuits.watcher.wait("sn_utilities_get_sys_id_result", parent=evt, timeout=timeout)
    assert event
    assert isinstance(event.kwargs["result"], FunctionResult)
    pytest.wait_for(event, "complete", True)
    return event.kwargs["result"].value

class TestSnUtilitiesGetSysId:
    """ Tests for the sn_utilities_get_sys_id function"""
    ...
    def test_success(self, circuits_app, inputs, expected_results):
        ...
        results = call_sn_utilities_get_sys_id_function(circuits_app, inputs)
